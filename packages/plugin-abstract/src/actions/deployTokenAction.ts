...
export const deployTokenAction: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            const receipt = await abstractPublicClient.waitForTransactionReceipt({ hash });
            const contractAddress = receipt.contractAddress;




            elizaLogger.success(`Token deployment completed! Contract address: ${contractAddress}. Transaction hash: ${hash}`);
            if (callback) {
                callback({
                    ...
                    content: { hash, tokenName: result.data.name, tokenSymbol: result.data.symbol, contractAddress, transactionHash: hash },
                });
            }
