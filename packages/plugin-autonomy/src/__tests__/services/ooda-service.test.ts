...
mock.module('../../resource-monitor', () => ({
  ResourceMonitorService: {
    start: mock().mockResolvedValue(
      createMockService('resource-monitor', {
        getCpuUsage: mock().mockReturnValue(45.5),
        getMemoryUsage: mock().mockReturnValue(60.2),
        getDiskUsage: mock().mockResolvedValue(75.8),
      })
    ),
  },
}));
...
describe('OODALoopService', () => {
  ...
  describe('error handling', () => {
    ...
    it('should handle phase execution errors gracefully', async () => {
      ...
      (service as any).orientPhase = mock().mockRejectedValue(new Error('Orient phase error'));
    ...
    it('should continue operation after individual phase failures', async () => {
      ...
      (service as any).observePhase = mock().mockRejectedValue(new Error('Observe error'));
      (service as any).orientPhase = mock().mockRejectedValue(new Error('Orient error'));
