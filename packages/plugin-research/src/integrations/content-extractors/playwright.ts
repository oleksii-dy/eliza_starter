...
export class PlaywrightContentExtractor {
  ...
  async initialize(): Promise<void> {
    if (!this.browser) {
      logger.info('[Playwright] Initializing browser');
  ...
  async extractContent(url: string, retryCount: number = 0): Promise<ExtractedContent | null> {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      logger.error(`[Playwright] Extraction error after ${duration}ms:`, error);
  ...
  async extractBatch(urls: string[]): Promise<Map<string, ExtractedContent | null>> {
    ...
    for (const url of urls) {
      try {
        ...
        logger.error(`[Playwright] Failed to extract ${url}:`, error);
  ...
  async screenshot(url: string, outputPath: string): Promise<boolean> {
    ...
    try {
      ...
      logger.info(`[Playwright] Screenshot saved to ${outputPath}`);
      return true;
    } catch (error) {
  ...
  async pdf(url: string, outputPath: string): Promise<boolean> {
    ...
    try {
      ...
      logger.info(`[Playwright] PDF saved to ${outputPath}`);
      return true;
    } catch (error) {
      logger.error('[Playwright] PDF error:', error);
      return false;
    } finally {
      if (page) {
        await page.close().catch(() => {});
