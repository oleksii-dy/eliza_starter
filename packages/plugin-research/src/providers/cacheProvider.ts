...
export const cacheProvider: Provider = {
  name: 'cacheProvider',
  description:
    'Provides in-memory cache status including hit/miss statistics, operational state, and performance metrics when agent needs to understand cache utilization or make caching-related decisions',
...
export function cacheMiddleware(runtime: ExtendedRuntime) {
  ...
  runtime.composeState = async (
    ...
    if (!runtime.cacheManager) {
      return originalComposeState.call(
        runtime,
        message,
        includeList,
        onlyInclude
  ...
  runtime.processActions = async (
    ...
    const result = await originalProcessAction.call(
      runtime,
      message,
      responses,
      state,
