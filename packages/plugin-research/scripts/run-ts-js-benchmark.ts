...
async function createBenchmarkRuntime(): Promise<IAgentRuntime> {
  const runtime = {
    ...
    character: {
      ...
      bio: ['Expert software engineering research agent specializing in TypeScript and JavaScript'],
      system: 'You are an expert software engineering researcher with deep knowledge of TypeScript, JavaScript, and modern web development frameworks.',
      clients: [],
      plugins: ['research'],
    },
...
async function runTsJsBenchmark() {
  ...
  console.log(`📊 Testing ${SWE_BENCH.queries.length} software engineering issues`);
  console.log('=' .repeat(70));
  ...
  for (let i = 0; i < selectedQueries.length; i++) {
    ...
    try {
      ...
      while (elapsed < timeout) {
        ...
        const currentProgress = Math.round((updated.sources.length / benchmarkQuery.expectedSources) * 100);
        if (currentProgress > lastProgress + 10) {
          console.log(`📈 Progress: ${currentProgress}% (${updated.sources.length}/${benchmarkQuery.expectedSources} sources)`);
        ...
        if (updated.status === 'completed') {
          ...
          console.log(`\n📊 Research Statistics:`);
          console.log(`  - Sources found: ${updated.sources.length}/${benchmarkQuery.expectedSources}`);
        ...
        if (elapsed % 30000 === 0 && elapsed > 0) {
          console.log(`⏳ Still processing... (${Math.round(elapsed / 1000)}s elapsed)`);
  ...
  if (successful.length > 0) {
    ...
    const avgRaceScore = successful.filter(r => r.raceScore).reduce((sum, r) => sum + (r.raceScore || 0), 0) / successful.length;
    const avgFactScore = successful.filter(r => r.factScore).reduce((sum, r) => sum + (r.factScore || 0), 0) / successful.length;
  ...
  const reportData = {
    ...
    averageMetrics: successful.length > 0 ? {
      ...
      raceScore: successful.filter(r => r.raceScore).reduce((sum, r) => sum + (r.raceScore || 0), 0) / successful.length,
      factScore: successful.filter(r => r.factScore).reduce((sum, r) => sum + (r.factScore || 0), 0) / successful.length,
    } : null,
    results: results,
  };
