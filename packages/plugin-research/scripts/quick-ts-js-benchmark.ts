...
const TS_JS_QUERIES = [
  {
    ...
    query: 'How do React useEffect cleanup functions work and what are common patterns for preventing memory leaks?',
    expectedSources: 5
  },
  {
    id: 'typescript_generics',
    query: 'What are TypeScript generic constraints and how do they improve type safety in modern applications?',
    expectedSources: 5
  },
  {
    id: 'nodejs_performance',
    query: 'What are the key Node.js performance optimization techniques for high-throughput applications?',
    expectedSources: 5
  }
];
    ...
    try {
      ...
      while (elapsed < timeout) {
        ...
        if (elapsed % 15000 === 0 && elapsed > 0) {
          console.log(`⏳ Processing... (${Math.round(elapsed / 1000)}s) - Status: ${updated.status}, Phase: ${updated.phase}`);
          if (updated.sources.length > 0) {
            console.log(`  📊 Progress: ${updated.sources.length} sources, ${updated.findings.length} findings`);
          }
        }
