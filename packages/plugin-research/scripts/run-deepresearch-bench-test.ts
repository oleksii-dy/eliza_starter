...
const createMockRuntime = (): IAgentRuntime => {
  ...
  return {
    ...
    useModel: async (
      ...
      console.log(
        `\n🔍 Mock useModel called (${callCount}):`,
...
5. Provide recommendations based on evidence`;
      ...
      if (prompt.includes('Extract key findings')) {
        return JSON.stringify([
          {
            content:
              "Japan's elderly population (65+) is projected to reach 35.3% by 2040, up from 28.7% in 2020, representing approximately 42 million people.",
            relevance: 0.95,
            confidence: 0.9,
            category: 'fact',
          },
          {
            content:
              'The elderly consumer market in Japan is estimated to be worth ¥100 trillion ($900 billion) annually, with healthcare and housing being the largest expenditure categories.',
            relevance: 0.9,
            confidence: 0.85,
            category: 'data',
          },
          {
            content:
              'Elderly Japanese consumers show increasing willingness to spend on quality-of-life improvements, including premium food products, comfortable clothing, and smart home technologies.',
            relevance: 0.88,
            confidence: 0.82,
            category: 'trend',
          },
        ...
        return JSON.stringify([
          {
            ...
            citationIndex: 1,
            supportingEvidence:
              'Government statistics show 28.7% of population over 65 in 2020',
          },
          {
            statement:
              'Elderly spending patterns differ significantly from younger demographics',
            citationIndex: 2,
            supportingEvidence:
              'Consumer surveys indicate 40% higher spending on healthcare',
          },
        ]);
      }
      ...
      if (
        prompt.includes('Generate.*report') ||
        prompt.includes('comprehensive.*report')
      ) {
...
Japan's aging population represents one of the world's largest and most sophisticated elderly consumer markets. Success requires deep understanding of evolving needs and preferences, combined with innovative approaches to product and service delivery.`;
      ...
      if (prompt.includes('Evaluate this research report')) {
        return JSON.stringify({
          score: 85,
          reasoning:
            'Comprehensive coverage with good depth and clear structure',
          rubricScores: {
