...
class MockE2BService extends Service {
  ...
  get capabilityDescription() {
    return 'Mock E2B service for testing';
  }
...
class MockFormsService extends Service {
  ...
  get capabilityDescription() {
    return 'Mock Forms service for testing';
  }
...
class MockCodeGenerationService extends Service {
  ...
  get capabilityDescription() {
    return 'Mock Code Generation service for testing';
  }
  ...
  describe('Tesla News Alert Discord Bot Scenario', () => {
      ...
      if (result.success) {
        ...
        expect(
          Object.keys(packageJson.dependencies).some(
            (dep) => dep.includes('discord') || dep.includes('rss') || dep.includes('feed')
          )
        ).toBe(true);
    ...
    it('should include proper RSS monitoring functionality', async () => {
      ...
      if (result.success && result.files) {
        ...
        const hasKeywordFiltering = result.files.some(
          (f) =>
            f.content.toLowerCase().includes('tesla') && f.content.toLowerCase().includes('filter')
        );
    ...
    it('should include comprehensive Discord integration', async () => {
      ...
      if (result.success && result.files) {
        ...
        const hasEmbeds = result.files.some(
          (f) => f.content.includes('embed') || f.content.includes('Embed')
        );
  ...
  describe('Global Weather App Scenario', () => {
    ...
    it('should include comprehensive weather query capabilities', async () => {
      ...
      if (result.success && result.files) {
        ...
        const hasLocationHandling = result.files.some(
          (f) =>
