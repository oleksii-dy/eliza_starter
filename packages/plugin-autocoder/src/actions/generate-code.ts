      ...
      const form = await formsService.createForm({
        ...
        onComplete: async (form) => {
          ...
          try {
            ...
            const monitorProgress = async () => {
              ...
              while (!generationComplete) {
                ...
                if (iteration % 3 === 0) {
                  ...
                  const messages = [
                    '🔧 Claude Code is analyzing your requirements...',
                    '📝 Writing code files and setting up the project structure...',
                    '📦 Installing dependencies and configuring the environment...',
                    '🧪 Running tests to ensure everything works correctly...',
                    '🏗️ Building the project and validating the output...',
                  ];
                  const messageIndex = Math.floor((iteration - 1) / 3) % messages.length;

                  if (lastStatus !== messages[messageIndex]) {
                    lastStatus = messages[messageIndex];
                    await sendProgress(messages[messageIndex]);
                  }
                }
              }
            };
            ...
            const monitoringPromise = monitorProgress();
            ...
            if (result.success) {
              ...
              if (result.files && result.files.length > 0) {
                successMessage += `📁 Created ${result.files.length} files\n`;
              ...
              if (result.executionResults) {
                successMessage += '\n✅ Quality checks:\n';
                successMessage += `  • Tests: ${result.executionResults.testsPass ? '✅ Passing' : '❌ Failed'}\n`;
                successMessage += `  • Build: ${result.executionResults.buildPass ? '✅ Success' : '❌ Failed'}\n`;
                successMessage += `  • Types: ${result.executionResults.typesPass ? '✅ Valid' : '❌ Errors'}\n`;
                successMessage += `  • Lint: ${result.executionResults.lintPass ? '✅ Clean' : '⚠️ Warnings'}\n`;
              }
              ...
              successMessage +=
                '\n\n🚀 Your project is ready to use! All tests are passing and the build is successful.';

              await sendProgress(successMessage, true);
            } else {
              let errorMessage = `❌ Code generation encountered issues:\n\n${result.errors?.join('\n')}\n\n`;
              ...
              errorMessage +=
                "I can try again with different parameters if you'd like. Just let me know what changes you want to make.";

              await sendProgress(errorMessage, true);
            }
          } catch (error) {
            generationComplete = true;
            const errorMessage = `❌ An unexpected error occurred during code generation:\n\n${(error as Error).message}\n\nThis might be a temporary issue. Would you like me to try again?`;
            await sendProgress(errorMessage, true);
          }
        },
      });
