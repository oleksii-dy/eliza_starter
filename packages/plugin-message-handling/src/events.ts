...
const handleServerSync = async ({
  ...
  try {
    // Process each entity and room combination
    for (const entity of entities) {
      ...
      for (const room of rooms) {
        ...
        await runtime.ensureConnection({
          entityId: entity.id,
          roomId: room.id,
          name: (entity.metadata?.name || entity.metadata?.username) as string | undefined,
          source,
          channelId: room.metadata?.channelId as string | undefined,
          serverId: world.id,
          type: room.metadata?.type as ChannelType | undefined,
          worldId: world.id,
