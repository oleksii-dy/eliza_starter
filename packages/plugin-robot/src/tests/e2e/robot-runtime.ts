  ...
  tests = [
    {
      ...
      fn: async (runtime: any) => {
        ...
        console.log(`  Joints: ${state.joints.length}, Emergency Stop: ${state.isEmergencyStopped}`);
      },
    },
    
    {
      ...
      fn: async (runtime: any) => {
        ...
        if (!responseText.includes('head') && !responseText.includes('mov') && !responseText.includes('robot')) {
    ...
    
    {
      ...
      fn: async (runtime: any) => {
        ...
        const hasRobotInfo = stateText.includes('Robot Status:') || 
        ...
        const statusResponse = messages.find(m => 
          m.userId === runtime.agentId && 
          m.id !== message.id
        );
    ...
    
    {
      ...
      fn: async (runtime: any) => {
        ...
        const commandAction = runtime.actions.find((a: any) => 
          a.name === 'ROBOT_COMMAND' || a.name === 'ROBOT_COMMAND_V2'
        );
    ...
    
    {
      ...
      fn: async (runtime: any) => {
        ...
        const emergencyResponse = messages.find(m => 
          m.userId === runtime.agentId && 
          m.id !== emergencyMessage.id
        );
    ...
    
    {
      ...
      fn: async (runtime: any) => {
        ...
        // Find teach action
        const teachAction = runtime.actions.find((a: any) => 
          a.name === 'ROBOT_TEACH'
        );
        ...
        const teachResponse = messages.find(m => 
          m.userId === runtime.agentId && 
          m.id !== teachMessage.id
        );
    ...
    
    {
      ...
      fn: async (runtime: any) => {
        ...
        if (imuData) {
          ...
          console.log(`  Accelerometer: x=${imuData.accelerometer.x.toFixed(2)}, y=${imuData.accelerometer.y.toFixed(2)}, z=${imuData.accelerometer.z.toFixed(2)}`);
          console.log(`  Gyroscope: x=${imuData.gyroscope.x.toFixed(2)}, y=${imuData.gyroscope.y.toFixed(2)}, z=${imuData.gyroscope.z.toFixed(2)}`);
          
          if (imuData.orientation) {
            console.log(`  Orientation: w=${imuData.orientation.w.toFixed(2)}, x=${imuData.orientation.x.toFixed(2)}, y=${imuData.orientation.y.toFixed(2)}, z=${imuData.orientation.z.toFixed(2)}`);
          }
        } else {
