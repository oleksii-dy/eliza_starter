...
export const robotStateProvider: Provider = {
  name: 'ROBOT_STATE',
  description: 'Provides current robot joint states, mode, and status information',
...
function formatJointSummary(joints: any[]): string {
  ...
  for (const joint of joints) {
    ...
    } else if (joint.name.includes('left') && (joint.name.includes('shoulder') || 
               joint.name.includes('elbow') || joint.name.includes('wrist') || 
               joint.name.includes('gripper'))) {
      groups.leftArm.push(joint);
    } else if (joint.name.includes('right') && (joint.name.includes('shoulder') || 
               joint.name.includes('elbow') || joint.name.includes('wrist') || 
               joint.name.includes('gripper'))) {
  ...
  if (groups.head.length > 0) {
    const headPositions = groups.head.map(j => 
      `${j.name}: ${(j.position * 180 / Math.PI).toFixed(1)}°`
    ).join(', ');
  ...
  if (groups.leftArm.length > 0) {
    const avgPos = groups.leftArm.reduce((sum, j) => sum + Math.abs(j.position), 0) / groups.leftArm.length;
    lines.push(`- Left Arm: ${groups.leftArm.length} joints, avg position ${(avgPos * 180 / Math.PI).toFixed(1)}°`);
  }
  
  if (groups.rightArm.length > 0) {
    const avgPos = groups.rightArm.reduce((sum, j) => sum + Math.abs(j.position), 0) / groups.rightArm.length;
    lines.push(`- Right Arm: ${groups.rightArm.length} joints, avg position ${(avgPos * 180 / Math.PI).toFixed(1)}°`);
  }
  
  // Legs
  if (groups.leftLeg.length > 0) {
    const avgPos = groups.leftLeg.reduce((sum, j) => sum + Math.abs(j.position), 0) / groups.leftLeg.length;
    lines.push(`- Left Leg: ${groups.leftLeg.length} joints, avg position ${(avgPos * 180 / Math.PI).toFixed(1)}°`);
  }
  
  if (groups.rightLeg.length > 0) {
    const avgPos = groups.rightLeg.reduce((sum, j) => sum + Math.abs(j.position), 0) / groups.rightLeg.length;
    lines.push(`- Right Leg: ${groups.rightLeg.length} joints, avg position ${(avgPos * 180 / Math.PI).toFixed(1)}°`);
  }
