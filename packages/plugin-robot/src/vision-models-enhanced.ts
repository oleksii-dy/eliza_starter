...
export class VisionModelsEnhanced {
  ...
  async detectObjects(imageData: Buffer, width: number, height: number, vlmDescription?: string): Promise<DetectedObject[]> {
  ...
  async detectPoses(imageData: Buffer, width: number, height: number, vlmDescription?: string): Promise<PersonInfo[]> {
  ...
  private extractObjectsFromDescription(description: string, width: number, height: number): DetectedObject[] {
    const objects: DetectedObject[] = [];
  ...
  private extractPeopleFromDescription(description: string, width: number, height: number): PersonInfo[] {
    const people: PersonInfo[] = [];
    const lowerDesc = description.toLowerCase();
    
    // Check if person is mentioned
    if (lowerDesc.includes('person') || lowerDesc.includes('individual') || lowerDesc.includes('someone')) {
