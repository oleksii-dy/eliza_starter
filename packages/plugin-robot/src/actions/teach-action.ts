...
export const teachAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<boolean> => {
    ...
    const hasTeachKeywords = [
      'teach', 'record', 'save', 'demonstration', 'learn',
      'pose', 'motion', 'movement', 'sequence', 'capture position'
    ...
    try {
      ...
      if (command.type === 'save_motion') {
        ...
        try {
          ...
          if (callback) {
            await callback({
              text: `❌ Error saving motion: ${errorMessage}\n` +
      ...
      if (callback) {
        await callback({
          text: `❓ I didn't understand that teaching command. Try:\n` +
                `- "Start teaching mode"\n` +
                `- "Record pose [name]"\n` +
                `- "Save motion [name]"\n` +
                `- "List saved motions"\n` +
...
function parseTeachCommand(text: string): {
  ...
  // Start/stop teaching
  if (lowerText.includes('start teach') || lowerText.includes('teaching mode') || 
      lowerText.includes('enter teach')) {
    return { type: 'start_teaching' };
  }
  
  if (lowerText.includes('stop teach') || lowerText.includes('exit teach') || 
      lowerText.includes('finish teach')) {
    return { type: 'stop_teaching' };
  }
  
  // Record pose
  if (lowerText.includes('record') && (lowerText.includes('pose') || lowerText.includes('position'))) {
  ...
  if (lowerText.includes('execute') || lowerText.includes('run') || lowerText.includes('play')) {
    if (lowerText.includes('motion')) {
      ...
      if (actionIndex >= 0 && actionIndex < words.length - 1) {
        ...
        const motionName = words.slice(actionIndex + 2).join(' ').replace(/["']/g, '');
