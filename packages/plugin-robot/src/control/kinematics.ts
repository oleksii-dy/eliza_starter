...
export class Kinematics {
  ...
  async inverseKinematics(
    ...
    const jointAngles = initialGuess || new Array(chain.joints.length).fill(0);
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      const error = Math.sqrt(
        positionError.x ** 2 +
          positionError.y ** 2 +
          positionError.z ** 2 +
          orientationError.x ** 2 +
          orientationError.y ** 2 +
          orientationError.z ** 2
      );
  ...
  private computeJacobian(chainName: string, jointAngles: number[]): number[][] {
    const _chain = this.chains.get(chainName)!;
    ...
    for (let i = 0; i < jointAngles.length; i++) {
      ...
      const dOrientation = this.quaternionError(
        perturbedTransform.orientation,
        currentTransform.orientation
      );
