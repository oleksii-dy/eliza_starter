...
export class RLService extends Service {
  ...
  override capabilityDescription = 'Provides reinforcement learning training and inference for robot control.';
  ...
  async runEpisode(usePolicy: boolean = true): Promise<{
    ...
    logger.info(`[RLService] Episode complete - Steps: ${steps}, Reward: ${totalReward.toFixed(2)}`);
  ...
  async startTraining(episodes: number = 1000): Promise<void> {
    ...
    try {
      for (let episode = 0; episode < episodes && this.isTraining; episode++) {
        ...
        // Log progress
        if (episode % 10 === 0) {
          logger.info(`[RLService] Episode ${episode}/${episodes} - ` +
  ...
  async evaluate(episodes: number = 10): Promise<{
    ...
    logger.info(`[RLService] Evaluation complete - ` +
