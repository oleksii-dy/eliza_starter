...
export const DEFAULT_AINEX_CONFIG: RobotConfig = {
  ...
  hardware: {
    ...
    baudRate: parseInt(process.env.ROBOT_BAUD_RATE || '115200', 10),
    updateRateHz: parseInt(process.env.ROBOT_UPDATE_RATE || '10', 10),
    imuUpdateRateHz: parseInt(process.env.IMU_UPDATE_RATE || '50', 10),
  },
  ...
  simulation: {
    ...
    updateRateHz: parseInt(process.env.SIMULATION_UPDATE_RATE || '50', 10),
...
// Get joint limits
export function getJointLimits(config: RobotConfig): {
...
export function validateJointPosition(
  config: RobotConfig,
  jointName: string,
  position: number
): boolean {
