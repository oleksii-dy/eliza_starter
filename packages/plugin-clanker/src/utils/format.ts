...
export function formatTokenInfo(info: any): string {
  ...
  if (info.marketCap !== undefined) {
    lines.push(`Market Cap: ${formatUsd(Number(info.marketCap))}`);
  }

  if (info.liquidity !== undefined) {
    lines.push(`Liquidity: ${formatUsd(Number(info.liquidity))}`);
  }
  ...
  if (info.volume24h !== undefined) {
    lines.push(`24h Volume: ${formatUsd(Number(info.volume24h))}`);
  }
