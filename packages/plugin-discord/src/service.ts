...
export class DiscordService extends Service implements IDiscordService {
  ...
  private setupEventListeners() {
    ...
    this.client.on('messageCreate', async (message) => {
      ...
      if (
        ...
        (message.author.bot &&
          typeof this.runtime.character.settings === 'object' &&
          this.runtime.character.settings !== null &&
          'discord' in this.runtime.character.settings &&
          typeof this.runtime.character.settings.discord === 'object' &&
   ...
   * @param limit - Maximum number of messages to fetch (default: 500)
   * @param hoursToFetch - How many hours back to fetch messages (default: 24)
   * @returns Array of Discord messages
   */
  async fetchChannelMessages(
    channelId: string,
    limit: number = 500,
    hoursToFetch: number = 24
  ): Promise<DiscordMessage[]> {
    if (!this.client?.isReady()) {
      logger.error('Discord client not ready for fetching messages');
      return [];
    }
    ...
    try {
      ...
      const startTime = Date.now() - hoursToFetch * 60 * 60 * 1000;

      // Discord allows fetching max 100 messages at a time
      while (messages.length < limit) {
        ...
        try {
          // Fetch messages - this returns a Collection
          const fetchedMessages = await textChannel.messages.fetch(fetchOptions);

          // Check if we got any messages
          if (
            !fetchedMessages ||
            (fetchedMessages instanceof Collection && fetchedMessages.size === 0)
          ) {
            break;
          }

          // Handle both single message and collection cases
          let messagesToProcess: DiscordMessage[] = [];
          if (fetchedMessages instanceof Collection) {
            // Filter messages by time and convert to array
            messagesToProcess = Array.from(fetchedMessages.values()).filter(
              (msg) => msg.createdTimestamp >= startTime
            );
          } else if (fetchedMessages && 'createdTimestamp' in fetchedMessages) {
            // Single message case
            const singleMessage = fetchedMessages as any;
            if (singleMessage.createdTimestamp >= startTime) {
              messagesToProcess = [singleMessage as DiscordMessage];
            }
          }
          ...
          messages.push(...messagesToProcess);

          // Update lastMessageId for pagination
          const oldestMessage = messagesToProcess[messagesToProcess.length - 1];
          if (oldestMessage && oldestMessage.createdTimestamp < startTime) {
            break;
          }

          lastMessageId = oldestMessage?.id;
        } catch (fetchError) {
          logger.error(`Error fetching batch of messages: ${fetchError}`);
          break;
        }
      }

      return messages.filter((msg) => msg.createdTimestamp >= startTime);
    } catch (error) {
