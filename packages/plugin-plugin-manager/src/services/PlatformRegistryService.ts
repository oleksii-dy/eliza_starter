...
export class PlatformRegistryService extends Service {
  ...
  constructor() {
    super();
    // Database path will be set after runtime is available
    this.databasePath = join(process.cwd(), 'data', 'platform-registry.json');
    this.database = {
      items: [],
      buildJobs: [],
      collections: [],
  ...
  static async start(runtime: IAgentRuntime): Promise<PlatformRegistryService> {
    ...
    // Set database path using runtime settings
    const customPath = runtime.getSetting('PLATFORM_REGISTRY_DB_PATH');
    if (customPath) {
      service.databasePath = customPath;
    }
