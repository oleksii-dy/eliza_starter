  ...
  async handler(
    ...
    try {
      ...
      if (isOriginalPluginError) {
        const originalPluginMessage =
          "Cannot unload this plugin because it's an original plugin that was loaded at startup. Only dynamically loaded plugins can be unloaded.";
        if (callback) {
          await callback({
