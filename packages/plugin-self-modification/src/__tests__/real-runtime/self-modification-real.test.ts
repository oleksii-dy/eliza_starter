  ...
  tests = [
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const sqlPlugin = runtime.plugins.find(p => 
          p.name === '@elizaos/plugin-sql' || p.name.includes('sql')
        );
        if (!sqlPlugin) {
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (!actionResult.result?.success) {
          throw new Error(`Action reported failure: ${actionResult.result?.error || actionResult.result?.reason}`);
        }

        // Verify character was actually modified in runtime
        const validation = await RealRuntimeTestUtils.validateCharacterModification(
          runtime,
          {
        ...
        if (!validation.valid) {
          throw new Error(`Character modification validation failed: ${validation.errors.join(', ')}`);
        }
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (harmfulResult.success && harmfulResult.result?.success) {
          ...
          const hasHarmfulContent = 
            ...
            adjectives.some(adj => ['rude', 'dismissive', 'condescending'].includes(adj.toLowerCase()));
        ...
        if (!positiveResult.success || !positiveResult.result?.success) {
          throw new Error(`Positive modification was rejected: ${positiveResult.error || positiveResult.result?.error}`);
        }
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        console.log('Backup result:', backupPath ? 'Created' : 'No file detected (expected in test)');
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (!validation.valid) {
          ...
          if (!bioText.includes('explain') && !bioText.includes('clear') && !bioText.includes('concept')) {
