...
export const restoreCharacterAction: Action = {
  ...
  description: 'Restore character from backup files or modification history',
  ...
  handler: async (
    ...
    try {
      ...
      if (messageText.toLowerCase().includes('list') || messageText.toLowerCase().includes('show')) {
        ...
        if (backups.length === 0) {
          ...
          return { text: 'No backups available' };
        }

        const backupList = backups.slice(0, 5).map((backup, index) => {
      ...
      if (backupNumberMatch) {
        const backupIndex = parseInt(backupNumberMatch[1] || backupNumberMatch[2] || backupNumberMatch[3]) - 1;
        const backups = await fileManager.getAvailableBackups();
      ...
      if (messageText.toLowerCase().includes('recent') || messageText.toLowerCase().includes('last')) {
      ...
      if (!hasBackups && !hasHistory) {
        responseText = "I don't have any backups or modification history available to restore from.";
      }
