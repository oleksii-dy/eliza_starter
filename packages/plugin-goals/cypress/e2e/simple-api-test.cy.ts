describe('Goals Plugin API Tests', () => {
  ...
  describe('Basic Server Tests', () => {
    ...
    it('should check if plugin routes are available', () => {
      // Try to access the goals endpoint - it might not exist in dev mode
      cy.request({
        method: 'GET',
        ...
        if (response.status === 200) {
          ...
          cy.log('Goals API endpoint not available in dev mode');
        }
      });
    });
  ...
  describe('Frontend Routes', () => {
    ...
    it('should check goals page availability', () => {
      cy.request({
        url: '/goals',
        failOnStatusCode: false,
      }).then((response) => {
        // Page might exist or not in dev mode
        cy.log(`Goals page status: ${response.status}`);
      });
    });
  });
});
