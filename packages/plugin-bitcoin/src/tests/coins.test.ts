...
describe("coinsAction", () => {
  ...
  it("should validate with bitcoin provider", async () => {
    expect(await coinsAction.validate(mockRuntime, mockMessage, mockState)).toBe(true);
  });

  it("should display confirmed UTXOs correctly", async () => {
    const result = await coinsAction.handler(mockRuntime, mockMessage, mockState, {}) as { text: string };
  ...
  it("should handle empty wallet state", async () => {
    ...
    const result = await coinsAction.handler(emptyRuntime, mockMessage, mockState, {}) as { text: string };
  ...
  it("should handle missing bitcoin provider", async () => {
    ...
    const result = await coinsAction.handler(noProviderRuntime, mockMessage, mockState, {}) as { text: string };
  ...
  it("should handle provider error gracefully", async () => {
    ...
    const result = await coinsAction.handler(errorRuntime, mockMessage, mockState, {}) as { text: string };
