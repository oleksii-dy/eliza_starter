import { IAgentRuntime, Memory, State, Content, HandlerCallback, ActionExample, ModelClass } from "@ai16z/eliza";
import { composeContext, generateObjectDeprecated } from "@ai16z/eliza";
...
function isSendBitcoinContent(content: any): content is SendBitcoinContent {
  ...
  return (
    ...
    (
      ...
      (typeof content.amountUSD === "string" || typeof content.amountUSD === "number") ||
      (typeof content.amountSats === "string" || typeof content.amountSats === "number")
    )
  );
}
...
export const sendBitcoinAction = {
  ...
  handler: async (
    ...
    try {
      ...
      const network = runtime.getSetting("BITCOIN_NETWORK") as "bitcoin" | "testnet" | "signet" | "mutinynet";
      const arkServerUrl = runtime.getSetting("BITCOIN_ARK_SERVER_URL");
      const arkServerPublicKey = runtime.getSetting("BITCOIN_ARK_SERVER_PUBLIC_KEY");

      if (!privateKey || !network) {
        throw new Error("Bitcoin configuration is not set");
      }

      const provider = new BitcoinTaprootProvider({ privateKey, network, arkServerUrl, arkServerPublicKey } as BitcoinConfig);

      // Convert amounts if needed
      let amount: bigint | undefined;
      if (content.amount) {
        // Convert BTC to satoshis
        amount = BigInt(Math.floor(Number(content.amount) * 100_000_000));
  ...
  validate: async (runtime: IAgentRuntime) => {
    return !!runtime.getSetting("BITCOIN_PRIVATE_KEY") && !!runtime.getSetting("BITCOIN_NETWORK");
  },
  examples: [
    [
      {
