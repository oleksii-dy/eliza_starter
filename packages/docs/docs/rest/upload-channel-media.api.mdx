...
Upload image, video, audio, document or other media files to a specific channel. Files are stored in channel-specific directories and accessible via unique URLs. This endpoint includes rate limiting (100 requests per 15 minutes per IP).
