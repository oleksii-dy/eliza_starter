...
describe('Plugin Configuration', () => {
  it('should have correct plugin metadata', () => {
    expect(plugin.name).toBe('mr-tee-starter-plugin');
    expect(plugin.description).toBe("Mr. TEE's starter plugin - using plugin-tee for attestation");
  ...
  it('should include the WALLET_SECRET_SALT in config', () => {
    expect(plugin.config).toHaveProperty('WALLET_SECRET_SALT');

    documentTestResult('Plugin config check', {
      hasWalletSecretSalt: plugin.config ? 'WALLET_SECRET_SALT' in plugin.config : false,
      configKeys: Object.keys(plugin.config || {}),
    });
  });

  it('should initialize properly', async () => {
    const originalEnv = process.env.WALLET_SECRET_SALT;

    try {
      process.env.WALLET_SECRET_SALT = 'test-value';
      ...
      try {
        await plugin.init?.({ WALLET_SECRET_SALT: 'test-value' }, runtime as any);
      ...
      documentTestResult(
        ...
        {
          success: !error,
          configValue: process.env.WALLET_SECRET_SALT,
        },
        error
      );
    } finally {
      process.env.WALLET_SECRET_SALT = originalEnv;
    }
  });
  ...
  it('should throw an error on invalid config', async () => {
    ...
    if (plugin.init) {
      ...
      try {
        await plugin.init({ WALLET_SECRET_SALT: '' }, runtime as any);
  ...
  it('should have a valid config', () => {
    expect(plugin.config).toBeDefined();
    if (plugin.config) {
      // Check if the config has expected WALLET_SECRET_SALT property
      expect(Object.keys(plugin.config)).toContain('WALLET_SECRET_SALT');
    }
  });
});
