...
export const managePositions: Action = {
  ...
  description:
    'Automatically manage Meteora positions by rebalancing when they drift from active bin',
  ...
  handler: async (
    ...
    await handleRepositioning(fetchedPositions, repositionThresholdBps, wallet);
  ...
  return await Promise.all(
    fetchedPositions.map(async (position) => {
      if (position.distanceFromActiveBinBps > repositionThresholdBps) {
        const dlmmPool = await DLMM.create(wallet.connection, new PublicKey(position.poolAddress));
        ...
        try {
          ...
          const userPosition = userPositions.find(
            (p) => p.publicKey.toString() === position.positionPubKey
          );
