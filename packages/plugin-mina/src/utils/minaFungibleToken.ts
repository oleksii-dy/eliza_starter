...
import { FungibleTokenAdmin, FungibleTokenAdminBase } from "./minaFungibleTokenAdmin"
...
export class FungibleToken extends TokenContract {
    ...
    public async getAdminContract(): Promise<FungibleTokenAdminBase> {
        const admin = await Provable.witnessAsync(PublicKey, async () => {
            const pk = await this.admin.fetch()
    ...
    async getCirculating(): Promise<UInt64> {
        const circulating = await this.getBalanceOf(this.address)
        return circulating
    }
