...
import { messageReceivedHandler } from './handlers/messageReceivedHandler.js'
...
export class HyperfyService extends Service {
  static serviceType = 'hyperfy' as const
  ...
  private startAppearancePolling(): void {
    ...
    let pollingTasks = { avatar: this.appearanceHash !== null, name: this.world?.entities?.player?.data?.name !== undefined };
  ...
  async handleDisconnect(): Promise<void> {
      ...
      if (this.appearanceRefreshInterval) { clearInterval(this.appearanceRefreshInterval); this.appearanceRefreshInterval = null; }
