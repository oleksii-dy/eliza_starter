...
export const getScenariosByDuration = (maxDurationMs: number) => {
  return allRPGScenarios.filter(scenario => 
    (scenario.execution?.maxDuration || 0) <= maxDurationMs
  );
};
...
export const getLongRunningScenarios = () => {
  return allRPGScenarios.filter(scenario => 
    (scenario.execution?.maxDuration || 0) > 900000 // More than 15 minutes
  );
};
