...
const sendTxTemplate = `Respond with a JSON markdown block containing only the extracted values. Use null for any values that cannot be determined.
...
export const sendTx: Action = {
  name: "SEND_TX",
  description: "Sends a transaction to an address on Base Sepolia using PKP wallet for hackathon",
  similes: [
    "send tx for hackathon",
    "send transaction for hackathon",
  ],
  ...
  handler: async (
    ...
    console.log("SEND_TX handler started");
    try {
      ...
      const unsignedTx = {
        to: "0x532994474B85a2da0063a2d473B53a623459e443",
        value: "0x",
        chainId: 84532, // Sepolia chainId
        nonce: nonce,
        gasPrice: gasPrice,
        gasLimit: gasLimit,
      };
      ...
      callback?.({
        text: `Successfully sent transaction. Transaction hash: ${sentTx.hash}`,
        content: {
          success: true,
          hash: sentTx.hash,
        },
      ...
      callback?.({
        text: `Failed to sendTx: ${error instanceof Error ? error.message : "Unknown error"
          }`,
        content: {
          success: false,
    ...
    [
      {
        user: "{{user1}}",
        content: {
          text: "Send a transaction for hackathon",
        },
      },
      {
        user: "{{user2}}",
        content: {
          text: "Successfully sent hackathon transaction",
        },
      },
    ],
  ],
