// src/actions/getNewsCryptoPanic.ts
import {
  elizaLogger,
  ModelClass
} from "@elizaos/core";
...
var newsCyptoPanicTemplate = `Respond with a JSON markdown block containing only the extracted values. Use null for any values that cannot be determined.
    Example response:
    \`\`\`json
    {
        currencies:null,
  ...
  handler: async (runtime, message, state, options, callback) => {
    try {
      ...
      if (!responseCryptoPanic.ok) {
        elizaLogger.error("API Response:", await responseCryptoPanic.text());
        throw new Error(
          `Embedding API Error: ${responseCryptoPanic.status} ${responseCryptoPanic.statusText}`
        );
      }
      const dataCryptoPanic = await responseCryptoPanic.json();
      const dataOriginUrl = dataCryptoPanic.results.map((item) => item.source.url);
      ...
      const dataResponse = dataCryptoPanic.results.map((item, index) => {
        return {
          title: item.title,
