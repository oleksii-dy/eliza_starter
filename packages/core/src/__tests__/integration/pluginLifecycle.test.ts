...
const testQueryAction: Action = {
  ...
  examples: [
    [
      ...
      {
        name: 'agent',
        content: { text: 'Executed test query successfully', actions: ['TEST_QUERY'] },
      },
    ],
  ],
...
describe('Plugin Lifecycle Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    dbAdapter = {
      ...
      createEntities: mock().mockImplementation((entities: Entity[]) => {
  ...
  describe('Plugin Loading with Environment Variables', () => {
    ...
    it('should fail to load plugin when required env vars are missing', async () => {
      ...
      await expect(runtimeWithoutEnv.registerPlugin(testPluginWithEnvVars)).rejects.toThrow(
        'TEST_DB_API_KEY is required for TestDatabaseService'
      );
  ...
  describe('Plugin Dependency and Error Handling', () => {
    it('should handle service initialization errors gracefully', async () => {
      ...
      await expect(runtime.registerPlugin(failingPlugin)).rejects.toThrow(
        'Service initialization failed due to external dependency'
      );
    ...
    it('should handle partial component failures in hot-swap operations', async () => {
      ...
      if (!pluginConfig) {
        ...
        expect(result.errors[0].message).toBe('Plugin test-plugin-env not found');
      ...
      expect(result.errors).toContain(
        'Component NON_EXISTENT_ACTION not found in plugin test-plugin-env'
      );
  ...
  describe('Real-world Plugin Usage Scenarios', () => {
    ...
    it('should handle complex action execution with service dependencies', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        console.log(
          `Iteration ${i}: Initial connections: ${initialConnections}, Current connections: ${dbService.getConnections().length}, Result connections: ${(result as any).values?.connectionCount}`
        );
        expect((result as any).text).toBe('Query completed');
    ...
    it('should demonstrate provider state changes based on service state', async () => {
      ...
      // Get initial state
      const initialState = await systemProvider.get(runtime, testMessage, {
        values: {},
        data: {},
        text: '',
      });
      ...
      const disabledState = await systemProvider.get(runtime, testMessage, {
        values: {},
        data: {},
        text: '',
      });
  ...
  describe('Environment Variable Edge Cases', () => {
    it('should handle runtime environment changes', async () => {
      ...
      // Attempt to register plugin - should fail
      await expect(runtimeMissingEnv.registerPlugin(testPluginWithEnvVars)).rejects.toThrow(
        'TEST_DB_API_KEY is required for TestDatabaseService'
      );
