      ...
      for (const serviceItem of plugin.services) {
        ...
        if ('component' in serviceItem && typeof serviceItem.enabled === 'boolean') {
          if (serviceItem.enabled) {
            const service = await serviceItem.component.start(this as any);
            this.services.set(serviceItem.component.serviceName, service);
            console.log(`Service ${serviceItem.component.serviceName} enabled and registered`);
          } else {
            console.log(`Service ${serviceItem.component.serviceName} disabled by default`);
          }
        } else {
          // Legacy format - always enabled for backwards compatibility
          const ServiceClass = serviceItem as typeof Service;
    ...
    if (plugin.actions) {
      for (const action of plugin.actions) {
        ...
        if (isEnabled) {
          ...
          console.log(`Action ${action.name} enabled and registered`);
        } else {
          console.log(`Action ${action.name} disabled by default`);
        }
      }
    }
    ...
    if (plugin.providers) {
      for (const provider of plugin.providers) {
        ...
        if (isEnabled) {
          ...
          console.log(`Provider ${provider.name} enabled and registered`);
        } else {
          console.log(`Provider ${provider.name} disabled by default`);
        }
      }
    }
    ...
    if (plugin.evaluators) {
      for (const evaluator of plugin.evaluators) {
        ...
        if (isEnabled) {
          ...
          console.log(`Evaluator ${evaluator.name} enabled and registered`);
        } else {
          console.log(`Evaluator ${evaluator.name} disabled by default`);
        }
      }
    }
