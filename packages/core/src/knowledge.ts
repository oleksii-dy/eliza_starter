...
async function get(
    ...
    if (!message?.content?.text) {
        nexLogger.warn("Invalid message for knowledge query:", {
            message,
    ...
    nexLogger.debug("Knowledge query:", {
    ...
    if (!processed || processed.trim().length === 0) {
        nexLogger.warn("Empty processed text for knowledge query");
        return [];
    }
...
export function preprocess(content: string): string {
    nexLogger.debug("Preprocessing text:", {
        input: content,
