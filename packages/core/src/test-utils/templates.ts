...
import { IAgentRuntime, Memory, Content, Character, Plugin } from '../index';
import { ElizaError, ErrorCategory, ValidationError } from '../errors/index';
import { validator, Schema } from '../types/validation';
...
export class ErrorTestTemplate extends TestTemplate {
  private testFunction: () => Promise<void>;
  private expectedError: ErrorCategory;

  constructor(config: TestConfig, testFunction: () => Promise<void>, expectedError: ErrorCategory) {
    super(config);
    this.testFunction = testFunction;
  ...
  async execute(): Promise<TemplateTestResult> {
    ...
    try {
      ...
      const passed = error instanceof ElizaError && error.category === this.expectedError;
...
export class TestAssertions {
   ...
   */
  static assertValid<T>(value: unknown, schema: Schema, message?: string): asserts value is T {
   ...
   */
  static assertValidMemory(memory: unknown): asserts memory is Memory {
    ...
    if (!mem.id || typeof mem.id !== 'string') {
      throw new ValidationError('Memory must have a valid id');
    }

    if (!mem.content || typeof mem.content !== 'object') {
      throw new ValidationError('Memory must have valid content');
    }
  }

  /**
   * Assert that a runtime has specific capabilities
   */
  static async assertRuntimeCapabilities(
    runtime: IAgentRuntime,
    capabilities: string[]
  ): Promise<void> {
    for (const capability of capabilities) {
      const service = runtime.getService(capability);
      if (!service) {
        throw new ValidationError(`Runtime missing required capability: ${capability}`);
      }
    }
  }
  ...
  static assertResponseTime(duration: number, maxMs: number): void {
    if (duration > maxMs) {
      throw new ValidationError(`Response time ${duration}ms exceeds limit ${maxMs}ms`);
    }
  }
   ...
   */
  static assertMemoryUsage(startUsage: number, maxIncreaseMB: number): void {
    const currentUsage = process.memoryUsage().heapUsed;
    const increaseMB = (currentUsage - startUsage) / (1024 * 1024);

    if (increaseMB > maxIncreaseMB) {
      throw new ValidationError(
        `Memory increase ${increaseMB.toFixed(2)}MB exceeds limit ${maxIncreaseMB}MB`
      );
    }
  }
}
