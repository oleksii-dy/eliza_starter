/**
 * Standardized testing infrastructure for ElizaOS
 * Replaces inconsistent mock-based testing with real runtime integration
 */

/// <reference types="@jest/globals" />

// Note: expect import removed - should be provided by test framework
declare global {
...
import { createUniqueUuid as _createUniqueUuid } from '../entities';
...
export class TestAssertions {
  static assertRealRuntime(runtime: IAgentRuntime) {
    if (typeof expect === 'undefined') {
      return;
    } // Skip if no test framework
    expect(runtime).toBeDefined();
    expect(runtime.agentId).toBeDefined();
    expect(runtime.character).toBeDefined();
    expect((runtime as any).adapter).toBeDefined();

    // Verify not a mock
    expect(runtime.constructor.name).not.toContain('Mock');
    expect((runtime as any).adapter.constructor.name).not.toContain('Mock');
  }

  static assertRealDatabase(adapter: IDatabaseAdapter) {
    if (typeof expect === 'undefined') {
      return;
    } // Skip if no test framework
    expect(adapter).toBeDefined();
    expect(typeof adapter.createMemory).toBe('function');
    expect(typeof adapter.getMemories).toBe('function');

    // Verify not a mock
    expect(adapter.constructor.name).not.toContain('Mock');
  }

  static async assertDatabaseConnectivity(adapter: IDatabaseAdapter) {
    if (typeof expect === 'undefined') {
      return;
    } // Skip if no test framework
    ...
    // Test capabilities
    const capabilities = (adapter as any).getCapabilities?.() || { isReady: true, tables: [] };
  ...
  static async assertPerformance<T>(
    ...
    if (duration > maxDuration) {
      throw new Error(
        `Performance assertion failed: ${description} took ${duration}ms (max: ${maxDuration}ms)`
      );
    }
