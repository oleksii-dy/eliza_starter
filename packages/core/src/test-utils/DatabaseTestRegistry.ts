...
export class DatabaseTestRegistry {
  ...
  private async createTestDatabase(
    ...
    try {
      ...
      adapter = await this.createAdapterByType(config.preferredAdapter, testId);
      adapterType = config.preferredAdapter === 'auto' ? 'detected' : config.preferredAdapter;
    } catch (_error) {
      if (config.allowMockFallback) {
        console.warn(`Falling back to mock database for test ${testId}:`, _error);
        adapter = await this.createMockAdapter(testId);
        adapterType = 'mock';
      } else {
        throw new Error(
  ...
  private async createAdapterByType(
    type: 'postgresql' | 'auto',
    testId: string
  ): Promise<IDatabaseAdapter> {
    // Use dynamic imports to avoid circular dependencies
    try {
      if (type === 'postgresql' || type === 'auto') {
      ...
      throw new Error(`Failed to create ${type} adapter: ${(_error as any).message}`);
    }
  }
  ...
  private async getAdapterCapabilities(
    ...
    try {
      ...
      let capabilities: any = { isReady, tables: [], hasVector: false };
      ...
      return {
        ...
        hasVector: capabilities.hasVector || false,
  ...
  async validateTestRequirements(
    ...
    for (const adapterType of requirements.requiredAdapters) {
      try {
        const testAdapter = await this.createAdapterByType(adapterType, 'validation-test');
        ...
        if (requirements.requiresVector && !capabilities.hasVector) {
          result._errors.push(`Required adapter ${adapterType} does not support vector operations`);
        }
