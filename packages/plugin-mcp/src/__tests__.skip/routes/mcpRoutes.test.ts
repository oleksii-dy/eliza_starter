...
function createMockResponse(): Response & { _json?: any; _status?: number; _type?: string; _send?: string } {
  const res: any = {
    _json: null,
    _status: 200,
    _type: null,
...
describe('MCP Routes', () => {
  describe('GET /mcp/servers', () => {
    const getServersRoute = mcpRoutes.find(r => r.path === '/mcp/servers' && (r as any).type === 'GET')!;
      ...
      const mockServers = [
        {
          ...
          resources: [
            { uri: 'resource1', name: 'Test Resource', description: 'Test', mimeType: 'text/plain' }
          ]
        }
      ];
  ...
  describe('POST /mcp/tools/:serverName/:toolName', () => {
    const callToolRoute = mcpRoutes.find(r => r.path === '/mcp/tools/:serverName/:toolName' && (r as any).type === 'POST')!;
    ...
    it('should call tool successfully', async () => {
      ...
      expect(mockService.callTool).toHaveBeenCalledWith('test-server', 'test-tool', { arg1: 'value1' });
      expect(res._json).toEqual({
        success: true,
        data: {
          result: mockResult,
  ...
  describe('POST /mcp/resources/:serverName', () => {
    const readResourceRoute = mcpRoutes.find(r => r.path === '/mcp/resources/:serverName' && (r as any).type === 'POST')!;
    ...
    it('should read resource successfully', async () => {
      ...
      const mockService = {
        readResource: vi.fn().mockResolvedValue(mockResult)
      };
  ...
  describe('POST /mcp/servers/:serverName/reconnect', () => {
    const reconnectRoute = mcpRoutes.find(r => r.path === '/mcp/servers/:serverName/reconnect' && r.type === 'POST')!;
    ...
    it('should reconnect server successfully', async () => {
      ...
      expect(mockService.reconnectServer).toHaveBeenCalledWith('test-server', { type: 'stdio', command: 'test' });
      expect(res._json).toEqual({
        success: true,
        data: {
