...
describe('PGLite Lifecycle Tests', () => {
  ...
  describe('Manager Lifecycle', () => {
    it('should create and initialize manager properly', async () => {
      ...
      // Manager should have connection immediately (created in constructor)
      const connection = manager.getConnection();
      expect(connection).toBeDefined();
      expect(connection).not.toBeNull();

      // Initialize is a no-op for backward compatibility
      await manager.initialize();

      // Should still have connection
      const connectionAfter = manager.getConnection();
      expect(connectionAfter).toBeDefined();
      expect(connectionAfter).toBe(connection); // Same instance
    });

    it('should handle multiple initialization calls gracefully', async () => {
      manager = new PGliteClientManager(testDataDir);

      // Initialize multiple times concurrently (should be no-op)
    ...
    it('should close connection properly', async () => {
      ...
      // Connection should throw after close
    ...
    it('should handle close on uninitialized manager', async () => {
      ...
      // Close without calling initialize (though manager is already initialized in constructor)
      await manager.close();
  ...
  describe('Adapter Lifecycle', () => {
    ...
    it('should handle multiple adapter instances on same manager', async () => {
      ...
      // When an adapter is closed, it marks itself as closed
      // So adapter1 should not be ready
      expect(await adapter1.isReady()).toBe(false);

      // The manager is also closed when adapter closes, so adapter2 is also not ready
      // This is the current behavior - adapters directly manage their managers
      expect(await adapter2.isReady()).toBe(false);

      // Note: Reference counting for shared managers could be implemented in the future
      // to keep the manager alive while other adapters are using it
    });
  ...
  describe('Error Recovery', () => {
    it('should recover from initialization errors', async () => {
      // Since PGLite is created in constructor, we need to test constructor errors
      // We can't easily mock the constructor, so let's test a different error scenario

      // Create a manager with an invalid path that might cause issues
      const invalidPath = '/root/invalid/path/that/should/not/exist';

      try {
        manager = new PGliteClientManager(invalidPath);
        // If it doesn't throw immediately, try to use it
        const connection = manager.getConnection();
        // Try a query to see if it fails
      ...
      // Test that we can create a manager with a valid path after the error
      manager = new PGliteClientManager(testDataDir);
      const connection = manager.getConnection();
      expect(connection).toBeDefined();
    ...
    it('should handle concurrent close operations', async () => {
      ...
      const closePromises = [manager.close(), manager.close(), manager.close()];
