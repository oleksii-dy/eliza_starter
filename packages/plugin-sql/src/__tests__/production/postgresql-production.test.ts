...
describe('PostgreSQL Production Validation', () => {
  ...
  describe('PostgreSQL-Specific Features', () => {
    it.skipIf(!isPostgreSQLAvailable)('should validate vector extension functionality', async () => {
      ...
      await Promise.all(
        memories.map(memory => runtime.createMemory(memory, 'messages'))
      );
      ...
      console.log(`[PostgreSQL VECTOR] Search found ${searchResults.length} results with vector size ${vectorSize}`);
    });

    it.skipIf(!isPostgreSQLAvailable)('should handle PostgreSQL-specific concurrent operations', async () => {
      ...
      console.log(`[PostgreSQL CONCURRENCY] Completed ${concurrentOps} operations in ${concurrencyTime}ms`);
      console.log(`[PostgreSQL CONCURRENCY] Success rate: ${successful}/${concurrentOps} (${(successful/concurrentOps*100).toFixed(1)}%)`);
    ...
    it.skipIf(!isPostgreSQLAvailable)('should handle large-scale PostgreSQL operations', async () => {
      ...
      console.log(`[PostgreSQL LARGE SCALE] Creating ${largeScale} entities with relationships...`);
      ...
      console.log(`[PostgreSQL LARGE SCALE] Created ${entityIds.length} entities in ${entityCreationTime}ms`);
      ...
      console.log(`[PostgreSQL LARGE SCALE] Created relationships in ${relationshipCreationTime}ms`);
      ...
      console.log(`[PostgreSQL LARGE SCALE] Queried ${queriedEntities.length} entities in ${queryTime}ms`);
    ...
    it.skipIf(!isPostgreSQLAvailable)('should achieve PostgreSQL performance targets', async () => {
      ...
      const memoryPromises = Array.from({ length: memoryCount }, (_, i) => 
        runtime.createMemory({
          ...
          embedding: Array(1536).fill(0).map(() => Math.random()), // Full OpenAI embedding size
          metadata: {
            type: 'message',
            benchmark: true,
            index: i,
      ...
      console.log(`[PostgreSQL BENCHMARK] Created ${memoryCount} memories in ${creationTime}ms (${memoriesPerSecond.toFixed(1)} memories/sec)`);
      ...
      console.log(`[PostgreSQL BENCHMARK] Queried ${allMemories.length} memories in ${queryTime}ms`);
      ...
      console.log(`[PostgreSQL BENCHMARK] Vector search found ${searchResults.length} results in ${searchTime}ms`);
