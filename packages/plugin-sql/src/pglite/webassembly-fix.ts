...
export async function initializePGLiteWithFix(options: PGLiteInitOptions = {}): Promise<PGlite> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.warn(`[PGLite WebAssembly Fix] Attempt ${attempt} failed:`, {
        error: errorMessage,
        isAbortError: errorMessage.includes('Aborted()'),
        isWebAssemblyError: errorMessage.includes('WebAssembly'),
      });
  ...
  // All retries failed
  const errorMessage =
    `PGLite WebAssembly initialization failed after ${maxRetries} attempts. ` +
...
export async function testPGLiteCompatibility(): Promise<{
  ...
  try {
    ...
    if ((result.rows[0] as any).count !== 1) {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.debug('[PGLite Compatibility] Vector extension test failed:', errorMessage);
    }
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.debug('[PGLite Compatibility] Fuzzystrmatch extension test failed:', errorMessage);
    }
