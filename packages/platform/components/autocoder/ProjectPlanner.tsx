...
export function ProjectPlanner({ project, onSpecificationUpdate, onStartBuild }: ProjectPlannerProps) {
  ...
  const addArrayItem = (field: keyof Pick<ProjectSpecification, 'dependencies' | 'features' | 'testCases' | 'securityRequirements'>, value: string) => {
    if (value.trim()) {
  ...
  const removeArrayItem = (field: keyof Pick<ProjectSpecification, 'dependencies' | 'features' | 'testCases' | 'securityRequirements'>, index: number) => {
    const currentArray = specification[field] as string[]
    updateSpecification({
  ...
  const validateSpecification = async () => {
    ...
    try {
      ...
      if (specification.testCases.length === 0) {
        updateSpecification({
          testCases: ['Unit tests for core functions', 'Integration tests', 'Error handling tests']
        })
      }
    } finally {
      ...
      <div className="space-y-2">
        ...
        {isEditing ? (
          ...
          <div
            ...
            className="p-3 bg-gray-50 border border-gray-200 rounded-md cursor-pointer hover:bg-gray-100 transition-colors"
          >
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Project Specification</h2>
            <p className="text-gray-600">
              Collaborate with the AI to plan your project in detail before building
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
            ...
            {renderEditableField('description', 'Description', specification.description, true)}
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Features & Requirements</h3>
            <ArrayField 
              label="Core Features" 
              placeholder="Add a feature..." 
              items={specification.features}
              onUpdate={(items) => setSpecification(prev => ({ ...prev, features: items }))}
            />
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <ArrayField 
              ...
              onUpdate={(items) => setSpecification(prev => ({ ...prev, dependencies: items }))}
            />
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <ArrayField 
              ...
              onUpdate={(items) => setSpecification(prev => ({ ...prev, testCases: items }))}
            />
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <ArrayField 
              ...
              onUpdate={(items) => setSpecification(prev => ({ ...prev, securityRequirements: items }))}
            />
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                disabled={!isSpecificationReady() || project.status === 'building'}
              ...
              >
                {project.status === 'building' ? '⚡ Building...' : '🚀 Start Build'}
              </button>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {[
                ...
                { label: 'Description', completed: !!specification.description.trim() },
                { label: 'Features', completed: specification.features.length > 0 },
                { label: 'Test Cases', completed: specification.testCases.length > 0 }
              ].map((item) => (
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-md">
              <p className="text-sm text-blue-800">
                💡 <strong>Tip:</strong> The more detailed your specification, the better the AI can build exactly what you need.
              </p>
            </div>
          </div>
