...
export function PluginPreview({ project, build }: PluginPreviewProps) {
  ...
  const [previewMode, setPreviewMode] = useState<'code' | 'tests' | 'docs' | 'live'>('code')
  ...
  useEffect(() => {
    if (build?.files && !selectedFile) {
      ...
      const mainFiles = ['index.ts', 'index.js', 'src/index.ts', 'src/index.js']
  ...
  const runLiveTest = async () => {
    ...
    try {
      const response = await fetch(`/api/autocoder/projects/${project.id}/test`, {
        method: 'POST',
  ...
  const downloadPlugin = async () => {
    ...
    try {
      const response = await fetch(`/api/autocoder/projects/${project.id}/download`)
      if (response.ok) {
      ...
      <div className="h-full flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Your plugin is being built. Come back here when it's complete to preview and test it.
          </p>
        </div>
      </div>
    )
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={metric.label} className="flex justify-between items-center">
                ...
                <span className={`text-xs font-medium ${getQualityColor(metric.value)}`}>
      ...
      <div className="flex-1 flex flex-col">
          ...
          <div className="flex items-center justify-between px-6 py-4">
            <div className="flex space-x-6">
              {[
                ...
                <button
                  key={tab.key}
                  onClick={() => setPreviewMode(tab.key as 'code' | 'tests' | 'docs' | 'live')}
        ...
        <div className="flex-1 overflow-hidden">
            ...
            <div className="h-full p-6 overflow-y-auto">
              <div className="max-w-4xl mx-auto">
                ...
                <div className="space-y-4">
                  {(liveTestResults.length > 0 ? liveTestResults : build.tests).map((test, index) => (
                    <div
                      key={index}
            ...
            <div className="h-full p-6 overflow-y-auto">
              <div className="max-w-4xl mx-auto prose prose-gray">
                ...
                {build.files['README.md'] ? (
                  <div dangerouslySetInnerHTML={{ __html: build.files['README.md'] }} />
                ) : (
          ...
          {previewMode === 'live' && (
            <div className="h-full p-6 overflow-y-auto">
              <div className="max-w-4xl mx-auto">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Live Plugin Testing</h3>
                
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                  <div className="flex items-center">
                    <span className="text-yellow-600 mr-2">⚠️</span>
                    <p className="text-sm text-yellow-800">
                      Live testing runs your plugin in a secure sandbox environment. 
                      This feature will be available once E2B integration is complete.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="bg-white border border-gray-200 rounded-lg p-6">
                    <h4 className="font-medium text-gray-900 mb-4">Interactive Console</h4>
                    <div className="bg-gray-900 text-green-400 p-4 rounded font-mono text-sm">
                      <div>$ eliza-plugin-test {project.name}</div>
                      <div className="mt-2 text-gray-400">{/* Interactive testing coming soon... */}</div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-6">
                    ...
                    <div className="text-sm text-gray-600">
                      Test your plugin's API endpoints and integrations in real-time.
                    </div>
                  </div>
                </div>
              </div>
