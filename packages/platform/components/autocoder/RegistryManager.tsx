...
export function RegistryManager({ project, userId }: RegistryManagerProps) {
  const [registryEntry, setRegistryEntry] = useState<PluginRegistry | null>(null)
  ...
  const pushToGitHub = async (repoFullName: string) => {
    try {
      await fetch(`/api/github/repos/${encodeURIComponent(repoFullName)}/push`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          projectId: project.id,
      ...
      <div className="h-full flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Complete your plugin build to publish it to the registry and share with others.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="max-w-4xl mx-auto p-6 space-y-8">
        {/* Header */}
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Plugin Registry</h2>
          <p className="text-gray-600">
            Publish your plugin to share it with the community or keep it private
          </p>
        </div>
        ...
        {registryEntry ? (
          <div className="bg-green-50 border border-green-200 rounded-lg p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-green-700 mt-1">{registryEntry.description}</p>
              ...
              <div className="space-y-2">
                {registryEntry.githubUrl && (
                  <a
                    ...
                    className="block px-4 py-2 bg-gray-900 text-white rounded-md text-sm hover:bg-gray-800 transition-colors"
                  >
                    View on GitHub
                  </a>
                )}
                {registryEntry.npmUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block px-4 py-2 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 transition-colors"
                  >
                    View on NPM
                  </a>
                )}
                <button
                  ...
                  className="block w-full px-4 py-2 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 transition-colors"
                >
                  Unpublish
                </button>
              </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Publish Plugin</h3>
                ...
                <div className="space-y-3">
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setPublishForm(prev => ({ ...prev, createGitHubRepo: e.target.checked }))}
                      className="mr-3"
                    />
                    <span className="text-sm text-gray-700">
                  ...
                  <label className="flex items-center">
                    <input
                      ...
                      onChange={(e) => setPublishForm(prev => ({ ...prev, npmPublish: e.target.checked }))}
                      className="mr-3"
                    />
                    <span className="text-sm text-gray-700">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          <h4 className="font-medium text-blue-900 mb-3">Publishing Guidelines</h4>
          <ul className="text-sm text-blue-800 space-y-1">
            ...
            <li>• Consider making your plugin open source for community contributions</li>
          </ul>
        </div>
      </div>
    </div>
