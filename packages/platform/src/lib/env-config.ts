...
export function getEnvironmentConfig(): EnvironmentConfig {
  ...
  if (isTauri) {
    ...
    apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.platform.elizaos.com';
...
export function getServerEnvironmentConfig() {
  if (typeof window !== 'undefined') {
    throw new Error('getServerEnvironmentConfig() should only be called on the server side');
  }
...
export function getBuildConfig() {
  ...
  return {
    ...
    // Image optimization
    images: process.env.BUILD_MODE === 'export' ? { unoptimized: true } : undefined,
