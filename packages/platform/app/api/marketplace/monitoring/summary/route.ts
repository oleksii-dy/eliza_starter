...
import { hostedContainers, assetUsageRecords } from '@/lib/database/marketplace-schema';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const healthSummary = {
      ...
      healthy: userContainers.filter((c: any) => c.healthStatus === 'healthy').length,
      unhealthy: userContainers.filter((c: any) => c.healthStatus === 'unhealthy').length,
      unknown: userContainers.filter((c: any) => c.healthStatus === 'unknown').length,
    };
    ...
    const runningContainers = userContainers.filter((c: any) => c.status === 'running');
    const currentHourlyCost = runningContainers.reduce(
      (sum: number, container: any) => sum + parseFloat(container.billedCostPerHour), 0
    );

    // Get recent container activity
    const recentActivity = userContainers.slice(0, 10).map((container: any) => ({
      id: container.id,
...
function calculateAverageUptime(containers: any[]): number {
    ...
    const runningContainers = containers.filter(c => c.status === 'running' && c.startedAt);
...
function generateRecommendations(containers: any[], usageStats: any): string[] {
    const recommendations: string[] = [];

    // Cost optimization recommendations
    if (usageStats.totalCost > 50) {
      recommendations.push('Consider optimizing container resource allocation to reduce costs');
    }

    // Health recommendations
    const unhealthyContainers = containers.filter(c => c.healthStatus === 'unhealthy');
    if (unhealthyContainers.length > 0) {
      recommendations.push(`${unhealthyContainers.length} container(s) are unhealthy and may need attention`);
    }

    // Resource recommendations
    const runningContainers = containers.filter(c => c.status === 'running');
    if (runningContainers.length > 10) {
      recommendations.push('You have many running containers. Consider consolidating or stopping unused ones.');
    }

    // Billing recommendations
    if (usageStats.totalHours > 100) {
      recommendations.push('High usage detected. Consider optimizing container usage patterns.');
    }
