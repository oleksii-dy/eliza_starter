...
export async function GET(request: NextRequest) {
  try {
    ...
    const searchParams: any = {
      ...
      rating: url.searchParams.get('minRating') ? parseFloat(url.searchParams.get('minRating')!) : undefined,
      priceRange,
    };

    // Validate parameters
    if (searchParams.assetType && !['mcp', 'agent', 'workflow', 'plugin'].includes(searchParams.assetType)) {
    ...
    if (searchParams.sortBy && !['name', 'rating', 'installs', 'created', 'updated', 'price'].includes(searchParams.sortBy)) {
      return NextResponse.json(
    ...
    if (searchParams.rating && (searchParams.rating < 0 || searchParams.rating > 5)) {
      return NextResponse.json(
    ...
    const totalCount = assets.length < searchParams.limit ? 
