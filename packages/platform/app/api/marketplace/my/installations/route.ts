...
export async function GET(request: NextRequest) {
  try {
    ...
    const conditions = [
      eq(assetInstallations.userId, session.user.id)
    ];
    ...
    .leftJoin(marketplaceAssets, eq(assetInstallations.assetId, marketplaceAssets.id))
    ...
    const formattedInstallations = installations.map(({ installation, asset, creator, user }: any) => ({
      id: installation.id,
