...
const PROVIDER_MAP: Record<SocialProvider, { connection?: string; domain?: string; hint?: string }> = {
...
function getRedirectUri(request: NextRequest): string {
  ...
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || process.env.NEXTAUTH_URL || 'http://localhost:3333';
...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      errorUrl.searchParams.set('message', `${provider} login is temporarily unavailable`);
