...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitKey = RateLimitRepository.createKey(clientIP, 'device-authorize');
    const rateLimit = await rateLimitRepository.checkRateLimit(rateLimitKey, 30, 60);
    ...
    const userValidation = await userRepository.validateUserForDeviceAuth(user.id);
    
    if (!userValidation.isValid) {
      console.error(`[DEVICE AUTHORIZE] User validation failed: ${userValidation.error} for user: ${user.id}`);
      return NextResponse.json({
        success: false,
    ...
    if (dbUser.email !== user.email) {
      console.error(`[DEVICE AUTHORIZE] Email mismatch - DB: ${dbUser.email}, Provided: ${user.email}`);
      return NextResponse.json({
        success: false,
    ...
    const result = await deviceFlowService.authorizeDevice(
      user_code,
      user.id,
      {
        id: user.id,
    ...
    console.log(`[DEVICE AUTHORIZE] Device authorized successfully for user: ${user.email}, code: ${user_code}`);
