...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitKey = RateLimitRepository.createKey(clientIP, 'device-token');
    const rateLimit = await rateLimitRepository.checkRateLimit(rateLimitKey, 60, 60);
    ...
    console.log(`[TOKEN SUCCESS] Device authorization completed for client: ${client.clientName}`);
