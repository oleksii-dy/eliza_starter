...
export async function POST(
  ...
  try {
    ...
    const buildResult = currentProject.build_result ? JSON.parse(currentProject.build_result) : null
    ...
    try {
      ...
      await githubService.pushFiles({
        ...
        repo,
        files: filesToPush,
        commitMessage: commitMessage || `Update ${currentProject.name} v${buildResult.packageJson?.version || '1.0.0'}`,
        branch
      })
