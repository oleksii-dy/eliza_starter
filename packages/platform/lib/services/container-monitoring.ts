...
import { hostedContainers, assetUsageRecords } from '../database/marketplace-schema';
...
export class ContainerMonitoringService {
  ...
  async performHealthChecks(): Promise<ContainerHealthStatus[]> {
    try {
      ...
      const runningContainers = await db.select()
      ...
      const healthCheckPromises = runningContainers.map(async (container: any) => {
      ...
      const unhealthyContainers = healthStatuses.filter(h => h.status === 'unhealthy');
  ...
  async checkContainerHealth(container: any): Promise<ContainerHealthStatus> {
    ...
    try {
      ...
      if (deployment.status === 'running') {
        status = 'healthy';
      } else if (deployment.status === 'failed' || deployment.status === 'stopped') {
        status = 'unhealthy';
      } else {
        status = 'unknown';
      }
      ...
      try {
        ...
        const metrics = await this.getContainerMetrics(container.e2bDeploymentId);
  ...
  private async updateContainerHealth(
    ...
    try {
      ...
      console.error(`Failed to update health status for container ${containerId}:`, error);
    }
  }
    ...
    for (const container of unhealthyContainers) {
      try {
        ...
        if (unhealthyDuration > maxUnhealthyTime) {
          console.log(`Container ${container.containerId} unhealthy for too long, stopping...`);
        ...
        console.error(`Failed to handle unhealthy container ${container.containerId}:`, error);
      }
    }
  }
  ...
  async processBillingCycle(): Promise<BillingCycle[]> {
    try {
      ...
      const containersToProcess = await db.select()
        .from(hostedContainers)
        .where(and(
      ...
      for (const container of containersToProcess) {
        try {
          ...
          console.error(`Failed to process billing for container ${container.id}:`, error);
        }
      }
   ...
   */
  private async processContainerBilling(container: any): Promise<BillingCycle | null> {
    try {
      // Calculate usage since last billing
      const now = new Date();
      const lastBilling = container.lastBillingAt || container.startedAt;
      const hoursUsed = (now.getTime() - new Date(lastBilling).getTime()) / (1000 * 60 * 60);
      ...
      if (balance < totalCost) {
        console.log(`Insufficient balance for container ${container.id}, stopping...`);
        await this.stopContainerForInsufficientFunds(container);
        return null;
      }

      // Process billing
      const billingResult = await MarketplaceBillingService.recordContainerUsage(
        container.id,
        hoursUsed
      );
  ...
  private async stopUnhealthyContainer(container: any): Promise<void> {
    try {
      ...
      console.error(`Failed to stop unhealthy container ${container.id}:`, error);
    }
  }
   ...
   */
  private async stopContainerForInsufficientFunds(container: any): Promise<void> {
    try {
      await this.stopUnhealthyContainer(container);
      
      // Could add notification to user about insufficient funds
      console.log(`Container ${container.id} stopped due to insufficient credits`);
    } catch (error) {
      console.error(`Failed to stop container for insufficient funds ${container.id}:`, error);
    }
  }
  ...
  async getMonitoringSummary(): Promise<{
    ...
    try {
      ...
      const summary = {
        ...
        healthyContainers: containers.filter((c: any) => c.healthStatus === 'healthy').length,
        unhealthyContainers: containers.filter((c: any) => c.healthStatus === 'unhealthy').length,
        unknownContainers: containers.filter((c: any) => c.healthStatus === 'unknown').length,
        totalHourlyCost: containers.reduce((sum: number, c: any) => sum + parseFloat(c.billedCostPerHour), 0)
      };
