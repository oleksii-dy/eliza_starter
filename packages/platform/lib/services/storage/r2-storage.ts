...
export class R2StorageService implements IStorageService {
  ...
  constructor(
    ...
    this.endpoint = options.endpoint || `https://${accountId}.r2.cloudflarestorage.com`;
    ...
    try {
      ...
      throw new StorageConnectionError('Failed to initialize R2 storage client', error as Error);
    }
  }
  ...
  async exists(key: string): Promise<boolean> {
    try {
      ...
      throw new StorageError(`Failed to check existence of ${key}`, error as Error);
    }
  }
  ...
  async getMetadata(key: string): Promise<FileMetadata | null> {
    try {
      ...
      throw new StorageError(`Failed to get metadata for ${key}`, error as Error);
    }
  }
  ...
  async setMetadata(key: string, metadata: Record<string, string>): Promise<void> {
    try {
      ...
      throw new StorageError('Setting metadata on existing objects is not supported by R2/S3');
    } catch (error) {
  ...
  async list(prefix?: string, options?: ListOptions): Promise<FileInfo[]> {
    try {
      ...
      throw new StorageError(`Failed to list objects with prefix ${prefix}`, error as Error);
    }
  }
  ...
  async getSignedUrl(key: string, operation: 'read' | 'write', expiresIn: number = 3600): Promise<string> {
    try {
      let command;
      ...
      throw new StorageError(`Failed to generate signed URL for ${key}`, error as Error);
    }
  }
  ...
  async cleanup(): Promise<void> {
    ...
    console.log('[R2Storage] Cleanup not needed - R2 handles lifecycle automatically');
  }
}