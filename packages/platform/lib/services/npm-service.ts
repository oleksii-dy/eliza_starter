...
export class NPMService {
  ...
  async publishPackage(
    ...
    try {
      ...
      const enhancedPackageJson = {
        ...
        files: request.packageJson.files || [
          'dist/**/*',
          'README.md',
          'LICENSE',
        ],
        ...
        homepage:
          request.packageJson.homepage ||
          `https://github.com/elizaos/${request.name}#readme`,
      ...
      if (enhancedPackageJson.scripts?.test) {
        try {
          ...
          console.warn(
            'Tests failed but continuing with publish:',
      ...
      const publishResult = await execAsync(publishCommand, {
        cwd: tempPackageDir,
      });
      ...
      throw new Error(
        `NPM publish failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
  ...
  async checkPackageExists(packageName: string): Promise<boolean> {
    try {
      await execAsync(
        `npm view ${packageName} name --registry=${this.registry}`,
      );
      return true;
    } catch (error) {
      return false;
  ...
  async validatePackageName(
    ...
    if (name.startsWith('.') || name.startsWith('_') || name.startsWith('-')) {
      errors.push(
        'Package name cannot start with a dot, underscore, or hyphen',
      );
    }
  ...
  async unpublishPackage(packageName: string, version?: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Successfully unpublished ${packageName}${version ? `@${version}` : ''}`,
      );
    } catch (error) {
      console.error('Failed to unpublish package:', error);
      throw new Error(
        `Failed to unpublish package: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
...
export function createNPMService(
  userToken?: string,
  registry?: string,
): NPMService {
