...
import { pgTable, text, integer, decimal, timestamp, boolean, jsonb, uuid, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const marketplaceAssets = pgTable('marketplace_assets', {
  ...
  visibility: text('visibility').$type<VisibilityType>().notNull().default('private'),
  
  // Pricing and Monetization
  pricingModel: text('pricing_model').$type<PricingModel>().notNull().default('free'),
  basePrice: decimal('base_price', { precision: 10, scale: 2 }).default('0.00'), // One-time price
  usagePrice: decimal('usage_price', { precision: 10, scale: 6 }).default('0.000000'), // Per-usage price
  subscriptionPrice: decimal('subscription_price', { precision: 10, scale: 2 }).default('0.00'), // Monthly subscription
...
export const assetVersions = pgTable('asset_versions', {
  id: uuid('id').primaryKey().defaultRandom(),
  assetId: uuid('asset_id').references(() => marketplaceAssets.id, { onDelete: 'cascade' }).notNull(),
  organizationId: uuid('organization_id').references(() => organizations.id, { onDelete: 'cascade' }).notNull(),
...
// Container Hosting and Billing
export const hostedContainers = pgTable('hosted_containers', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id').references(() => organizations.id, { onDelete: 'cascade' }).notNull(),
  assetId: uuid('asset_id').references(() => marketplaceAssets.id, { onDelete: 'cascade' }).notNull(),
  versionId: uuid('version_id').references(() => assetVersions.id, { onDelete: 'cascade' }).notNull(),
  ...
  // Billing
  baseCostPerHour: decimal('base_cost_per_hour', { precision: 10, scale: 6 }).notNull(),
  markupPercentage: decimal('markup_percentage', { precision: 5, scale: 2 }).notNull().default('20.00'),
  billedCostPerHour: decimal('billed_cost_per_hour', { precision: 10, scale: 6 }).notNull(),
  ...
  deploymentIndex: uniqueIndex('hosted_containers_deployment_idx').on(table.e2bDeploymentId),
...
export const assetUsageRecords = pgTable('asset_usage_records', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id').references(() => organizations.id, { onDelete: 'cascade' }).notNull(),
  assetId: uuid('asset_id').references(() => marketplaceAssets.id, { onDelete: 'cascade' }).notNull(),
  userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
  containerId: uuid('container_id').references(() => hostedContainers.id, { onDelete: 'set null' }),
  ...
  creatorRevenue: decimal('creator_revenue', { precision: 10, scale: 2 }).notNull(), // 50% to creator
  platformRevenue: decimal('platform_revenue', { precision: 10, scale: 2 }).notNull(), // 50% to platform
  ...
  assetUserIndex: index('asset_usage_asset_user_idx').on(table.assetId, table.userId),
  userDateIndex: index('asset_usage_user_date_idx').on(table.userId, table.createdAt),
  assetDateIndex: index('asset_usage_asset_date_idx').on(table.assetId, table.createdAt),
}));
...
export const creatorPayouts = pgTable('creator_payouts', {
  ...
  creatorShare: decimal('creator_share', { precision: 12, scale: 2 }).notNull(), // 50%
  ...
  creatorPeriodIndex: index('creator_payouts_creator_period_idx').on(table.creatorId, table.periodStart),
...
export const assetReviews = pgTable('asset_reviews', {
  ...
  userAssetIndex: uniqueIndex('asset_reviews_user_asset_idx').on(table.userId, table.assetId),
}));
...
export const userFavorites = pgTable('user_favorites', {
  ...
  userAssetIndex: uniqueIndex('user_favorites_user_asset_idx').on(table.userId, table.assetId),
  assetIndex: index('user_favorites_asset_idx').on(table.assetId),
}));

// Asset Installations and Subscriptions
export const assetInstallations = pgTable('asset_installations', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id').references(() => organizations.id, { onDelete: 'cascade' }).notNull(),
  assetId: uuid('asset_id').references(() => marketplaceAssets.id, { onDelete: 'cascade' }).notNull(),
  userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
  ...
  userAssetIndex: index('asset_installations_user_asset_idx').on(table.userId, table.assetId),
