...
import { pgTable, text, integer, decimal, timestamp, boolean, jsonb, uuid, index, uniqueIndex, pgPolicy, pgRole, date } from 'drizzle-orm/pg-core';
...
export const organizations = pgTable('organizations', {
  ...
  // Credits system
  creditBalance: decimal('credit_balance', { precision: 10, scale: 6 }).notNull().default('0'),
  creditThreshold: decimal('credit_threshold', { precision: 10, scale: 6 }).notNull().default('10'), // Auto top-up threshold
  autoTopUpEnabled: boolean('auto_top_up_enabled').notNull().default(false),
  autoTopUpAmount: decimal('auto_top_up_amount', { precision: 10, scale: 6 }).notNull().default('50'),
  ...
  workosOrgIdx: index('organizations_workos_org_idx').on(table.workosOrganizationId),
...
export const anonymousSessions = pgTable('anonymous_sessions', {
  ...
  sessionIdIdx: index('anonymous_sessions_session_id_idx').on(table.sessionId),
  expiresAtIdx: index('anonymous_sessions_expires_at_idx').on(table.expiresAt),
  lastActivityIdx: index('anonymous_sessions_last_activity_idx').on(table.lastActivity),
}));
...
export const agents = pgTable('agents', {
  ...
  createdByUserIdx: index('agents_created_by_user_idx').on(table.createdByUserId),
  orgSlugUnique: uniqueIndex('agents_org_slug_unique').on(table.organizationId, table.slug),
  deploymentStatusIdx: index('agents_deployment_status_idx').on(table.deploymentStatus),
...
export const plugins = pgTable('plugins', {
  ...
  securityReviewStatus: text('security_review_status').notNull().default('pending'), // pending, approved, rejected
...
export const organizationPlugins = pgTable('organization_plugins', {
  id: uuid('id').primaryKey().defaultRandom(),
  organizationId: uuid('organization_id').references(() => organizations.id, { onDelete: 'cascade' }).notNull(),
  ...
  organizationIdIdx: index('organization_plugins_organization_id_idx').on(table.organizationId),
  pluginIdIdx: index('organization_plugins_plugin_id_idx').on(table.pluginId),
  orgPluginUnique: uniqueIndex('organization_plugins_org_plugin_unique').on(table.organizationId, table.pluginId),
}));
...
export const uploads = pgTable('uploads', {
  ...
  organizationIdIdx: index('uploads_organization_id_idx').on(table.organizationId),
  uploadedByUserIdx: index('uploads_uploaded_by_user_idx').on(table.uploadedByUserId),
...
export const creditTransactions = pgTable('credit_transactions', {
  ...
  // Balance tracking
  balanceAfter: decimal('balance_after', { precision: 10, scale: 6 }).notNull(),

  metadata: jsonb('metadata').$type<Record<string, any>>().notNull().default({}),

  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => ({
  organizationIdIdx: index('credit_transactions_organization_id_idx').on(table.organizationId),
...
export const usageRecords = pgTable('usage_records', {
  ...
  orgProviderDateIdx: index('usage_records_org_provider_date_idx').on(table.organizationId, table.provider, table.createdAt),
}));
...
export const inferenceLogs = pgTable('inference_logs', {
  ...
  conversationId: uuid('conversation_id_ref').references(() => conversations.id, { onDelete: 'cascade' }),
  ...
  markupPercentage: decimal('markup_percentage', { precision: 5, scale: 2 }).notNull().default('20.00'), // Our markup %
  markupAmount: decimal('markup_amount', { precision: 10, scale: 6 }).notNull().default('0'), // Calculated markup
  ...
  organizationIdIdx: index('inference_logs_organization_id_idx').on(table.organizationId),
  ...
  orgProviderModelDateIdx: index('inference_logs_org_provider_model_date_idx').on(table.organizationId, table.provider, table.model, table.createdAt),
  orgDateStatusIdx: index('inference_logs_org_date_status_idx').on(table.organizationId, table.createdAt, table.status),
  providerModelIdx: index('inference_logs_provider_model_idx').on(table.provider, table.model),
}));
...
export const platformConfig = pgTable('platform_config', {
  ...
  organizationIdIdx: index('platform_config_organization_id_idx').on(table.organizationId),
  categoryIdx: index('platform_config_category_idx').on(table.category),
  // Unique constraint for global or per-org config
  orgKeyUnique: uniqueIndex('platform_config_org_key_unique').on(table.organizationId, table.configKey),
}));
...
export const webhooks = pgTable('webhooks', {
  ...
  createdByUserIdx: index('webhooks_created_by_user_idx').on(table.createdByUserId),
...
export const conversations = pgTable('conversations', {
  ...
  organizationIdIdx: index('conversations_organization_id_idx').on(table.organizationId),
...
export const memories = pgTable('memories', {
  ...
  organizationIdIdx: index('memories_organization_id_idx').on(table.organizationId),
  agentIdIdx: index('memories_agent_id_idx').on(table.agentId),
  userIdIdx: index('memories_user_id_idx').on(table.userId),
  conversationIdIdx: index('memories_conversation_id_idx').on(table.conversationId),
...
export const entities = pgTable('entities', {
  ...
  organizationIdIdx: index('entities_organization_id_idx').on(table.organizationId),
  ...
  lastInteractionAtIdx: index('entities_last_interaction_at_idx').on(table.lastInteractionAt),
  // Unique constraint on agent + name to prevent duplicates
  agentNameUnique: uniqueIndex('entities_agent_name_unique').on(table.agentId, table.name),
}));
...
export const agentTasks = pgTable('agent_tasks', {
  ...
  organizationIdIdx: index('agent_tasks_organization_id_idx').on(table.organizationId),
  ...
  nextExecutionAtIdx: index('agent_tasks_next_execution_at_idx').on(table.nextExecutionAt),
...
export const deviceCodes = pgTable('device_codes', {
  ...
  isAuthorizedIdx: index('device_codes_is_authorized_idx').on(table.isAuthorized),
}));
...
export const oauthClients = pgTable('oauth_clients', {
  ...
  grantTypes: jsonb('grant_types').$type<string[]>().notNull().default(['authorization_code', 'device_code']),
  ...
  organizationIdIdx: index('oauth_clients_organization_id_idx').on(table.organizationId),
...
export const cryptoPayments = pgTable('crypto_payments', {
  ...
  slippageTolerance: decimal('slippage_tolerance', { precision: 3, scale: 1 }),
  metadata: jsonb('metadata').$type<Record<string, any>>().notNull().default({})
}, (table) => ({
  organizationIdIdx: index('crypto_payments_organization_id_idx').on(table.organizationId),
  ...
  transactionHashIdx: index('crypto_payments_transaction_hash_idx').on(table.transactionHash),
...
export const walletConnections = pgTable('wallet_connections', {
  ...
  organizationIdIdx: index('wallet_connections_organization_id_idx').on(table.organizationId),
  walletAddressIdx: index('wallet_connections_wallet_address_idx').on(table.walletAddress),
  ...
  userWalletUnique: uniqueIndex('wallet_connections_user_wallet_unique').on(table.userId, table.walletAddress, table.chainId)
}));
...
export const generations = pgTable('generations', {
  ...
  organizationIdIdx: index('generations_organization_id_idx').on(table.organizationId),
  ...
  idempotencyKeyIdx: index('generations_idempotency_key_idx').on(table.idempotencyKey),
  ...
  orgTypeProviderIdx: index('generations_org_type_provider_idx').on(table.organizationId, table.type, table.provider),
  statusQueuedIdx: index('generations_status_queued_idx').on(table.status, table.queuedAt)
}));
...
export const batchGenerations = pgTable('batch_generations', {
  ...
  organizationIdIdx: index('batch_generations_organization_id_idx').on(table.organizationId),
...
export const providerMetrics = pgTable('provider_metrics', {
  ...
  qualityScore: decimal('quality_score', { precision: 3, scale: 2 }).default('0'), // 0-10 scale
  userSatisfactionScore: decimal('user_satisfaction_score', { precision: 3, scale: 2 }).default('0'),
  
  // Error analysis
  errorTypes: jsonb('error_types').$type<Record<string, number>>().default({}),
  ...
  providerDateHourUnique: uniqueIndex('provider_metrics_provider_date_hour_unique').on(table.provider, table.type, table.date, table.hour),
  providerDateUnique: uniqueIndex('provider_metrics_provider_date_unique').on(table.provider, table.type, table.date)
}));
...
export const generationQueue = pgTable('generation_queue', {
  ...
  generationIdIdx: index('generation_queue_generation_id_idx').on(table.generationId),
  ...
  scheduledForIdx: index('generation_queue_scheduled_for_idx').on(table.scheduledFor),
  workerIdIdx: index('generation_queue_worker_id_idx').on(table.workerId),
  // Composite index for queue processing
  queueStatusPriorityIdx: index('generation_queue_queue_status_priority_idx').on(table.queueName, table.status, table.priority, table.scheduledFor)
}));
