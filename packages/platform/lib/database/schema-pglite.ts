...
export const organizations = pgTable('organizations', {
  ...
  creditThreshold: decimal('credit_threshold', { precision: 10, scale: 2 }).notNull().default('10'),
  autoTopUpEnabled: boolean('auto_top_up_enabled').notNull().default(false),
  autoTopUpAmount: decimal('auto_top_up_amount', { precision: 10, scale: 2 }).notNull().default('50'),
...
export const users = pgTable('users', {
  ...
  orgUserIdx: index('users_org_user_idx').on(table.organizationId, table.email),
}));
...
export const anonymousSessions = pgTable('anonymous_sessions', {
  ...
  workflowProgress: json('workflow_progress').$type<{
    currentStep: string;
...
export const agents = pgTable('agents', {
  ...
  characterConfig: json('character_config').$type<Record<string, any>>().notNull().default({}),
  plugins: json('plugins').$type<string[]>().notNull().default([]),
  runtimeConfig: json('runtime_config').$type<Record<string, any>>().notNull().default({}),
  ...
  orgSlugIdx: index('agents_org_slug_idx').on(table.organizationId, table.slug),
...
export const characterConversations = pgTable('character_conversations', {
  id: uuid('id').primaryKey().defaultRandom(),
  characterId: uuid('character_id').notNull().references(() => agents.id, { onDelete: 'cascade' }),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  organizationId: uuid('organization_id').notNull().references(() => organizations.id, { onDelete: 'cascade' }),
  title: text('title'),
  ...
  characterUserIdx: index('character_conversations_character_user_idx').on(table.characterId, table.userId),
  createdAtIdx: index('character_conversations_created_at_idx').on(table.createdAt),
}));
...
export const plugins = pgTable('plugins', {
  ...
  configSchema: json('config_schema').$type<Record<string, any>>().notNull().default({}),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
});
...
export const organizationPlugins = pgTable('organization_plugins', {
  ...
  orgPluginIdx: index('organization_plugins_org_plugin_idx').on(table.organizationId, table.pluginId),
}));
...
export const usageRecords = pgTable('usage_records', {
  ...
  orgResourceIdx: index('usage_records_org_resource_idx').on(table.organizationId, table.resource),
...
export const oauthClients = pgTable('oauth_clients', {
  ...
  grantTypes: json('grant_types').$type<string[]>().notNull().default(['authorization_code']),
  scopes: json('scopes').$type<string[]>().notNull().default(['read']),
  redirectUris: json('redirect_uris').$type<string[]>().notNull().default([]),
  allowedOrigins: json('allowed_origins').$type<string[]>().notNull().default([]),
  ...
  orgClientIdx: index('oauth_clients_org_client_idx').on(table.organizationId),
}));
...
export const autocoderBuilds = pgTable('autocoder_builds', {
  id: uuid('id').primaryKey().defaultRandom(),
  projectId: uuid('project_id').notNull().references(() => autocoderProjects.id, { onDelete: 'cascade' }),
  userId: text('user_id').notNull(),
...
export const autocoderBuildLogs = pgTable('autocoder_build_logs', {
  id: uuid('id').primaryKey().defaultRandom(),
  projectId: uuid('project_id').notNull().references(() => autocoderProjects.id, { onDelete: 'cascade' }),
  buildId: uuid('build_id').notNull().references(() => autocoderBuilds.id, { onDelete: 'cascade' }),
...
export type NewCharacterConversation = typeof characterConversations.$inferInsert;
