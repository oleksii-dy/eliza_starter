...
function getIframeJWTSecret(): Uint8Array {
  ...
  if (!secret) {
    throw new Error('IFRAME_JWT_SECRET environment variable is required for production');
  }
  if (secret.length < 32) {
 ...
 */
export async function verifyIframeToken(token: string): Promise<IframeTokenData | null> {
  try {
