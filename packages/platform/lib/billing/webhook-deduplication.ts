...
export class WebhookDeduplicationService {
   ...
   */
  static validateWebhookEvent(event: WebhookEvent): { valid: boolean; reason?: string } {
    ...
    if (eventAge < -60) { // Allow 1 minute clock skew
      return {
        valid: false,
  ...
  static async processWebhookSafely<T>(
    ...
    try {
      ...
      // Mark as failed
      await this.markEventProcessed(event, { success: false, error: errorMessage });
  ...
  static async getProcessingStats(organizationId?: string): Promise<{
    ...
    try {
      ...
      const successfullyProcessed = events.filter((e: any) => e.metadata?.processingSuccess === true).length;
