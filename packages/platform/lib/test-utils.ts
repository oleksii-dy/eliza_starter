...
import { organizations, users, agents, conversations, messages, memories } from './database/schema';
...
// Export the database instance for direct testing
export const testDb = db;

/**
 * Create a test organization with a test user for integration testing
 */
export async function createTestOrganization(name: string = 'Test'): Promise<TestOrganization> {
  try {
 ...
 */
export async function createTestUser(organizationId: string, email?: string, name?: string): Promise<TestUser> {
  try {
    const userResult = await db.insert(users).values({
      organizationId,
 ...
 */
export async function clearTestDatabase(organizationId?: string): Promise<void> {
  try {
    if (organizationId) {
      ...
      // 1. Delete memories
      await db.delete(memories).where(eq(memories.organizationId, organizationId));
      
      // 2. Delete messages
      await db.delete(messages).where(eq(messages.organizationId, organizationId));
      
      // 3. Delete conversations
      await db.delete(conversations).where(eq(conversations.organizationId, organizationId));
      ...
      console.log(`Successfully cleaned up test data for organization: ${organizationId}`);
    } else {
 ...
 */
export async function createTestAgent(organizationId: string, userId: string, characterConfig: any): Promise<string> {
  try {
 ...
 */
export async function verifyDatabaseIsolation(organizationId: string): Promise<boolean> {
  try {
    ...
    const [orgMemories, orgMessages, orgConversations] = await Promise.all([
      ...
      db.select().from(conversations).where(eq(conversations.organizationId, organizationId))
    ]);

    const hasNoData = orgMemories.length === 0 && 
                      orgMessages.length === 0 && 
                      orgConversations.length === 0;

    if (!hasNoData) {
      console.warn(`Database isolation check failed for org ${organizationId}:`, {
 ...
 */
export async function waitForDatabase(maxAttempts: number = 10, delayMs: number = 1000): Promise<boolean> {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.log(`Database connection attempt ${attempt}/${maxAttempts} failed:`, error);
