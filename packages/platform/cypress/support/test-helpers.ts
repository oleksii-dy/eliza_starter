...
export function waitForElement(selector: string, options: { timeout?: number; visible?: boolean } = {}) {
...
export function safeVisit(url: string, options: { retries?: number; timeout?: number } = {}) {
  ...
  function attemptVisit(attemptsLeft: number) {
    cy.visit(url, { 
      ...
      cy.get('body').then(($body) => {
        if ($body.find('[data-testid="error-boundary"]').length > 0 && attemptsLeft > 0) {
          cy.log(`Page error detected, retrying... (${attemptsLeft} attempts left)`);
          cy.wait(2000);
...
export function fillFormField(selector: string, value: string, options: { clear?: boolean } = {}) {
...
export function safeClick(selector: string, options: { timeout?: number; retries?: number } = {}) {
...
export function recoverFromError() {
  ...
  cy.get('body').then(($body) => {
    if ($body.text().includes('error') || $body.text().includes('Something went wrong')) {
      devLogin();
    }
  });
}
