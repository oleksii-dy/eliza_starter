...
const MockStatsCard = ({ title, value, subtitle, icon }: { 
  title: string
  value: string | number
  subtitle?: string
  icon?: React.ReactNode
}) => (
  <div data-cy={`stats-${title.toLowerCase().replace(' ', '-')}`} className="bg-white p-6 rounded-lg shadow">
    <div className="flex items-center">
      {icon && <div className="mr-3">{icon}</div>}
      <div>
        <h3 className="text-sm font-medium text-gray-500">{title}</h3>
        <p className="text-2xl font-bold text-gray-900" data-cy={`${title.toLowerCase()}-count`}>
          {value}
        </p>
        {subtitle && (
          <p className="text-sm text-gray-600" data-cy={`${title.toLowerCase()}-subtitle`}>
            {subtitle}
          </p>
        )}
      </div>
...
const MockActivityItem = ({
  ...
  <div data-cy={`activity-item-${type}`} className="flex items-start space-x-3 py-3">
...
describe('Dashboard Components Unit Tests', () => {
  ...
  it('QuickAction Component - Renders correctly', () => {
    ...
    // Test styling
    cy.get('[data-cy="quick-action-create-agent"]').should('have.class', 'bg-white')
    cy.get('[data-cy="quick-action-create-agent"]').should('have.class', 'rounded-lg')
    cy.get('[data-cy="quick-action-create-agent"]').should('have.class', 'border')
  })
  ...
  it('QuickAction Component - Hover states', () => {
    ...
    cy.get('[data-cy="quick-action-plugin-creator"]').should('have.class', 'hover:shadow-md')
    cy.get('[data-cy="quick-action-plugin-creator"]').should('have.class', 'transition-shadow')
  })
  ...
  it('ActivityItem Component - Different activity types', () => {
    ...
    activities.forEach((activity, index) => {
      ...
      cy.get('[data-cy="activity-title"]').should('contain.text', activity.title)
      cy.get('[data-cy="activity-description"]').should('contain.text', activity.description)
      cy.get('[data-cy="activity-timestamp"]').should('contain.text', activity.timestamp)
    })
  })
  ...
  it('Components Integration - Dashboard Layout', () => {
    ...
    const DashboardLayout = () => (
      <div data-cy="dashboard-layout" className="min-h-screen bg-gray-50 p-6">
        ...
        {/* Stats Grid */}
        <div data-cy="stats-section" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <MockStatsCard title="Agents" value={5} subtitle="3 active" />
          <MockStatsCard title="Team" value={3} subtitle="1 pending invites" />
          <MockStatsCard title="Credits" value="$1,000.0" subtitle="premium plan" />
        ...
        <div data-cy="recent-activity">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/dashboard/audit" className="text-blue-600 hover:text-blue-800">View all</a>
          </div>
          <div data-cy="activity-list" className="bg-white rounded-lg shadow divide-y divide-gray-200">
            <MockActivityItem 
              type="agent_created"
              title="Agent Created"
              description="New agent 'Customer Support Bot' created"
  ...
  it('Component Accessibility - ARIA and semantic HTML', () => {
    ...
    const AccessibleStatsCard = ({ title, value, subtitle }: { 
      title: string
      value: string | number
      subtitle?: string
    }) => (
      ...
      >
        <h3 id={`${title}-heading`} className="text-sm font-medium text-gray-500">
          {title}
        </h3>
        <p className="text-2xl font-bold text-gray-900" aria-label={`${title} count: ${value}`}>
          {value}
        </p>
        {subtitle && (
          <p className="text-sm text-gray-600" aria-label={`Additional info: ${subtitle}`}>
            {subtitle}
          </p>
        )}
      </div>
  ...
  it('Component Performance - Large data sets', () => {
    ...
    cy.get('[data-cy="large-activity-list"]').should('be.visible').then(() => {
  ...
  it('Component Error Boundaries', () => {
    ...
    const ErrorProneComponent = ({ shouldError }: { shouldError: boolean }) => {
      ...
      return <div data-cy="error-prone-component">Component rendered successfully</div>
    }
    
    const ComponentWithErrorBoundary = ({ shouldError }: { shouldError: boolean }) => {
      try {
        return <ErrorProneComponent shouldError={shouldError} />
      } catch (error) {
        return (
          <div data-cy="error-fallback" className="p-4 bg-red-50 border border-red-200 rounded">
            <h3 className="text-red-800 font-medium">Something went wrong</h3>
            <p className="text-red-600 text-sm">Unable to render component</p>
          </div>
        )
      }
    }
