...
describe('Dashboard Comprehensive Test', () => {
  ...
  it('Dashboard Page - Complete Flow Test', () => {
    ...
    cy.get('[data-cy="pending-invites"]').should('contain.text', '1 pending invites');
    ...
    cy.get('[data-cy="subscription-tier"]').should('contain.text', 'premium plan');
    ...
    cy.get('[data-cy="quick-action-create-agent"]').should('be.visible').within(() => {
    ...
    cy.get('[data-cy="activity-item-agent_created"]').should('be.visible').within(() => {
      cy.get('[data-cy="activity-title"]').should('contain.text', 'Agent Created');
      cy.get('[data-cy="activity-description"]').should('contain.text', 'Customer Support Bot');
      cy.get('[data-cy="activity-timestamp"]').should('contain.text', '2 hours ago');
    });
    
    cy.get('[data-cy="activity-item-user_invited"]').should('be.visible').within(() => {
      cy.get('[data-cy="activity-title"]').should('contain.text', 'User Invited');
      cy.get('[data-cy="activity-description"]').should('contain.text', 'john.doe@company.com');
      cy.get('[data-cy="activity-timestamp"]').should('contain.text', '4 hours ago');
    });
    
    cy.get('[data-cy="activity-item-credit_added"]').should('be.visible').within(() => {
      cy.get('[data-cy="activity-title"]').should('contain.text', 'Credits Added');
      cy.get('[data-cy="activity-description"]').should('contain.text', '$100 credits');
      cy.get('[data-cy="activity-timestamp"]').should('contain.text', '1 day ago');
    });
    ...
    cy.get('[data-cy="quick-action-create-agent"]').should('have.attr', 'href', '/dashboard/agents/create');
    cy.get('[data-cy="quick-action-plugin-creator"]').should('have.attr', 'href', '/dashboard/plugin-creator');
    cy.get('[data-cy="quick-action-manage-api-keys"]').should('have.attr', 'href', '/dashboard/api-keys');
    cy.get('[data-cy="quick-action-view-usage"]').should('have.attr', 'href', '/dashboard/billing');
    ...
    cy.contains('Add Credits').should('be.visible').and('have.attr', 'href', '/settings/billing');
  ...
  it('Dashboard Navigation Integration Test', () => {
    ...
    cy.get('[data-cy="quick-action-create-agent"]').should('have.attr', 'href').and('include', '/agents/create');
    cy.get('[data-cy="quick-action-plugin-creator"]').should('have.attr', 'href').and('include', '/plugin-creator');
    cy.get('[data-cy="quick-action-manage-api-keys"]').should('have.attr', 'href').and('include', '/api-keys');
    cy.get('[data-cy="quick-action-view-usage"]').should('have.attr', 'href').and('include', '/billing');
  ...
  it('Dashboard Data Integrity Test', () => {
    ...
    testScenarios.forEach((scenario, index) => {
      ...
      cy.get('[data-cy="agent-count"]').should('contain.text', scenario.stats.agentCount.toString());
      cy.get('[data-cy="user-count"]').should('contain.text', scenario.stats.userCount.toString());
      cy.get('[data-cy="credit-balance"]').should('contain.text', `$${scenario.stats.creditBalance}`);
      cy.get('[data-cy="subscription-tier"]').should('contain.text', `${scenario.stats.subscriptionTier} plan`);
      cy.get('[data-cy="api-requests"]').should('contain.text', scenario.stats.apiRequests24h.toLocaleString());
      cy.get('[data-cy="api-cost"]').should('contain.text', `$${scenario.stats.totalCost24h} cost`);
      cy.get('[data-cy="active-agents"]').should('contain.text', `${scenario.stats.activeAgents} active`);
      cy.get('[data-cy="pending-invites"]').should('contain.text', `${scenario.stats.pendingInvites} pending invites`);
    });
