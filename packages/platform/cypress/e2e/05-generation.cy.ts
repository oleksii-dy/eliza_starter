describe('AI Generation - Complete Test Suite', () => {
  ...
  describe('Text Generation', () => {
    ...
    it('should handle text generation request', () => {
      // Fill out form
      cy.get('textarea[name="prompt"]').type('Write a short story about a robot');
      ...
      cy.contains('Once upon a time, there was a robot named...').should('be.visible');
    ...
    it('should handle advanced text generation options', () => {
      ...
      cy.get('textarea[name="systemPrompt"]').type('You are a creative writer.');
  ...
  describe('Image Generation', () => {
    ...
    it('should display image generation interface', () => {
      ...
      // Check generate button
      cy.get('button[type="submit"]').contains('Generate Images').should('be.visible');
    });

    it('should handle image generation request', () => {
      // Fill out form
      cy.get('textarea[name="prompt"]').type('A beautiful sunset over mountains');
      ...
      cy.intercept('GET', '**/generation/img-gen-1', {
        ...
        body: {
          ...
          result: {
            images: [
              { url: 'https://example.com/image1.jpg', filename: 'sunset1.jpg' },
              { url: 'https://example.com/image2.jpg', filename: 'sunset2.jpg' }
            ]
          },
          creditsUsed: 195
        }
  ...
  describe('Video Generation', () => {
    ...
    it('should display video generation interface', () => {
      ...
      // Check generate button
      cy.get('button[type="submit"]').contains('Generate Video').should('be.visible');
    });

    it('should handle video generation request', () => {
      // Fill out form
      cy.get('textarea[name="prompt"]').type('A cat playing with a ball of yarn');
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle network disconnection during generation', () => {
      ...
      cy.intercept('POST', '**/generation', { forceNetworkError: true }).as('networkError');
  ...
  describe('Responsive Design and Accessibility', () => {
    ...
    it('should have proper ARIA labels', () => {
      ...
      cy.get('textarea[name="prompt"]').should('have.attr', 'aria-label')
        .or('have.attr', 'placeholder');
      
      // Generate button should have proper labeling
      cy.get('button[type="submit"]').should('contain.text', 'Generate')
