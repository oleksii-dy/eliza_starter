...
describe('Standalone Server Integration', () => {
  ...
  before(() => {
    ...
    cy.request({
      ...
      if (response.status !== 200) {
        throw new Error('Standalone server is not running. Start it with: npm run start:standalone');
      }
    });
  });
  ...
  describe('Server Health and API', () => {
    it('should respond to health check', () => {
      cy.request(`${STANDALONE_BASE_URL}/api/runtime/health`).then((response) => {
    ...
    it('should serve client assets correctly', () => {
      ...
      cy.window().then((win) => {
        ...
        const assetErrors = errors.filter(call => 
          call.args.some(arg => 
            typeof arg === 'string' && (arg.includes('404') || arg.includes('Failed to load'))
          )
        );
