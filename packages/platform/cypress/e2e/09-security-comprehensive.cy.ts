...
describe('Security Comprehensive Testing', () => {
  ...
  it('XSS Prevention - Input Sanitization Test', () => {
    ...
    maliciousInputs.forEach((maliciousInput, index) => {
      cy.log(`Testing malicious input ${index + 1}: ${maliciousInput.substring(0, 30)}...`);
      
      // Test email input
      cy.get('[data-cy="email-input"] input').clear().type(maliciousInput);
      cy.get('[data-cy="email-input"] input').should('have.value', maliciousInput);
      
      // Test password input
      cy.get('[data-cy="password-input"] input').clear().type(maliciousInput);
      cy.get('[data-cy="password-input"] input').should('have.value', maliciousInput);
  ...
  it('CSRF Protection Test', () => {
    ...
    cy.intercept('POST', '**/api/auth/login', (req) => {
      ...
      const hasCSRFHeader = req.headers['x-csrf-token'] || 
                           req.headers['x-xsrf-token'] ||
                           req.headers['csrf-token'];
      
      // Check for CSRF token in body
      const hasCSRFBody = req.body && (
        req.body.csrfToken || 
        req.body._token ||
        req.body.authenticity_token
      );
  ...
  it('Input Validation and Sanitization Test', () => {
    ...
    commandInjectionPayloads.forEach((payload) => {
      ...
      cy.get('[data-cy="organizationName-input"] input').should('have.value', payload);
    });
  ...
  it('Authentication Security Test', () => {
    ...
    weakPasswords.forEach((weakPassword) => {
      ...
      cy.get('[data-cy="organizationName-input"] input').clear().type('Test Org');
    ...
    for (let i = 0; i < rapidAttempts; i++) {
      ...
      cy.get('[data-cy="email-input"] input').clear().type(`attempt${i}@example.com`);
  ...
  it('API Security Test', () => {
    ...
    cy.intercept('GET', '**/api/auth/identity', (req) => {
      ...
      if (!authHeader.startsWith('Bearer ')) {
        ...
        req.reply({
          statusCode: 401,
          body: { error: 'Unauthorized', message: 'Invalid authorization header format' }
        });
        return;
      }
  ...
  it('Data Protection and Privacy Test', () => {
    ...
    cy.window().then((win) => {
      ...
      win.console.log = (...args) => {
        args.forEach(arg => {
          if (typeof arg === 'string' && arg.includes('sensitive-password-123')) {
            passwordLogged = true;
      ...
      req.reply({
        ...
        body: {
          ...
          data: {
            ...
            organization: { id: 'privacy-test-org', name: body.organizationName }
          }
        }
      });
    }).as('privacySignup');
