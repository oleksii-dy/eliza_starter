...
import { organizations, users, creditTransactions, apiKeys } from '@/lib/database/schema';
...
describe('Credit Middleware Integration Tests', () => {
  ...
  describe('Credit Deduction for AI Usage', () => {
    test('should deduct credits for GPT-3.5 usage', async () => {
      ...
      expect(result.remainingBalance).toBeCloseTo(100.0 - result.deductedAmount, 2);
    });
    ...
    test('should prevent usage when insufficient credits', async () => {
      ...
      try {
        await database.delete(creditTransactions).where(eq(creditTransactions.organizationId, insufficientOrgId));
        await database.delete(users).where(eq(users.id, insufficientUserId));
        await database.delete(organizations).where(eq(organizations.id, insufficientOrgId));
      } catch (error) {
      ...
      console.log('Insufficient org balance should be 0.0001, actually is:', actualBalance);
      
      // Let's also verify the organization was created correctly
      const [createdOrg] = await database.select().from(organizations).where(eq(organizations.id, insufficientOrgId)).limit(1);
      console.log('Created org credit balance from database:', createdOrg?.creditBalance);
      ...
      console.log('Billing config minimum charge:', billingConfig.pricing.minimumCharge);
      console.log('Auto top-up settings:', billingConfig.autoTopUp);
      
      // Check actual organization auto top-up settings
      const [orgSettings] = await database.select().from(organizations).where(eq(organizations.id, insufficientOrgId)).limit(1);
      ...
      try {
        const [finalOrgSettings] = await database.select().from(organizations).where(eq(organizations.id, insufficientOrgId)).limit(1);
        if (finalOrgSettings) {
    ...
    test('should handle concurrent credit deductions correctly', async () => {
      ...
      console.log('Concurrent results:', results.map(r => ({ success: r.success, deducted: r.deductedAmount, remaining: r.remainingBalance })));
      ...
      const totalDeducted = results.reduce((sum, r) => sum + (r.success ? r.deductedAmount : 0), 0);
      const finalBalance = await getCreditBalance(testOrgId);
      console.log('Total deducted:', totalDeducted, 'Final balance:', finalBalance);
      ...
      expect(results.every(r => typeof r.deductedAmount === 'number')).toBe(true);
    });
  });
  ...
  describe('Usage Analytics and Monitoring', () => {
    ...
    test('should check sufficient credits before operations', async () => {
      ...
      const hasSufficient = await CreditService.checkSufficientCredits(testOrgId, smallAmount);
      ...
      const hasInsufficient = await CreditService.checkSufficientCredits(testOrgId, largeAmount);
