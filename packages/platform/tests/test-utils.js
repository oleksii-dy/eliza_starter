...
const mockElizaCore = {
  createUniqueUuid: (prefix, str) => `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  UUID: 'test-uuid-type',
...
const databaseProxy = new Proxy({}, {
  get: (target, prop) => {
    ...
    if (prop === 'execute') {
      return jest.fn().mockResolvedValue([{ id: 'test-org-id', name: 'Core Test Organization' }]);
    }
    if (typeof prop === 'string' && [
      'select', 'insert', 'update', 'delete', 'where', 'returning', 'values',
      'from', 'limit', 'offset', 'orderBy', 'groupBy', 'having', 'distinct', 'join',
      'leftJoin', 'rightJoin', 'innerJoin', 'fullJoin', 'on', 'as', 'set'
    ].includes(prop)) {
...
jest.mock('../lib/database', () => ({
  ...
  organizations: { [Symbol.for('drizzle:Name')]: 'organizations', name: 'organizations' },
  users: { [Symbol.for('drizzle:Name')]: 'users', name: 'users' },
  userSessions: { [Symbol.for('drizzle:Name')]: 'userSessions', name: 'userSessions' },
  creditTransactions: { [Symbol.for('drizzle:Name')]: 'creditTransactions', name: 'creditTransactions' },
...
const resetMockCreditBalance = () => { mockCreditBalance = 100; }; // Start with some credits for tests

jest.mock('../lib/server/services/billing-service', () => ({
  addCredits: jest.fn().mockImplementation(async ({ organizationId, amount }) => {
  ...
  deductCredits: jest.fn().mockImplementation(async ({ organizationId, amount }) => {
    if (mockCreditBalance >= amount) {
      ...
      throw new Error('Insufficient credit balance. Please add more credits to your account.');
    }
  }),
  ...
  CreditService: {
    ...
    deductCredits: jest.fn().mockImplementation(async (organizationId, amount) => {
      if (mockCreditBalance >= amount) {
        mockCreditBalance -= amount;
        return { success: true, deductedAmount: amount, remainingBalance: mockCreditBalance };
      } else {
        throw new Error('Insufficient credits');
      }
    }),
    checkSufficientCredits: jest.fn().mockImplementation(async (organizationId, amount) => {
...
jest.mock('../lib/billing/credit-service', () => ({
  CreditService: {
    addCredits: jest.fn().mockImplementation(async (organizationId, amount) => {
      mockCreditBalance += amount;
      return true;
    ...
    calculateModelCost: jest.fn().mockImplementation((params) => {
      ...
      if ((params.inputTokens === 0 && params.outputTokens === 0) || params.tokens === 0) {
        return 0;
      }
      ...
      if (params.service === 'anthropic' && params.modelName === 'claude-3-sonnet') {
    ...
    deductCreditsForUsage: jest.fn().mockImplementation(async (organizationId, userId, usage) => {
    ...
    estimateOperationCost: jest.fn().mockImplementation((params) => {
      ...
      if (params.service === 'openai' && params.modelName === 'gpt-4') {
        const inputCost = (params.estimatedInputTokens || 1000) * 0.03 / 1000;
        const outputCost = (params.estimatedOutputTokens || 500) * 0.06 / 1000;
        return inputCost + outputCost;
      }
...
jest.mock('../lib/billing/config', () => ({
  ...
  getAgentLimitForTier: jest.fn().mockImplementation((tier) => {
    const limits = { free: 1, basic: 5, pro: 10, premium: 25, enterprise: 100 };
...
jest.mock('../lib/services/github-service', () => ({
  GitHubService: jest.fn().mockImplementation(() => ({
    createRepository: jest.fn().mockResolvedValue({ html_url: 'https://github.com/test/repo' }),
    pushFiles: jest.fn().mockResolvedValue(true),
    getUserRepositories: jest.fn().mockResolvedValue([]),
  }))
}), { virtual: true })

jest.mock('../lib/services/npm-service', () => ({
  NPMService: jest.fn().mockImplementation(() => ({
    publishPackage: jest.fn().mockResolvedValue({ packageUrl: 'https://npm.js/test' }),
    checkPackageExists: jest.fn().mockResolvedValue(false),
  }))
}), { virtual: true })

jest.mock('../lib/autocoder/e2b-container-service', () => ({
  E2BContainerService: {
...
jest.mock('../lib/database/schema', () => ({
  ...
  apiKeys: { [Symbol.for('drizzle:Name')]: 'apiKeys', name: 'apiKeys' },
  agents: { [Symbol.for('drizzle:Name')]: 'agents', name: 'agents' },
  agentBillingEvents: { [Symbol.for('drizzle:Name')]: 'agentBillingEvents', name: 'agentBillingEvents' },
}), { virtual: true })
...
jest.mock('../lib/server/services/api-key-service', () => ({
  createApiKey: jest.fn().mockResolvedValue({ 
    id: 'test-api-key-id', 
    hashedKey: 'hashed-test-key',
    organizationId: 'test-org-id'
