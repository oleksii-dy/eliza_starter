...
describe('Real Runtime Integration', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.POSTGRES_URL = process.env.POSTGRES_URL || 'postgresql://localhost:5432/eliza_test';
  ...
  describe('Standalone Server Integration', () => {
    ...
    test('should validate client assets exist', async () => {
      ...
      await expect(standaloneServer.validateClientAssets()).resolves.not.toThrow();
    });
    ...
    test('should start server and serve API endpoints', async () => {
      ...
      // Test API endpoints
      const healthResponse = await fetch(`http://localhost:${TEST_PORT}/api/runtime/health`);
      expect(healthResponse.ok).toBe(true);
      
      const healthData = await healthResponse.json() as any;
      expect(healthData.status).toBe('OK');
      
      const pingResponse = await fetch(`http://localhost:${TEST_PORT}/api/runtime/ping`);
    ...
    test('should serve client static files', async () => {
      ...
      // Test client serving
      const clientResponse = await fetch(`http://localhost:${TEST_PORT}/client`);
      ...
      // Test asset serving
      const cssResponse = await fetch(`http://localhost:${TEST_PORT}/client/assets/index.css`);
    ...
    test('should handle SPA routing', async () => {
      ...
      const spaResponse = await fetch(`http://localhost:${TEST_PORT}/client/some-spa-route`);
  ...
  describe('Character Loading and Agent Creation', () => {
    ...
    test('should create agent via API', async () => {
      ...
      const createResponse = await fetch(`http://localhost:${TEST_PORT}/api/agents`, {
        method: 'POST',
        headers: {
      ...
      // Start the agent
      const startResponse = await fetch(`http://localhost:${TEST_PORT}/api/agents/${agentId}/start`, {
        method: 'POST'
      });
      ...
      const listResponse = await fetch(`http://localhost:${TEST_PORT}/api/agents`);
