...
import { cleanupTestDatabase, createTestOrganization } from '../test-utils/database';
...
describe('Agent Memory (Real Runtime)', () => {
  ...
  test('should store and retrieve memory in real database', async () => {
    ...
    const foundMemory = memories.find(m => m.content.text === 'Real memory test');
...
import { ElizaService } from '../../lib/runtime/eliza-service';
import { testCharacterConfig } from '../fixtures/test-character';
import { createTestOrganization, cleanupTestDatabase } from '../test-utils/database';
