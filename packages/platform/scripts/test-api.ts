...
class ApiTester {
  ...
  private setAuthHeader(token: string) {
    this.context.api.defaults.headers.common['Authorization'] = `Bearer ${token}`;
  }
  ...
  async testApiKeyManagement() {
    ...
    this.assert(Array.isArray(listRes.data.data.apiKeys), 'Should return array of API keys');
    ...
    if (createRes.status === 201) {
      this.assert(createRes.data.success === true, 'Create should be successful');
      ...
      this.assert(authWithKeyRes.status === 200, 'Should authenticate with API key');
      ...
      this.assert(getRes.data.data.apiKey.id === apiKeyId, 'Should return correct API key');
      ...
      const updateRes = await this.context.api.patch(`/api-keys/${apiKeyId}`, updateData);
      this.assert(updateRes.status === 200, 'Update should return 200');
      this.assert(updateRes.data.data.apiKey.name === updateData.name, 'Name should be updated');
      ...
      this.assert(getDeletedRes.status === 404, 'Deleted key should return 404');
    }
  ...
  async testOpenApiSpec() {
    ...
    this.assert(specRes.headers['content-type'].includes('yaml'), 'Should return YAML content');
    this.assert(specRes.data.includes('openapi: 3.0.3'), 'Should be valid OpenAPI 3.0 spec');
