...
export const getUniqueAgentId = async (req: Request, res: Response) => {
    try {
        const result = await db.query(
            'SELECT DISTINCT "agentId" FROM traces WHERE "agentId" IS NOT NULL'
        );
        res.status(200).json({
            unique_agent_ids: result.rows.map((row) => row.agentId),
        });
    } catch (error: any) {
...
export const getUniqueRunsByAgent = async (req: Request, res: Response) => {
    try {
        const { agentId } = req.params;
        if (!agentId) {
            return res
        ...
        // Query to fetch distinct run values where agentId is provided
        const result = await db.query(
            'SELECT DISTINCT "run" FROM traces WHERE "agentId" = $1',
            [agentId]
        );
