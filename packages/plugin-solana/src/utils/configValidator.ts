...
export function validateSolanaConfig(runtime: IAgentRuntime): ConfigValidationResult {
  ...
  if (!walletSecretKey && !walletPrivateKey && !solanaPrivateKey && !walletSecretSalt) {
    errors.push(
      'No wallet configuration found. You must provide one of: WALLET_SECRET_KEY, WALLET_PRIVATE_KEY, SOLANA_PRIVATE_KEY, or WALLET_SECRET_SALT'
    );
  }
  ...
  for (const setting of recommendedSettings) {
    ...
    if (!value) {
      warnings.push(
        `Missing recommended setting: ${setting.key} - ${setting.description}. Using default: ${setting.default || 'none'}`
      );
    }
  }
