...
export class RpcService extends Service {
    ...
    capabilityDescription = 'Advanced RPC connection management with failover, load balancing, and circuit breakers';
    ...
    async executeWithFailover<T>(
        ...
        const endpoints = [...this.endpoints].sort((a, b) => 
        ...
        for (const endpoint of endpoints) {
            ...
            try {
                ...
                if (endpoint.failureCount >= this.maxFailures) {
                    endpoint.healthy = false;
                    logger.error(`Marking ${endpoint.url} as unhealthy after ${endpoint.failureCount} failures`);
                }
            }
        }
    ...
    getStatus(): {
        ...
        const healthyEndpoints = this.endpoints.filter(e => e.healthy);
        const currentEndpoint = healthyEndpoints.length > 0 ? healthyEndpoints[0].url : this.endpoints[0]?.url || '';
    ...
    async recoverEndpoint(url: string): Promise<boolean> {
        ...
        if (!endpoint || !breaker) return false;
