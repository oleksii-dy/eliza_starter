...
export class JupiterDexService extends Service {
  ...
  constructor(runtime: IAgentRuntime) {
    ...
    this.tokenService =
      runtime.getService<TokenService>('token-service') || new TokenService(runtime);
    this.transactionService =
      runtime.getService<TransactionService>('transaction-service') ||
      new TransactionService(runtime);
  }
   ...
   */
  async executeSwap(walletPublicKey: PublicKey, quote: SwapQuoteResponse): Promise<string> {
    try {
  ...
  private async getSwapTransaction(
    ...
    try {
      ...
      const response = await axios.post(`${this.baseUrl}/swap`, requestBody, {
        headers: {
      ...
      throw new Error(
        `Failed to get swap transaction: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
