...
export class MultiSigService extends Service {
  ...
  private readonly MULTISIG_PROGRAM_ID = new PublicKey(
  ...
  async createMultiSig(
    ...
    try {
      ...
      // Initialize multisig
      transaction.add(
        this.createInitMultiSigInstruction(multiSigKeypair.publicKey, owners, threshold)
      );
  ...
  async createTransaction(
    ...
    try {
      ...
      const isOwner = multiSigInfo.owners.some((owner) => owner.equals(walletKeypair.publicKey));
   ...
   */
  async approveTransaction(multiSig: PublicKey, transactionAccount: PublicKey): Promise<string> {
    ...
    try {
      ...
      transaction.add(
        this.createApproveInstruction(multiSig, transactionAccount, walletKeypair.publicKey)
      );
   ...
   */
  async executeTransaction(multiSig: PublicKey, transactionAccount: PublicKey): Promise<string> {
