...
export class WalletBalanceService extends Service {
  ...
  capabilityDescription =
    'Provides wallet balance information for Solana wallets with enhanced token metadata';
  ...
  async getWalletBalance(walletAddress: string): Promise<WalletBalance> {
    try {
      ...
      if (this.rpcService) {
        solBalance = await this.rpcService.executeWithFailover((conn) =>
  ...
  async getSolBalance(walletAddress: string): Promise<number> {
    try {
      ...
      let balance: number;
      if (this.rpcService) {
        balance = await this.rpcService.executeWithFailover((conn) => conn.getBalance(publicKey));
      } else {
