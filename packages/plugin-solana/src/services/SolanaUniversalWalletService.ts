...
export class SolanaChainAdapter implements ChainAdapter {
    ...
    try {
      ...
      if (!tokenAddress) {
        ...
        if (this.walletBalanceService) {
          const tokenBalance = await this.walletBalanceService.getTokenBalance(
            address,
            tokenAddress
          );
          if (tokenBalance) {
  ...
  async sendTransaction(params: UniversalTransactionParams): Promise<UniversalTransactionResult> {
    try {
      ...
      throw new Error(
        'Direct transaction sending not implemented for Solana. Use transfer() for token transfers.'
      );
    } catch (error) {
...
export class SolanaUniversalWalletService extends Service implements IUniversalWalletService {
  ...
  capabilityDescription =
    'Universal wallet operations for Solana, including balance checking, transfers, and token operations';
  ...
  constructor(runtime: IAgentRuntime) {
    ...
    this.walletBalanceService =
      runtime.getService<WalletBalanceService>('wallet-balance') || undefined;
  ...
  async getBalance(assetAddress: string, owner?: string): Promise<UniversalTokenBalance> {
    ...
    return await this.solanaAdapter.getBalance(
      walletAddress,
