...
export class WebSocketService extends Service {
   ...
   */
  async subscribeToAccount(pubkey: PublicKey, callback: AccountChangeCallback): Promise<string> {
    try {
      const subscriptionId = await this.connection.onAccountChange(pubkey, callback, 'confirmed');
   ...
   */
  async subscribeToLogs(filter: LogsFilter, callback: LogsCallback): Promise<string> {
    try {
      const subscriptionId = await this.connection.onLogs(filter, callback, 'confirmed');
  ...
  async subscribeToSignature(
    ...
    try {
      const subscriptionId = await this.connection.onSignature(signature, callback, 'confirmed');
      ...
      for (const [key, info] of subscriptions) {
        try {
          if (key.startsWith('account-')) {
            ...
            await this.subscribeToProgramAccounts(programId, undefined, info.callback as any);
          }
        } catch (error) {
