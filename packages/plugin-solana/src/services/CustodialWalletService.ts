...
export class CustodialWalletService extends Service {
    ...
    capabilityDescription = 'Enterprise-grade custodial wallet management with permissions, restrictions, and audit trails';
    ...
    private deriveKeypair(entityId: string, entityType: EntityType): Keypair {
        ...
        const seed = crypto.createHmac('sha256', this.masterSeed)
    ...
    async transferOwnership(entityId: string, currentOwner: string, newOwner: string): Promise<void> {
        ...
        await this.grantPermission(entityId, newOwner, ['read', 'transfer', 'trade', 'admin'], newOwner);
    ...
    async grantPermission(
        ...
        if (wallet.owner !== grantedBy && !await this.hasPermission(entityId, grantedBy, 'admin')) {
            throw new Error(`User ${grantedBy} does not have permission to grant access to wallet ${entityId}`);
        }
        ...
        logger.info(`Granted permissions [${permissions.join(', ')}] to user ${userId} for wallet ${entityId}`);
    }
    ...
    async revokePermission(entityId: string, userId: string, revokedBy: string): Promise<void> {
        ...
        if (wallet.owner !== revokedBy && !await this.hasPermission(entityId, revokedBy, 'admin')) {
            throw new Error(`User ${revokedBy} does not have permission to revoke access to wallet ${entityId}`);
        }
    ...
    async reactivateWallet(entityId: string, reactivatedBy: string): Promise<void> {
        ...
        if (!await this.hasPermission(entityId, reactivatedBy, 'admin')) {
            throw new Error(`User ${reactivatedBy} does not have admin permission for wallet ${entityId}`);
        }
    ...
    async exportWalletData(entityId: string, requesterId: string): Promise<{
        publicKey: string;
