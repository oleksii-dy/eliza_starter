...
export const PortfolioTracker: React.FC<PortfolioTrackerProps> = ({
    ...
    const metrics = useMemo(() => {
        if (!portfolio || historicalData.length < 2) return null;
        ...
        <div className="bg-card rounded-lg shadow-lg p-6">
            ...
            <div className="flex justify-between items-start mb-6">
                <div>
                    ...
                    <div className={`flex items-center gap-2 text-sm ${metrics.change24h >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <div className="h-48 mb-6 bg-gray-50 rounded-lg p-4">
                    <div className="h-full flex items-end justify-between gap-1">
                        {historicalData.map((point, index) => {
                            const height = (point.totalUsd / Math.max(...historicalData.map(p => p.totalUsd))) * 100;
                            return (
                                <div
                                    key={index}
