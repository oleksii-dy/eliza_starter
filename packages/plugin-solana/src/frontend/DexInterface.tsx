    ...
    const { data: availableTokens, isLoading: tokensLoading } = useQuery<Token[]>({
        ...
        queryFn: async () => {
            ...
            return [
                ...
                { address: '7GCihgDB8fe6KNjn2MYtkzZcRjQy3t9GHdC8uHYmW2hr', symbol: 'BONK', name: 'Bonk', decimals: 5 },
            ];
        },
    });

    // Get swap quote
    const { data: swapQuote, isLoading: quoteLoading, refetch: refetchQuote } = useQuery<SwapQuote | null>({
    ...
    return (
        <div className="bg-card rounded-lg shadow-lg p-6 max-w-md mx-auto">
            ...
            {swapQuote && (
                <div className="bg-blue-50 rounded-lg p-3 mb-4 text-sm">
                    <div className="flex justify-between mb-1">
                        <span>Rate</span>
                        <span>1 {inputToken?.symbol} = {swapQuote.rate} {outputToken?.symbol}</span>
                    </div>
                    <div className="flex justify-between mb-1">
            ...
            <button
                ...
                disabled={!inputToken || !outputToken || !inputAmount || parseFloat(inputAmount) <= 0 || swapMutation.isPending}
