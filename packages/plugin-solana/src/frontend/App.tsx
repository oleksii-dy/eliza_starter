...
export const SolanaPluginApp: React.FC<SolanaPluginAppProps> = ({
  ...
  const handleSendTransaction = async (transaction: any): Promise<string> => {
    ...
    setTimeout(() => {
      setTxHistory((prev) =>
        prev.map((tx) => (tx.signature === mockSignature ? { ...tx, status: 'success' } : tx))
      );
    }, 3000);
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <header className="bg-white shadow-sm border-b">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center py-4">
              <h1 className="text-2xl font-bold text-gray-900">Solana Plugin Dashboard</h1>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="space-y-6">
            ...
            {activeTab === 'swap' && (
              <DexInterface walletAddress={walletAddress || ''} apiBase={apiBase} />
            )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-card rounded-lg shadow-lg p-6">
                  ...
                  {txHistory.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {txHistory.map((tx) => (
                        <div key={tx.signature} className="p-3 bg-gray-50 rounded-lg">
        ...
        <footer className="bg-white border-t mt-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="text-center text-sm text-gray-600">
              ...
              <p className="mt-1">
                Network: {process.env.SOLANA_NETWORK || 'mainnet-beta'} • RPC:{' '}
                {process.env.SOLANA_RPC_URL ? 'Custom' : 'Default'}
              </p>
            </div>
          </div>
        </footer>
