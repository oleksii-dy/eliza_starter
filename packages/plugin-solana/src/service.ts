...
export class SolanaService extends Service {
  ...
  private async fetchPrices(): Promise<Prices> {
    ...
    for (const token of tokens) {
      ...
      if ((response as any)?.data?.value) {
        const price = (response as any).data.value.toString();
  ...
  private async getTokenAccounts() {
    try {
      if (!this.publicKey) {
        logger.warn('Public key not initialized');
        return [];
      }
    ...
    try {
      ...
      if (birdeyeApiKey) {
        try {
          ...
          if ((walletData as any)?.success && (walletData as any)?.data) {
   ...
   *
   * @returns {PublicKey | undefined} The public key of the instance.
   */
  ...
  /**
   * Stops the Solana service and cleans up resources.
   *
   * @returns {Promise<void>} - A promise that resolves once the service has stopped.
   */
  async stop(): Promise<void> {
    logger.log('Stopping Solana service');
