      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        try {
          ...
          balances.forEach((balance, address) => {
            console.log(
              `  ${address}: SOL=${balance.sol.uiAmount.toFixed(4)}, Tokens=${balance.tokens.length}`
            );
          });
        } catch (error) {
          console.error(
            `✗ Failed to fetch batch balances: ${error instanceof Error ? error.message : String(error)}`
          );
        }
