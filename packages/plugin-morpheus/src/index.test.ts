...
import { morpheusPlugin } from './index';
import {
  type IAgentRuntime,
  ModelType,
  type Memory,
...
describe('MorpheusPlugin', () => {
  const mockCharacter: Character = {
    name: 'Test Agent',
    bio: 'A test agent for Morpheus plugin',
  ...
  const mockFetch = async (url: string, options: RequestInit) => {
    ...
    // Handle Morpheus API endpoints
    if (url.includes('api.mor.org/api/v1')) {
  ...
  const mockRuntime = {
    getSetting: (key: string) => {
      switch (key) {
        case 'MORPHEUS_API_KEY':
          return 'test-key';
        case 'MORPHEUS_SMALL_MODEL':
          return 'default';
        case 'MORPHEUS_LARGE_MODEL':
          return 'default';
        case 'OPENAI_API_KEY':
  ...
  it('should have correct plugin metadata', () => {
    expect(morpheusPlugin.name).toBe('morpheus');
    expect(morpheusPlugin.description).toContain('Morpheus AI plugin');
  });

  it('should initialize without errors', async () => {
    const result = await morpheusPlugin.init({}, mockRuntime);
    expect(result).toBeUndefined();
  });

  it('should have text generation models', () => {
    expect(morpheusPlugin.models[ModelType.TEXT_SMALL]).toBeDefined();
    expect(morpheusPlugin.models[ModelType.TEXT_LARGE]).toBeDefined();
  });

  it('should have object generation models', () => {
    expect(morpheusPlugin.models[ModelType.OBJECT_SMALL]).toBeDefined();
    expect(morpheusPlugin.models[ModelType.OBJECT_LARGE]).toBeDefined();
  });

  it('should have text embedding model handler', () => {
    expect(morpheusPlugin.models[ModelType.TEXT_EMBEDDING]).toBeDefined();
  });

  it('should generate text embeddings via OpenAI', async () => {
    const embeddingHandler = morpheusPlugin.models[ModelType.TEXT_EMBEDDING];
  ...
  it('should handle empty text for embeddings', async () => {
    ...
    expect(Array.isArray(result)).toBe(true);
    expect(result[0]).toBe(0.1); // Test vector for empty input
  });
});
