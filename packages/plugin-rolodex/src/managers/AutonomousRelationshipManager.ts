...
export class AutonomousRelationshipManager {
  ...
  private async quarantineEntity(entityId: UUID): Promise<void> {
    ...
    logger.info(`[AutonomousRelationshipManager] Entity ${entityId} has been quarantined (muted in all rooms)`);
  }
    ...
    if (decision.action === 'increase_monitoring' && this.rolodexService) {
      ...
      if (contact) {
        ...
        await this.rolodexService.upsertEntity({
          ...
          metadata: {
            ...contact.metadata,
            tags: [...((contact.metadata?.tags as any[]) || []), 'enhanced-monitoring'],
            customFields: {
  ...
  private learnCommunicationPreferences(
    ...
    const peakActivity = hourCounts[parseInt(mostActiveHour)];
    ...
    return {
      ...
      insight: `Most active during hour ${mostActiveHour}:00-${parseInt(mostActiveHour) + 1}:00`,
      evidence: [
