...
export const getAliAgentKeyBuyPriceAction: Action = {
    name: 'GET_ALI_AGENT_KEY_BUY_PRICE',
    similes: ['FETCH_KEY_BUY_PRICE', 'GET_BUY_PRICE', 'HOW_MUCH_TO_BUY_KEYS'],
    description: 'Get the current buy price of a specified amount of Keys for an ALI Agent (price is for the total amount in ALI token).',
    validate: async () => true,
    ...
    handler: async (
        ...
        try {
            const prompt = composePromptFromState({ state, template: getAliAgentKeyBuyPriceTemplate as unknown as TemplateType });
            const modelResponse = await runtime.useModel(ModelType.TEXT_SMALL, { prompt });
            const parsed = parseJSONObjectFromText(modelResponse) as GetAliAgentKeyBuyPriceParams | { error: string };
            ...
            logger.info(`[getAliAgentKeyBuyPriceAction] Fetching buy price for ${amount} keys of ALI Agent ${agentId}...`);
            ...
            const responseText = `💰 **ALI Agent Key Buy Price**\n\n` +
                `**Agent ID:** ${agentId}\n` +
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error fetching key buy price.';
            const errorContent: Content = {
                text: `❌ **Error fetching ALI Agent key buy price**: ${errorMessage}`,
                data: { error: errorMessage, agentId: params?.agentId, amount: params?.amount, timestamp: new Date().toISOString() },
            };
    ...
    examples: [
        [
            { name: '{{user1}}', content: { text: 'How much does it cost to buy 1 key for agent 0x123...abc?' } },
            { name: '{{user2}}', content: { text: 'Fetching buy price...', actions: ['GET_ALI_AGENT_KEY_BUY_PRICE'] } },
        ],
        [
            { name: '{{user1}}', content: { text: 'Get buy price for 10 keys of ALI agent 0xdef...456.' } },
            { name: '{{user2}}', content: { text: 'Sure, getting that price for you.', actions: ['GET_ALI_AGENT_KEY_BUY_PRICE'] } },
        ],
        [
            { name: '{{user1}}', content: { text: 'What\'s the buy price for agent 0xaaaa...cccc for 5 keys' } },
            { name: '{{user2}}', content: { text: 'Let me check the price for 5 keys of agent 0xaaaa...cccc.', actions: ['GET_ALI_AGENT_KEY_BUY_PRICE'] } },
        ],
    ],
}; 