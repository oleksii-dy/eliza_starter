...
export const sellKeysAction: Action = {
    name: 'SELL_KEYS',
    similes: ['SELL_AGENT_KEYS', 'TRADE_KEYS_FOR_ALI', 'REDEEM_KEYS'],
    ...
    handler: async (
        ...
        try {
            const prompt = composePromptFromState({ state, template: sellKeysTemplate as unknown as TemplateType });
            ...
            logger.info(`[sellKeysAction] Fetching sell price for ${amount} keys of ALI Agent ${agentId}...`);
            ...
            logger.info(`[sellKeysAction] Sell keys transaction submitted: ${sellTx.hash}. Waiting for confirmation...`);
            ...
            const responseText = `✅ **Successfully Sold Keys**\n\n` +
                `**Agent ID:** ${agentId}\n` +
                `**Keys Sold:** ${amount}\n` +
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error during key sale.';
    ...
    examples: [
        [
            { name: '{{user1}}', content: { text: 'Sell 10 keys for ALI agent 0xagentId123...xyz.' } },
            { name: '{{user2}}', content: { text: 'Okay, attempting to sell 10 keys for agent 0xagentId123...xyz. I will first check your balance and the current sell price.', actions: ['SELL_KEYS'] } },
        ],
        [
            { name: '{{user1}}', content: { text: 'I want to sell 1 key of agent 0xanotherAgentID.' } },
            { name: '{{user2}}', content: { text: 'Understood. Selling 1 key for agent 0xanotherAgentID.', actions: ['SELL_KEYS'] } },
        ],
    ],
}; 