import {
    type Action, type Content, type HandlerCallback, type IAgentRuntime, type Memory, type State,
    logger, composePromptFromState, ModelType, type TemplateType, parseJSONObjectFromText
} from '@elizaos/core';
...
export const fusePodWithAliAgentAction: Action = {
    name: 'FUSE_POD_WITH_ALI_AGENT',
    similes: ['FUSE_POD', 'ATTACH_POD_TO_AGENT', 'LINK_POD_TO_AGENT', 'UPGRADE_AGENT_WITH_POD'],
    ...
    handler: async (
        ...
        try {
            const prompt = composePromptFromState({ state, template: fusePodWithAliAgentTemplate as unknown as TemplateType });
            const modelResponse = await runtime.useModel(ModelType.TEXT_SMALL, { prompt });
            const parsed = parseJSONObjectFromText(modelResponse) as FusePodWithAliAgentParams | { error: string };
            ...
            if (!podContractAddress) {
                if (defaultPodAddressFromSettings && defaultPodAddressFromSettings !== 'YOUR_DEFAULT_POD_NFT_CONTRACT_ADDRESS_HERE') {
                    podContractAddress = defaultPodAddressFromSettings;
                    logger.info(`[fusePodWithAliAgentAction] Using Pod NFT contract address from settings: ${podContractAddress}`);
                } else if (DEFAULT_POD_NFT_CONTRACT_ADDRESS !== 'YOUR_DEFAULT_POD_NFT_CONTRACT_ADDRESS_HERE') {
                    podContractAddress = DEFAULT_POD_NFT_CONTRACT_ADDRESS;
                    logger.info(`[fusePodWithAliAgentAction] Using default Pod NFT contract address from constants: ${podContractAddress}`);
                } else {
                    throw new Error('Pod NFT Contract Address is required. Please provide it or set POD_NFT_CONTRACT_ADDRESS in settings.');
                }
            }
            if (!podContractAddress || podContractAddress === 'YOUR_DEFAULT_POD_NFT_CONTRACT_ADDRESS_HERE') {
            ...
            logger.info(`[fusePodWithAliAgentAction] Verifying ownership of Pod NFT ${podContractAddress} token ${podId}...`);
            const podOwner = await podNftContract.ownerOf(actualPodId);
            if (podOwner.toLowerCase() !== walletAddress.toLowerCase()) {
                throw new Error(`Caller does not own Pod NFT ${podId} from contract ${podContractAddress}. Owner: ${podOwner}`);
            }
            logger.info('[fusePodWithAliAgentAction] Pod NFT ownership verified.');

            logger.info(`[fusePodWithAliAgentAction] Checking Pod NFT ${podId} approval for ALI Agent ${agentId}...`);
            ...
            if (approvedAddress?.toLowerCase() !== agentId.toLowerCase() && !isApprovedForAll) {
                logger.info(`[fusePodWithAliAgentAction] Pod NFT not approved. Approving Pod NFT ${podId} for ALI Agent ${agentId}...`);
                const approveTx = await podNftContract.approve(agentId, actualPodId);
                logger.info(`[fusePodWithAliAgentAction] Approval transaction submitted: ${approveTx.hash}. Waiting for confirmation...`);
                const approveReceipt = await approveTx.wait();
                if (!approveReceipt || approveReceipt.status !== 1) {
                    throw new Error(`Pod NFT approval transaction failed or was reverted. Hash: ${approveTx.hash}`);
                }
            ...
            logger.warn('[fusePodWithAliAgentAction] IMPORTANT: This action assumes the Pod NFT is Level 5. On-chain level verification is not implemented.')
            ...
            logger.info(`[fusePodWithAliAgentAction] Fuse Pod transaction submitted: ${fuseTx.hash}. Waiting for confirmation...`);
            ...
            const responseText = `✅ **Successfully Fused Pod with ALI Agent**\n\n` +
            ...
            const responseContent: Content = {
                ...
                data: {
                    ...
                    notes: 'Agent metadata updated on-chain. Specific updated metadata not returned by this transaction.'
                },
            };
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error during Pod fusion.';
    ...
    examples: [
        [
            { name: '{{user1}}', content: { text: 'Fuse pod 123 from contract 0xPodContract... with my ALI agent 0xAgentAddress...' } },
            { name: '{{user2}}', content: { text: 'Okay, attempting to fuse Pod 123 with ALI Agent 0xAgentAddress.... I will verify ownership and approval first.', actions: ['FUSE_POD_WITH_ALI_AGENT'] } },
        ],
        [
            { name: '{{user1}}', content: { text: 'Attach pod ID 789 to agent 0xAgentXYZ. The pod is at the default address.' } },
            { name: '{{user2}}', content: { text: 'Understood. Fusing Pod 789 (from default Pod contract) with ALI Agent 0xAgentXYZ...', actions: ['FUSE_POD_WITH_ALI_AGENT'] } },
        ],
    ],
}; 