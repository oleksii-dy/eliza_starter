...
export const getAliAgentKeySellPriceAction: Action = {
    name: 'GET_ALI_AGENT_KEY_SELL_PRICE',
    similes: ['FETCH_KEY_SELL_PRICE', 'GET_SELL_PRICE', 'HOW_MUCH_FOR_SELLING_KEYS'],
    description: 'Get the current sell price for a specified amount of Keys of an ALI Agent (price is for the total amount in ALI token).',
    validate: async () => true,
    ...
    handler: async (
        ...
        try {
            const prompt = composePromptFromState({ state, template: getAliAgentKeySellPriceTemplate as unknown as TemplateType });
            const modelResponse = await runtime.useModel(ModelType.TEXT_SMALL, { prompt });
            const parsed = parseJSONObjectFromText(modelResponse) as GetAliAgentKeySellPriceParams | { error: string };
            ...
            logger.info(`[getAliAgentKeySellPriceAction] Fetching sell price for ${amount} keys of ALI Agent ${agentId}...`);
            ...
            const responseText = `💰 **ALI Agent Key Sell Price**\n\n` +
                `**Agent ID:** ${agentId}\n` +
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error fetching key sell price.';
    ...
    examples: [
        [
            { name: '{{user1}}', content: { text: 'How much will I get for selling 1 key of agent 0x123...abc?' } },
            { name: '{{user2}}', content: { text: 'Fetching sell price...', actions: ['GET_ALI_AGENT_KEY_SELL_PRICE'] } },
        ],
        [
            { name: '{{user1}}', content: { text: 'Get sell price for 5 keys of ALI agent 0xdef...456.' } },
            { name: '{{user2}}', content: { text: 'Sure, let me check the sell price.', actions: ['GET_ALI_AGENT_KEY_SELL_PRICE'] } },
        ],
    ],
}; 