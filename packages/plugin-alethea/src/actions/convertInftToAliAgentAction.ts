...
export const convertInftToAliAgentAction: Action = {
    name: 'CONVERT_INFT_TO_ALI_AGENT',
    similes: ['CREATE_ALI_FROM_INFT', 'TOKENIZE_INFT', 'INFT_TO_ALI_AGENT'],
    description: 'Convert an existing iNFT into an ALI Agent using the Alethea AI protocol (on Ethereum Mainnet).',
    ...
    handler: async (
        ...
        try {
            ...
            try {
                paramsJson = parseJSONObjectFromText(modelResponse) as ConvertInftToAliAgentParams | { error: string };
                logger.debug('[convertInftToAliAgentAction] iNFT Convert parameters extracted:', paramsJson);
                if ('error' in paramsJson) {
                    throw new Error(paramsJson.error);
                }
                convertParams = paramsJson;
            } catch (e) {
                logger.error('[convertInftToAliAgentAction] Failed to parse LLM response for iNFT convert params:', modelResponse, e);
            ...
            const rpcUrl = runtime.getSetting('ALETHEA_RPC_URL'); // Assumed to be Ethereum RPC for this action
            ...
            if (inftOwner.toLowerCase() !== walletAddress.toLowerCase()) {
                throw new Error(`Caller does not own the iNFT. Owner: ${inftOwner}, Caller: ${walletAddress}`);
            }

            logger.info('[convertInftToAliAgentAction] Converting iNFT to ALI Agent via deploySharesContractPaused...');
            ...
            logger.info('[convertInftToAliAgentAction] Transaction confirmed. Retrieving ALI Agent address...');
            const aliAgentAddress = await keysFactoryContract.lookupSharesContract({
                tokenAddress: inftContract,
                tokenId: inftTokenId,
            });

            logger.info(`[convertInftToAliAgentAction] Successfully converted iNFT to ALI Agent. Agent Address: ${aliAgentAddress}`);

            const responseText = `🤖 **iNFT Successfully Converted to ALI Agent (on Ethereum)**\n\n` +
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred during iNFT to ALI Agent conversion.';
    ...
    examples: [
        [
            {
                name: '{{user1}}',
                content: { text: 'Convert my iNFT 0xabcdef...1234 token ID 777 into an ALI Agent on Ethereum.' },
            },
            {
                name: '{{user2}}',
                content: {
                    text: "Okay, I'll convert your iNFT into an ALI Agent on Ethereum. This will use the standard ETH-based implementation (type 0).",
                    actions: ['CONVERT_INFT_TO_ALI_AGENT'],
                },
            },
        ],
        [
            {
                name: '{{user1}}',
                content: { text: 'Tokenize my iNFT collection_address token_id for my_friend_address.' },
            },
            {
                name: '{{user2}}',
                content: {
                    text: "Converting your iNFT to an ALI Agent and assigning ownership to your_friend_address. This will be on Ethereum.",
                    actions: ['CONVERT_INFT_TO_ALI_AGENT'],
                },
            },
        ],
