...
import { convertNftToAliAgentTemplate, convertInftToAliAgentTemplate, getAliAgentKeyBuyPriceTemplate } from '../templates';
import { ERC721_ABI, KEYS_FACTORY_ABI, TRADEABLE_SHARES_ABI } from '../abis';
import { KEYS_FACTORY_ADDRESS, DEFAULT_IMPLEMENTATION_TYPE, INFT_KEYS_FACTORY_ADDRESS, ALI_TOKEN_ADDRESS } from '../constants';
...
export const convertNftToAliAgentAction: Action = {
    name: 'CONVERT_NFT_TO_ALI_AGENT',
    similes: ['CREATE_ALI_AGENT', 'TOKENIZE_NFT', 'NFT_TO_ALI_AGENT', 'CONVERT_NFT_TO_AGENT'],
    ...
    handler: async (
        ...
        try {
            ...
            try {
                paramsJson = parseJSONObjectFromText(modelResponse) as ConvertNftToAliAgentParams | { error: string };
            ...
            if (!privateKey) {
                const errMsg = 'PRIVATE_KEY is required for NFT conversion. Please set it in agent settings.';
                logger.error(`[convertNftToAliAgentAction] ${errMsg}`);
                if (callback) await callback({ text: `❌ Error: ${errMsg}` });
                throw new Error(errMsg);
            }
            ...
            logger.info(`[convertNftToAliAgentAction] Transaction confirmed. Retrieving ALI Agent address...`);
        ...
        [
            {
                name: '{{user1}}',
                content: {
                    text: 'Convert my NFT at contract 0x1234567890abcdef1234567890abcdef12345678 token ID 42 into an ALI Agent',
                },
            },
            {
                name: '{{user2}}',
                content: {
                    text: "I'll convert your NFT into an ALI Agent. This will create a new intelligent agent backed by your NFT with embedded liquidity through its keys contract.",
                    actions: ['CONVERT_NFT_TO_ALI_AGENT'],
                },
            },
        ],
        [
            {
                name: '{{user1}}',
                content: {
                    text: 'Turn NFT 0xabcd1234abcd1234abcd1234abcd1234abcd1234 token 123 into an ALI agent using ALI implementation',
                },
            },
            {
                name: '{{user2}}',
                content: {
                    text: 'Converting your NFT to an ALI Agent using ALI implementation. I will verify ownership before proceeding. The new ALI Agent address will also be its keys contract address for trading.',
                    actions: ['CONVERT_NFT_TO_ALI_AGENT'],
                },
            },
        ],
        [
            {
                name: '{{user1}}',
                content: {
                    text: 'Create ALI agent from my Bored Ape #5555 at 0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D',
                },
            },
            {
                name: '{{user2}}',
                content: {
                    text: 'I will convert your Bored Ape #5555 into an ALI Agent. This will enable key trading and give your NFT AI-powered capabilities. You will receive the ALI Agent address, which is also its keys contract address.',
                    actions: ['CONVERT_NFT_TO_ALI_AGENT'],
                },
            },
        ],
