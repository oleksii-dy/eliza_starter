...
export const createLiquidityPoolAction: Action = {
  ...
  similes: ['CREATE_POOL', 'ADD_LIQUIDITY', 'SETUP_LIQUIDITY_POOL', 'BOOTSTRAP_LIQUIDITY'],
  ...
  handler: async (
    ...
    try {
      ...
      if (typeof parsed !== 'object' || parsed === null) {
        logger.error(`[createLiquidityPoolAction] Failed to parse model response: ${modelResponse}`);
      ...
      if (tokenSymbolMap[upperCaseTokenB]) {
        logger.info(`[createLiquidityPoolAction] Resolving token symbol "${tokenB}" to address ${tokenSymbolMap[upperCaseTokenB]}`);
      ...
      if (!feeTier || typeof feeTier !== 'number') {
        ...
        logger.info(`[createLiquidityPoolAction] No valid fee tier provided. Using default fee tier: ${feeTier}`);
      }
    ...
    [
      {
        name: '{{user1}}',
        content: {
          text: 'Create liquidity pool for token 0x123...abc paired with WETH, add 10000 tokens and 1 ETH with 0.3% fee',
        },
      },
      {
        name: '{{user2}}',
        content: {
          text: 'Creating a Uniswap V3 liquidity pool for the specified token pair. I will check balances, approve tokens, and add liquidity.',
          actions: ['CREATE_LIQUIDITY_POOL'],
        },
      },
    ],
    [
      {
        name: '{{user1}}',
        content: {
          text: 'Setup liquidity pool: tokenA=0xabc...123, tokenB=0xdef...456, amountA=5000000000000000000000, amountB=2000000000000000000000, fee=3000',
        },
      },
      {
        name: '{{user2}}',
        content: {
          text: 'Setting up the liquidity pool with the specified parameters.',
          actions: ['CREATE_LIQUIDITY_POOL'],
        },
      },
    ],
