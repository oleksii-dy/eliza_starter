...
export const buyKeysAction: Action = {
    name: 'BUY_KEYS',
    similes: ['PURCHASE_KEYS', 'ACQUIRE_KEYS', 'BUY_AGENT_KEYS'],
    description: 'Buy a specified number of keys for an ALI Agent. Requires prior approval of ALI tokens.',
    validate: async () => true,
    ...
    handler: async (
        ...
        try {
            const prompt = composePromptFromState({ state, template: buyKeysTemplate as unknown as TemplateType });
            ...
            logger.info(`[buyKeysAction] Fetching buy price for ${amount} keys of ALI Agent ${agentId}...`);
            ...
            logger.info(`[buyKeysAction] Price for ${amount} keys is ${priceForAmountFormatted} ALI. Checking allowance...`);

            const currentAllowance = await aliTokenContract.allowance(walletAddress, agentId);
            if (currentAllowance < priceForAmountBigInt) {
                logger.info(`[buyKeysAction] Insufficient allowance. Current: ${currentAllowance.toString()}, Required: ${priceForAmountBigInt.toString()}. Approving...`);
                const approveTx = await aliTokenContract.approve(agentId, MaxUint256); // Approve effectively infinite for simplicity
                logger.info(`[buyKeysAction] Approval transaction submitted: ${approveTx.hash}. Waiting for confirmation...`);
                const approveReceipt = await approveTx.wait();
                if (!approveReceipt || approveReceipt.status !== 1) {
                    throw new Error(`ALI Token approval transaction failed or was reverted. Hash: ${approveTx.hash}`);
                }
                logger.info(`[buyKeysAction] ALI Token approval confirmed.`);
            } else {
                logger.info(`[buyKeysAction] Sufficient allowance already granted: ${formatUnits(currentAllowance, 18)} ALI.`);
            }
            ...
            logger.info(`[buyKeysAction] Buy keys transaction submitted: ${buyTx.hash}. Waiting for confirmation...`);
            ...
            const responseText = `✅ **Successfully Bought Keys**\n\n` +
                `**Agent ID:** ${agentId}\n` +
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error during key purchase.';
    ...
    examples: [
        [
            { name: '{{user1}}', content: { text: 'Buy 10 keys for ALI agent 0xagentId123...xyz.' } },
            { name: '{{user2}}', content: { text: 'Okay, attempting to buy 10 keys for agent 0xagentId123...xyz. I will first check the price, approve the ALI tokens if necessary, and then execute the purchase.', actions: ['BUY_KEYS'] } },
        ],
        [
            { name: '{{user1}}', content: { text: 'Purchase 1 key for 0xanotherAgentID.' } },
            { name: '{{user2}}', content: { text: 'Understood. Buying 1 key for agent 0xanotherAgentID.', actions: ['BUY_KEYS'] } },
        ],
    ],
}; 