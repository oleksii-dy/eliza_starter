...
describe("Memory Operations with Vector", () => {
    ...
    test("should effectively handle complex multi-context semantic relationships", async () => {
        ...
        const queries = [
            {
                ...
                // Instead of exact weights, define min/max ranges
                contextRanges: {
                    science: { min: 0.2, max: 0.6 }, // Allow 20-60% science content
                    programming: { min: 0.2, max: 0.6 }, // Allow 20-60% programming content
                },
                contextTerms: {
                    science: [
                        "science",
            ...
            },
            {
                ...
                contextRanges: {
                    science: { min: 0.2, max: 0.6 },
                    programming: { min: 0.2, max: 0.6 },
                },
                contextTerms: {
                    science: [
                        "science",
        ...
        for (const query of queries) {
            ...
            const results = await adapter.searchMemoriesByEmbedding(embedding, {
                ...
                roomId: seedManager.ROOM_ID,
                agentId: seedManager.AGENT_ID,
                match_threshold: 0.3, // Lowered threshold to catch more semantic relationships
                count: 10,
            });
            ...
            // Test for semantic coherence with adjusted threshold
            ...
            // Verify we have some cross-context results or high similarity results
            const hasValidResults =
                crossContextResults.length > 0 ||
                results.some((r) => (r.similarity ?? 0) > 0.7);
    ...
    test("should get memory by ID - existing and non-existing cases", async () => {
        ...
        const expectedText = seedManager.getTextByMemoryId(existingMemoryId);
