    ...
    try {
        ...
        await Promise.all(
            containers.map(async (containerInfo) => {
                ...
                try {
                    // Force stop to ensure it terminates
                    await container.stop({ t: 5 });
                    elizaLogger.success(
                        `Stopped container: ${containerInfo.Id.substring(
                            0,
                            12
                        )}`
                    );
                    ...
                    if (
                        ...
                        elizaLogger.error(
                            `Failed to stop container ${containerInfo.Id.substring(
                                0,
                                12
                            )}: ${error.message}`
                        );
...
export const cleanCache = async (client: pg.Client) => {
    elizaLogger.info("Starting cache cleanup...");
    try {
        await client.query("DELETE FROM cache WHERE TRUE");
        elizaLogger.success("Cache cleanup completed successfully");
    } catch (error) {
        elizaLogger.error(
