...
describe("Knowledge Operations", () => {
    ...
    afterAll(async () => {
        try {
            // Vitest -> stopContainers causes 57P01 error.
            ...
            if (error instanceof Error && error.message.includes("57P01")) {
                // 57P01 is the code for admin termination
                return;
            }
            throw error;
        }
    ...
    describe("Knowledge Search Operations", () => {
        test("should find shared knowledge items across all agents", async () => {
            ...
            expect(searchResults[0].content.text).toContain(
                "Agile methodology"
            );
        });
        ...
        test("should not find private knowledge items from other agents", async () => {
            ...
            const privateResults = searchResults.filter(
                (r) => !r.content.metadata?.isShared
            );
        ...
        test("should find relevant chunks from main documents", async () => {
            ...
            const chunks = searchResults.filter(
                (r) => r.content.metadata?.isChunk
            );
        ...
        test("should properly rank chunks vs main documents in search results", async () => {
            ...
            const mainDocs = searchResults.filter(
                (r) => r.content.metadata?.isMain
            );
            const chunks = searchResults.filter(
                (r) => r.content.metadata?.isChunk
            );
        ...
        test("should return results above similarity threshold", async () => {
            ...
            expect(searchResults[0].content.text.toLowerCase()).toContain(
                "machine learning"
            );
        ...
        test("should find shared knowledge items when searching as different agents", async () => {
            ...
            expect(results1.some((r) => r.content.metadata?.isShared)).toBe(
                true
            );
            expect(results2.some((r) => r.content.metadata?.isShared)).toBe(
                true
            );
        });
        ...
        test("should respect privacy when searching across multiple agents", async () => {
            ...
            results
                .filter((r) => !r.content.metadata?.isShared)
        ...
        test("should handle search with both shared and private content", async () => {
            ...
            const privateResults = results.filter(
                (r) => !r.content.metadata?.isShared
            );
        ...
        test("should properly handle document hierarchies in search results", async () => {
            ...
            const relatedChunks = results.filter(
                (r) =>
        ...
        test("should maintain proper relevance scoring across document types", async () => {
            ...
            const sharedDocs = results.filter(
                (r) => r.content.metadata?.isShared
            );
            const privateDocs = results.filter(
                (r) => !r.content.metadata?.isShared
            );
            ...
            results.forEach((result, i) => {
                if (i > 0) {
                    expect(result.similarity).toBeLessThanOrEqual(
                        results[i - 1].similarity || 0
                    );
                }
            });

            // Most relevant result should be about version control or CI
            expect(results[0].content.text.toLowerCase()).toMatch(
    ...
    describe("Edge Cases and Error Handling", () => {
        ...
        test("should handle invalid search parameters", async () => {
            try {
                ...
                if (
                    ...
                    error.message !== "LIMIT must not be negative"
                ) {
                    throw error;
                }
            }
        ...
        test("should handle non-existent agent IDs", async () => {
            ...
            expect(results.every((r) => r.content.metadata?.isShared)).toBe(
                true
            );
        });
        ...
        test("should handle malformed knowledge items", async () => {
            ...
            await expect(
                adapter.createKnowledge(
                    malformedItem as unknown as RAGKnowledgeItem
                )
            ).rejects.toThrow();
        });
    });

    describe("Knowledge Timestamp Operations", () => {
        test("should properly handle timestamps during creation and retrieval", async () => {
            // Create a fixed timestamp for testing
            const testDate = new Date("2024-02-06T12:00:00.000Z");
            const timestamp = testDate.getTime();
    
            const knowledgeItem = {
                id: stringToUuid("test-knowledge-timestamp"),
                agentId: AGENT_4_ID,
                content: {
                    text: "Test content for timestamp",
                    metadata: {
                        category: "test",
                        isMain: true,
                        isShared: false,
            ...
            // Verify that the timestamp corresponds to the correct ISO string
            const retrievedDate = new Date(result[0].createdAt || 0);
            expect(retrievedDate.toISOString()).toBe(testDate.toISOString());
        });
    
        test("should handle current timestamp if createdAt is not provided", async () => {
            const knowledgeItem = {
                id: stringToUuid("test-knowledge-timestamp-now"),
                agentId: AGENT_4_ID,
                content: {
                    text: "Test content for current timestamp",
                    metadata: {
                        category: "test",
                        isMain: true,
                        isShared: false,
                ...
                // Not providing createdAt to test default behavior
            };
            ...
            expect(result[0].createdAt).toBeGreaterThanOrEqual(beforeCreate);
            expect(result[0].createdAt).toBeLessThanOrEqual(afterCreate);
            expect(typeof result[0].createdAt).toBe("number");
    
            // Verify the timestamp can be converted to a valid date
            const retrievedDate = new Date(result[0].createdAt || 0);
            expect(retrievedDate).toBeInstanceOf(Date);
            expect(retrievedDate.getTime()).toBe(result[0].createdAt);
        });
    });
});
