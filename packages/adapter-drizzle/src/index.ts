...
import { and, eq, gte, lte, sql, desc, inArray, or, cosineDistance } from "drizzle-orm";
import {
    accounts,
    goals,
    logs,
    ...
    async createMemory(
        ...
        try {
            ...
            const contentToInsert = typeof memory.content === 'string' 
                ...
                elizaLogger.info("CONTENT TO INSERT", contentToInsert);
            ...
            try {
                ...
                if (metadata.isChunk && metadata.originalId) {
                    ...
                    await tx.insert(knowledges).values({
                        id: knowledge.id,
                        agentId: metadata.isShared ? null : knowledge.agentId,
                        content: sql`${typeof knowledge.content === 'string' 
                            ? JSON.parse(knowledge.content) 
                            : knowledge.content}::jsonb`,
                        embedding: knowledge.embedding
