...
import { AgentRuntime, MemoryManager } from '@elizaos/core';
import { PgliteDatabase, PgLiteDatabaseAdapter } from '@elizaos/adapter-pglite';
...
You have access to both GitHub plugin actions and sandbox management capabilities.`,
  ...
  it('should process GitHub repository creation request through agent', async () => {
    ...
    const hasTeamMention = responseText.includes('team') || responseText.includes('developer') || responseText.includes('agent');
...
`;
    ...
    console.log(`View branch: https://github.com/${testRepoFullName}/tree/feature/todo-app-development`);
  }, 15000);
...
The development team should work on the feature/todo-app-development branch and create a complete full-stack application as specified in the requirements.`,
    ...
    const memories = await runtime.getMemories({ roomId, count: 5 });
