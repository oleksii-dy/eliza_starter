  ...
  private async validateConfiguration(): Promise<void> {
    ...
    try {
      ...
      throw new DatabaseError(
        ...
        { component: 'TrainingService', error: error instanceof Error ? error.message : String(error) }
      );
    }
  }
    ...
    return await ErrorHandler.handleError(
      ...
      async () => {
        ...
        const [room, participants] = await Promise.all([
          safely(
            () => (this.runtime as any).adapter?.getRoom(roomId),
            'get_room_metadata',
            { roomId }
          ),
          safely(
        ...
        const conversation: TrainingConversation = {
          ...
          metadata: {
            ...
            actionCount: messages.reduce((count, msg) => 
              count + (msg.metadata?.actions?.length || 0), 0),
            successfulActions: messages.reduce((count, msg) => 
              count + (msg.metadata?.actionResults?.filter(r => r.success).length || 0), 0),
          },
        };

        // Calculate quality score with error handling
        conversation.metadata.quality = await safely(
    ...
    return await ErrorHandler.handleError(
      ...
      async () => {
        ...
        if (config.extractionConfig.includeActions) {
          trainingMessage.metadata!.actionResults = await safely(
            () => this.extractActionResults(memory),
        ...
        if (config.extractionConfig.includeProviders) {
          trainingMessage.metadata!.providerResults = await safely(
            () => this.extractProviderResults(memory),
        ...
        if (config.extractionConfig.includeEvaluators) {
          trainingMessage.metadata!.evaluationResults = await safely(
        ...
        if (config.datasetConfig.maxTokens) {
          trainingMessage.metadata!.tokenCount = await safely(
  ...
  private async calculateConversationQuality(conversation: TrainingConversation): Promise<number> {
    try {
      ...
      if (conversation.metadata.actionCount > 0) {
        const successRate = conversation.metadata.successfulActions / conversation.metadata.actionCount;
      ...
      if (userMessages.length > 0 && assistantMessages.length > 0) {
        const ratio = Math.min(userMessages.length, assistantMessages.length) / 
    ...
    return await withRetry(
      async () => {
        ...
        try {
          ...
          const trainingError = ErrorHandler.normalizeError(
            error,
            'start_training',
            { jobId, configType: 'TrainingConfig' }
          );
