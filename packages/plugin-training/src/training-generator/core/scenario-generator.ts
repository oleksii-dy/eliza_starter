...
export class ScenarioGenerator {
    ...
    for (let i = 0; i < filesToProcess.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (file) => {
        try {
          ...
          elizaLogger.warn(
            `⚠️  Failed to generate scenario for ${file.relativePath}:`,
      ...
      const validScenarios = batchResults.filter(
        (scenario): scenario is TrainingScenario => scenario !== null
      );
      scenarios.push(...validScenarios);

      elizaLogger.info(
        `✅ Generated ${scenarios.length}/${filesToProcess.length} scenarios (batch ${Math.floor(i / batchSize) + 1})`
      );
    }
  ...
  private async createFileScenario(
    ...
    const thinkingProcess = await this.generateThinkingProcess(
      targetFile,
      relatedFiles,
      userQuery,
      repositoryType
   ...
   */
  private async generateUserQuery(
    file: ExtractedFile,
    repositoryType: 'core' | 'plugin'
  ): Promise<string> {
    const contextMap = {
...
${relatedFiles
  .slice(0, 3)
  .map((f) => `- ${f.relativePath}: ${f.purpose}`)
  .join('\n')}
...
Format as natural thinking, not as a numbered list.`;
  ...
  private findRelatedFiles(targetFile: ExtractedFile, allFiles: ExtractedFile[]): ExtractedFile[] {
    ...
    for (const file of allFiles) {
      ...
      if (
        targetFile.imports.some((imp) => imp.includes(file.relativePath.replace(/\.[^.]+$/, '')))
      ) {
        relevanceScore += 4;
      }
      if (
        file.imports.some((imp) => imp.includes(targetFile.relativePath.replace(/\.[^.]+$/, '')))
      ) {
        relevanceScore += 4;
      }
    ...
    return related.sort((a: any, b: any) => b.relevanceScore - a.relevanceScore).slice(0, 5);
  }
   ...
   */
  private generateRepositoryContext(
    files: ExtractedFile[],
    repositoryType: 'core' | 'plugin'
  ): string {
...
${file.content}
   ...
   */
  private filterFilesForScenarios(
    files: ExtractedFile[],
    options: ScenarioGenerationOptions
  ): ExtractedFile[] {
    return files.filter((file) => {
      // Skip test files if not included
      if (!options.includeTestFiles && file.isTestFile) {
        return false;
      }

      // Skip config files if not included
      if (!options.includeConfigFiles && file.isConfigFile) {
        return false;
      }

      // Skip complex files if not included
      if (!options.includeComplexFiles && file.complexity === 'complex') {
        return false;
      }
  ...
  getScenarioStatistics(scenarios: TrainingScenario[]): {
    ...
    scenarios.forEach((scenario) => {
      complexityDistribution[scenario.metadata.complexity] =
        (complexityDistribution[scenario.metadata.complexity] || 0) + 1;
      languageDistribution[scenario.metadata.language] =
        (languageDistribution[scenario.metadata.language] || 0) + 1;
