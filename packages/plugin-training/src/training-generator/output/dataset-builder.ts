...
export class DatasetBuilder {
   ...
   */
  async exportByCategoryTogetherAI(
   ...
   */
  async exportByComplexityTogetherAI(
    examples: TrainingExample[],
    outputDir: string
  ): Promise<string[]> {
  ...
  calculateMetrics(examples: TrainingExample[]): DatasetMetrics {
    ...
    for (const example of examples) {
      ...
      typeDistribution[example.metadata.type] = (typeDistribution[example.metadata.type] || 0) + 1;
  ...
  private generateRecommendations(metrics: DatasetMetrics): string[] {
    ...
    if (metrics.totalTokens < 100000) {
      recommendations.push(
        'Consider generating more training examples for better model performance'
      );
    } else if (metrics.totalTokens > 10000000) {
      recommendations.push(
        'Dataset is quite large - consider filtering or sampling for faster training'
      );
    }
    ...
    if (simpleRatio > 0.6) {
      recommendations.push(
        'Consider adding more medium and complex examples for comprehensive learning'
      );
    }
    if (complexRatio < 0.1) {
    ...
    if (typeCount < 3) {
      recommendations.push(
        'Consider adding more diverse scenario types (file-creation, plugin-creation, documentation)'
      );
    }
