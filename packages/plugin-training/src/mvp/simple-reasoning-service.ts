...
export class SimpleReasoningService {
    ...
    getStatus(): { enabled: boolean; dataCount: number; lastActivity?: number } {
        return {
            ...
            lastActivity: this.trainingData.length > 0 ? 
                Math.max(...this.trainingData.map(d => d.timestamp)) : undefined
        };
    }
    ...
    private determineModelType(modelType: any, params: any): 'should_respond' | 'planning' | 'coding' {
