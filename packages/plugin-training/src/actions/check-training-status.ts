...
export const checkTrainingStatusAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory) => {
    ...
    return (text.includes('check') || text.includes('status') || text.includes('progress')) &&
...
function extractJobId(message: Memory): string | null {
  ...
  const jobIdMatch = text.match(/(?:job|id)[:\s]+(ft-[a-zA-Z0-9-]+)/i) ||
                     text.match(/(ft-[a-zA-Z0-9-]+)/);
...
function formatJobStatus(job: any): string {
  ...
  const finishedAt = job.finished_at ? new Date(job.finished_at * 1000).toLocaleString() : 'Not finished';
  ...
  if (job.status === 'completed') {
    ...
    report += `Your model is being fine-tuned. This typically takes 2-6 hours depending on dataset size.`;
...
function formatAllJobs(jobsResponse: any): string {
  ...
  for (const job of sortedJobs.slice(0, 10)) { // Show last 10 jobs
    const statusEmoji = getStatusEmoji(job.status);
