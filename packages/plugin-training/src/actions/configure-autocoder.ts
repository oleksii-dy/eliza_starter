...
export const configureAutoCoderAction: Action = {
  ...
  description:
    'Configure auto-coder to use Together.ai trained models for ElizaOS development. Enables action chaining with training status checks and model testing.',

  validate: async (runtime: IAgentRuntime, message: Memory) => {
    const text = message.content.text?.toLowerCase() || '';

    return (
      (text.includes('configure') || text.includes('setup') || text.includes('config')) &&
      (text.includes('auto') ||
        text.includes('coder') ||
  ...
  handler: async (
    ...
    try {
      ...
      if (!proxyService) {
        ...
        return {
          text: 'Reasoning proxy service not found',
          data: {
            actionName: 'CONFIGURE_AUTOCODER',
      ...
      if (!selectedModel) {
        ...
        return {
          text: 'No fine-tuned models available',
          data: {
            actionName: 'CONFIGURE_AUTOCODER',
            availableModels,
            error: 'no_models',
  ...
  examples: [
    // Multi-action: Check status then configure
    [
      {
        name: 'User',
        content: {
          text: "Check my training job status and configure auto-coder if it's complete",
        },
      },
      {
        name: 'Assistant',
        content: {
          text: "I'll check your training job status and configure auto-coder if it's ready.",
          thought: 'User wants to check training status and setup auto-coder',
          actions: ['CHECK_TRAINING_STATUS', 'CONFIGURE_AUTOCODER'],
        },
      },
    ],
    // Multi-action: Configure then test
    [
      {
        name: 'User',
        content: {
          text: 'Configure auto-coder with my trained model and test it by generating an action',
        },
      },
      {
        name: 'Assistant',
        content: {
          text: "I'll configure auto-coder with your trained model and test it by generating code.",
          thought: 'User wants to configure and test auto-coder',
          actions: ['CONFIGURE_AUTOCODER', 'GENERATE_TRAINING_DATA'],
        },
      },
    ],
    // Multi-action: Train then configure
    [
      {
        name: 'User',
        content: {
          text: 'Train a model with my dataset and then configure auto-coder to use it',
        },
      },
      {
        name: 'Assistant',
        content: {
          text: "I'll start training your model and then configure auto-coder to use it.",
          thought: 'User wants complete training and configuration workflow',
          actions: ['TRAIN_MODEL', 'MONITOR_TRAINING', 'CONFIGURE_AUTOCODER'],
        },
      },
    ],
    [
...
function extractAutoCoderConfig(message: Memory) {
  ...
  return {
    model:
      extractValue(text, /model[:\s]+(ft-[a-zA-Z0-9-]+)/i) ||
      extractValue(text, /model[:\s]+([^\s]+)/i),
    enabled: !text.includes('disable') && !text.includes('off'),
    temperature: parseFloat(extractValue(text, /temperature[:\s]+([\d.]+)/i, '0.1') || '0.1'),
    maxTokens: parseInt(
      extractValue(text, /(?:max[_\s]?tokens?|tokens)[:\s]+(\d+)/i, '4000') || '4000',
      10
    ),
 ...
 * Save configuration to runtime settings
 */
async function saveConfigurationToRuntime(
  runtime: IAgentRuntime,
