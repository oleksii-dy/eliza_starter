  ...
  program
    ...
    .action(async (options) => {
      try {
        ...
        elizaLogger.info('  3. Train model: eliza-training train-model --model "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"');
        
      } catch (error) {
        elizaLogger.error('❌ Error extracting from database:', error instanceof Error ? error.message : String(error));
        process.exit(1);
      }
    });
}