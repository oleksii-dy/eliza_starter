...
export function testFineTunedCommand(program: Command) {
  program
    ...
    .option('-b, --base-model <model>', 'Base model for comparison', 'deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B')
    ...
    .action(async (options) => {
      try {
        ...
        const testPrompts = [
          {
            ...
            prompt: "Create a Discord plugin for ElizaOS that responds to mentions with personalized greetings based on user history."
          },
          {
            name: "Twitter Integration",
            prompt: "Build an ElizaOS plugin that monitors Twitter mentions and automatically responds with contextual replies."
          },
          {
            name: "Database Action",
            prompt: "Create an ElizaOS action that stores user preferences in a database and retrieves them in future conversations."
          },
          {
            name: "Blockchain Provider",
            prompt: "Design an ElizaOS provider that fetches real-time crypto prices and wallet balances for Solana."
          },
          {
            name: "Complex Workflow",
            prompt: "Build an ElizaOS plugin with actions, providers, and evaluators that manages a complete project workflow from task creation to completion tracking."
          }
        ];
        ...
        const avgFineScore = results.testResults.reduce((sum, r) => sum + r.fineTuned.score, 0) / results.testResults.length;
        const avgBaseScore = results.testResults.reduce((sum, r) => sum + r.base.score, 0) / results.testResults.length;
        ...
        if (avgFineScore > avgBaseScore) {
          elizaLogger.info('✅ Fine-tuning successful! Model shows improvement in ElizaOS development tasks.');
        ...
        elizaLogger.error('❌ Testing failed:', error instanceof Error ? error.message : String(error));
        process.exit(1);
      }
    });
}
...
async function testModelInference(apiKey: string, model: string, prompt: string): Promise<string> {
  try {
    ...
    throw new Error(`Inference failed for ${model}: ${error instanceof Error ? error.message : String(error)}`);
  }
}

function analyzeResponses(fineResponse: string, baseResponse: string, prompt: string): {
  fineScore: number;
  baseScore: number;
  analysis: string;
...
function scoreResponse(response: string, prompt: string): number {
  ...
  const elizaPatterns = [
    'Plugin', 'Action', 'Provider', 'Evaluator', 'IAgentRuntime',
    'handler', 'validate', 'export', 'import.*@elizaos',
    'runtime\\.', 'memory', 'state', 'character'
  ];
  ...
  if (response.includes('export') && response.includes('interface')) score += 0.5;
  if (response.includes('async') && response.includes('await')) score += 0.5;
  ...
  if (codeBlocks >= 2) score += 0.5;

  // Check for good practices
  if (response.includes('type') || response.includes('interface')) score += 0.5;
  if (response.includes('// ') || response.includes('* ')) score += 0.3;

  // Penalty for incomplete or broken code
  if (response.includes('...') || response.includes('TODO')) score -= 0.5;
  if (response.length < 200) score -= 1;

  return Math.max(0, Math.min(10, score));
}

function generateAnalysis(fineResponse: string, baseResponse: string, fineScore: number, baseScore: number): string {
  const analyses = [];
  ...
  return analyses.length > 0 ? analyses.join('. ') + '.' : 'Both models show similar performance characteristics.';
}