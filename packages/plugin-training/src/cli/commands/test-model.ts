    ...
    .action(async (options) => {
      try {
        ...
        if (!options.model || !options.prompt) {
          elizaLogger.error('❌ Error: Both --model and --prompt are required for inference testing');
          elizaLogger.info('💡 Or use --job-id to check training status, or --list-models to see available models');
          process.exit(1);
        }

        const maxTokens = parseInt(options.maxTokens) || 100;
        ...
        elizaLogger.error('❌ Error testing model:', error instanceof Error ? error.message : String(error));
        process.exit(1);
      }
    });
}