...
export function extractSimpleCommand(program: Command) {
  program
    ...
    .option('-db, --db-dir <path>', 'ElizaOS database directory', process.env.ELIZA_DB_DIR || './.elizadb')
    ...
    .action(async (options) => {
      try {
        ...
        elizaLogger.info(`📋 Filtered to ${filteredExamples.length} high-quality examples (quality >= ${minQuality})`);
        ...
        elizaLogger.info('  3. Train model: npm run cli train-model --model "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"');
        
      } catch (error) {
        elizaLogger.error('❌ Error extracting data:', error instanceof Error ? error.message : String(error));
        process.exit(1);
      }
    });
}