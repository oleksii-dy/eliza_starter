...
describe('TrainingDatabaseManager Real Runtime Integration', () => {
  ...
  afterEach(async () => {
    ...
    if (runtime?.db) {
      try {
        ...
        await runtime.db.run('DELETE FROM reasoning_decisions WHERE agent_id = ?', [runtime.agentId]);
      } catch (error) {
  ...
  describe('Training Data Storage with Real Database', () => {
    ...
    it('should handle JSON serialization correctly in real database', async () => {
      ...
      expect(retrieved!.metadata.complexMetadata).toEqual(complexDataPoint.metadata!.complexMetadata);
  ...
  describe('Training Data Statistics with Real Database', () => {
    ...
    it('should calculate statistics from real database queries', async () => {
      ...
      console.log(`✅ Statistics calculated: ${stats.total} total records, avg confidence: ${stats.avgConfidence}`);
    });
  ...
  describe('Training Session Management with Real Database', () => {
    it('should create and manage training sessions with real database', async () => {
      ...
      await expect(dbManager.updateTrainingSession(sessionId, {
        status: 'running',
        progress_percent: 50,
    ...
    it('should handle complex training session updates', async () => {
      ...
      await expect(dbManager.updateTrainingSession(sessionId, complexUpdates)).resolves.not.toThrow();
