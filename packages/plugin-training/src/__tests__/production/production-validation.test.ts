...
const productionCharacter: Character = {
  ...
  messageExamples: [
    [
      ...
      { name: 'ProductionAgent', content: { text: 'Enabling custom reasoning with full production configuration.' } }
    ]
  ],
  postExamples: [],
  topics: ['production', 'training', 'validation', 'deployment'],
  adjectives: ['reliable', 'scalable', 'production-ready'],
...
describe('Production Validation Tests', () => {
  ...
  describe('Complete Training Data Collection Pipeline', () => {
    it('should collect training data from realistic conversation flows', async () => {
      ...
      elizaLogger.info(`✅ Collected ${collectedData.length} training data points from conversation`);
    });
  ...
  describe('Dataset Creation and Export Workflows', () => {
      ...
      for (const modelType of modelTypes) {
        for (let i = 0; i < samplesPerType; i++) {
          await dbManager.storeTrainingData({
            ...
            outputData: modelType === 'should_respond' 
              ? { decision: 'RESPOND', reasoning: `Production reasoning ${i + 1}`, confidence: 0.85 + (i * 0.001) }
              : modelType === 'planning'
              ? { thought: `Production thought ${i + 1}`, actions: ['REPLY'], confidence: 0.9 }
              : { code: `// Production code ${i + 1}\nfunction example() { return true; }`, explanation: `Production explanation ${i + 1}` },
            conversationContext: [],
            stateData: {},
            metadata: {
              agentId: runtime.agentId,
    ...
    it('should handle dataset export with filtering and quality controls', async () => {
      ...
      elizaLogger.info(`📊 Quality filtering: ${highQualityDataset.samples.length} high-quality, ${mediumQualityDataset.samples.length} medium+ quality samples`);
    });
  });
  ...
  describe('Custom Reasoning Integration', () => {
    it('should integrate custom reasoning models in production workflow', async () => {
      ...
      const agentResponse = responses.find(r => 
        r.entityId === runtime.agentId && 
        r.id !== shouldRespondMessage.id
      );
      ...
      elizaLogger.info(`📚 Collected ${collectedData.length} training samples from custom reasoning`);
    });
  });
  ...
  describe('Performance and Scale Testing', () => {
    it('should handle concurrent training workflows efficiently', async () => {
      ...
      elizaLogger.info(`⚡ Processed ${totalMessages} messages across ${concurrentRooms} concurrent rooms in ${totalTime}ms`);
      ...
      elizaLogger.info(`📊 Concurrent performance: ${avgTimePerMessage.toFixed(2)}ms average per message`);
      elizaLogger.info(`📈 Final database stats: ${stats.totalSamples} samples, avg confidence: ${stats.avgConfidence?.toFixed(3)}`);
    });
  });
  ...
  describe('Production Configuration Validation', () => {
    it('should validate all production configuration scenarios', async () => {
      ...
      const requiredSettings = [
        ...
        'CUSTOM_REASONING_COLLECT_TRAINING_DATA',
      ...
      for (const setting of requiredSettings) {
        ...
        elizaLogger.info(`✅ ${setting}: ${typeof value === 'string' ? value.substring(0, 50) : value}`);
      }
  ...
  describe('Error Recovery and Resilience', () => {
    it('should gracefully handle production error scenarios', async () => {
      ...
      elizaLogger.info(`✅ System recovery: ${finalStats.totalSamples} total samples after error testing`);
    });
  });
});
