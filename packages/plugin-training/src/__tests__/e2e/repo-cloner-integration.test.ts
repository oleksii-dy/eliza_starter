...
import { RepositoryCloner, type RepositoryInfo, type CloneProgress } from '../../training-generator/core/repo-cloner';
...
describe('RepositoryCloner Runtime Integration', () => {
  ...
  it('should handle clone failures gracefully with retries', async () => {
    ...
    await expect(cloner.cloneRepository(fakeRepoUrl, 'plugin', {
      retries: 2,
      timeout: 10000
    })).rejects.toThrow();
  ...
  it('should handle network errors and timeouts gracefully', async () => {
    ...
    await expect(cloner.cloneRepository(
  ...
  it('should extract repository names correctly', async () => {
    ...
    for (const testCase of testCases) {
      const info = await cloner.cloneRepository('https://github.com/octocat/Hello-World.git', 'plugin', { 
        depth: 1,
        branch: 'master', // octocat/Hello-World uses master
