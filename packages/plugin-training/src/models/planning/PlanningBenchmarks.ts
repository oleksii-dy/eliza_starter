...
export class PlanningBenchmarks {
  ...
  private getRequiredConsiderations(domain: string): string[] {
    ...
    return (
      considerationsMap[domain] || ['thorough_analysis', 'risk_assessment', 'resource_planning']
    );
  }
  ...
  private getRiskFactors(domain: string, complexity: string): string[] {
    ...
    return [...baseRisks, ...(domainRisks[domain] || []), ...(complexityRisks[complexity] || [])];
  }
  ...
  private getEvaluationCriteria(complexity: string): BenchmarkScenario['evaluationCriteria'] {
    ...
    const complexityAdjustments: Record<
      string,
      Partial<BenchmarkScenario['evaluationCriteria']>
    > = {
   ...
   */
  async evaluateResponse(benchmarkId: string, modelResponse: any): Promise<BenchmarkResult> {
  ...
  private extractPlanFromResponse(response: any): any {
    ...
    if (response.output?.plan) {
      return response.output.plan;
    }
    if (response.steps) {
  ...
  private evaluateCompleteness(plan: any, benchmark: BenchmarkScenario): number {
    ...
    if (plan.overview) {
      score += 0.2;
      checks.push('Has overview');
    }
    if (plan.steps && plan.steps.length > 0) {
      score += 0.3;
      checks.push('Has steps');
    }
    if (plan.success_criteria && plan.success_criteria.length > 0) {
      score += 0.2;
      checks.push('Has success criteria');
    }
    if (plan.risk_assessment) {
      score += 0.15;
      checks.push('Has risk assessment');
    }
    if (plan.contingencies && plan.contingencies.length > 0) {
      score += 0.15;
      checks.push('Has contingencies');
    }

    // Check step count is reasonable
    const stepCount = plan.steps?.length || 0;
    if (
      stepCount >= benchmark.expectedOutcomes.min_steps &&
      stepCount <= benchmark.expectedOutcomes.max_steps
    ) {
      score += 0.1;
  ...
  private evaluateFeasibility(plan: any, benchmark: BenchmarkScenario): number {
    ...
    if (plan.steps) {
      const hasRealisticTimelines = plan.steps.every(
        (step: any) => step.timeline && !step.timeline.includes('1 day') // Overly optimistic
      );
      if (hasRealisticTimelines) {
        score += 0.1;
      }
    }
    ...
    if (
      plan.steps &&
      plan.steps.some((step: any) => step.resources_needed || step.resource_requirements)
    ) {
      score += 0.1;
    }
  ...
  private evaluateResourceEfficiency(plan: any, benchmark: BenchmarkScenario): number {
    ...
    if (
      plan.steps &&
      plan.steps.some((step: any) => !step.dependencies || step.dependencies.length === 0)
    ) {
      score += 0.25;
    }
  ...
  private checkResourceAllocation(plan: any): boolean {
    ...
    const stepsWithResources = plan.steps.filter(
      (step: any) => step.resources_needed || step.resource_requirements
    ).length;
  ...
  private generateFeedback(
    ...
    if (scores.overall >= 0.9) {
      feedback.push(
        '🎉 Excellent planning performance! The plan is comprehensive and well-structured.'
      );
    ...
    if (scores.risk_awareness < 0.7) {
      feedback.push(
        'Risk awareness is insufficient. Consider more potential risks and mitigation strategies.'
      );
    }

    if (scores.feasibility < 0.7) {
      feedback.push(
        'Some aspects of the plan may not be feasible. Review timelines and resource requirements.'
      );
    }

    if (analysis.steps_generated < benchmark.expectedOutcomes.min_steps) {
      feedback.push(
        `Plan has too few steps (${analysis.steps_generated}). Consider breaking down into more detailed steps.`
      );
    }

    if (
      analysis.considerations_covered <
      benchmark.expectedOutcomes.required_considerations.length / 2
    ) {
      feedback.push(
        'Important considerations are missing from the plan. Review domain-specific requirements.'
      );
    }
  ...
  async runBenchmarkSuite(): Promise<{
    ...
    for (const [id, benchmark] of this.benchmarkScenarios) {
      ...
      if (!domainScores[benchmark.domain]) {
        domainScores[benchmark.domain] = [];
      }
      domainScores[benchmark.domain].push(result.scores.overall);

      // Track complexity performance
      if (!complexityScores[benchmark.complexity]) {
        complexityScores[benchmark.complexity] = [];
      }
    ...
    elizaLogger.info(
      `🎯 Benchmark suite complete. Average score: ${Math.round(avgOverallScore * 100)}%`
    );
