...
export class MessageHandlerIntegration {
  ...
  static registerHooks(runtime: IAgentRuntime): void {
    ...
    runtime.useModel = async function<T extends any, R = any>(
      ...
      const customReasoningEnabled = this.getSetting('CUSTOM_REASONING_ENABLED') === 'true';
      ...
      if (customReasoningEnabled && reasoningService) {
        try {
          ...
          if (modelType === ModelType.TEXT_LARGE && MessageHandlerIntegration.isCodingRequest(params)) {
            const codingEnabled = this.getSetting('CUSTOM_REASONING_CODING_ENABLED') === 'true';
            if (codingEnabled) {
    ...
    (runtime as any).customShouldRespond = async (
      ...
      const shouldRespondEnabled = runtime.getSetting('CUSTOM_REASONING_SHOULD_RESPOND_ENABLED') === 'true';
      ...
      try {
        return await ReasoningHooks.overrideShouldRespond(
          runtime,
          message,
          state,
    ...
    (runtime as any).customResponseGenerator = async (
      ...
      const customReasoningEnabled = runtime.getSetting('CUSTOM_REASONING_ENABLED') === 'true';
      const planningEnabled = runtime.getSetting('CUSTOM_REASONING_PLANNING_ENABLED') === 'true';
      ...
      try {
        ...
        return await ReasoningHooks.overridePlanning(
          runtime,
          message,
          state,
          actionNames,
  ...
  static isCustomReasoningEnabled(runtime: IAgentRuntime): boolean {
    const enabled = runtime.getSetting('CUSTOM_REASONING_ENABLED') === 'true';
  ...
  static getIntegrationStatus(runtime: IAgentRuntime): {
    ...
    return {
      ...
      shouldRespondOverride: runtime.getSetting('CUSTOM_REASONING_SHOULD_RESPOND_ENABLED') === 'true',
      planningOverride: runtime.getSetting('CUSTOM_REASONING_PLANNING_ENABLED') === 'true',
      codingOverride: runtime.getSetting('CUSTOM_REASONING_CODING_ENABLED') === 'true',
