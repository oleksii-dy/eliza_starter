...
export class PersonalityAnalyzer {
  ...
  async analyzePersonalities(
    ...
    for (const user of enhancedUsers) {
      ...
      user.relationships = await this.analyzeRelationships(user, userMessages, userConversations);
    }
  ...
  private analyzeCommunicationStyle(messages: DiscordMessage[]): string[] {
    ...
    if (shortMessages / totalMessages > 0.6) {
      styles.push('concise');
    }
    if (longMessages / totalMessages > 0.3) {
      styles.push('detailed');
    }
    if (questionCount / totalMessages > 0.4) {
      styles.push('inquisitive');
    }
    if (exclamationCount / totalMessages > 0.3) {
      styles.push('enthusiastic');
    }
    if (codeBlocks / totalMessages > 0.2) {
      styles.push('technical');
    }
    if (emojiCount / totalMessages > 0.4) {
  ...
  private assessLeadership(
    ...
    const conversationStarters = userConversations.filter(
      (conv) => conv.messages[0]?.author.id === messages[0]?.author.id
    ).length;
