...
export const trainingPlugin: Plugin = {
  ...
  description: 'Training data extraction, RLAIF training, and custom reasoning with fine-tuned models for ElizaOS agents',
  ...
  init: async (config, runtime) => {
    ...
    const missingTrainingSettings = trainingSettings.filter(setting => !runtime.getSetting(setting));
    
    if (missingTrainingSettings.length > 0) {
      elizaLogger.warn(`Training features missing settings: ${missingTrainingSettings.join(', ')}`);
      elizaLogger.warn('Traditional training features may be disabled without proper configuration');
    }

    // Validate custom reasoning settings
    const customReasoningEnabled = runtime.getSetting('CUSTOM_REASONING_ENABLED') === 'true';
    const togetherApiKey = runtime.getSetting('TOGETHER_AI_API_KEY');
    
    if (customReasoningEnabled) {
      if (!togetherApiKey) {
        elizaLogger.error('CUSTOM_REASONING_ENABLED=true but TOGETHER_AI_API_KEY is missing');
        ...
        const modelStates = [
          { name: 'ShouldRespond', enabled: runtime.getSetting('CUSTOM_REASONING_SHOULD_RESPOND_ENABLED') === 'true' },
          { name: 'Planning', enabled: runtime.getSetting('CUSTOM_REASONING_PLANNING_ENABLED') === 'true' },
          { name: 'Coding', enabled: runtime.getSetting('CUSTOM_REASONING_CODING_ENABLED') === 'true' },
        ];
        ...
        if (enabledModels.length > 0) {
          elizaLogger.info(`Enabled custom reasoning models: ${enabledModels.map(m => m.name).join(', ')}`);
        }
        if (disabledModels.length > 0) {
          elizaLogger.info(`Disabled custom reasoning models: ${disabledModels.map(m => m.name).join(', ')}`);
        }
        
        // Log cost management settings
        const budgetLimit = runtime.getSetting('CUSTOM_REASONING_BUDGET_LIMIT');
        const autoShutdown = runtime.getSetting('CUSTOM_REASONING_AUTO_SHUTDOWN_MINUTES');
      ...
      elizaLogger.info('Custom reasoning service disabled (CUSTOM_REASONING_ENABLED not set to true)');
    }
...
1. Configuration: Set CUSTOM_REASONING_ENABLED=true and TOGETHER_AI_API_KEY
2. Enable models: Set CUSTOM_REASONING_SHOULD_RESPOND_ENABLED=true (and planning/coding)
