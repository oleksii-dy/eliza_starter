...
async function createRealElizaRuntime(): Promise<AgentRuntime> {
    try {
        ...
        throw new Error(`Real runtime creation failed: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
export async function testMVPAgainstRealEliza(): Promise<{
    ...
    try {
        ...
        const testRoomId = `test-room-${Date.now()}` as `${string}-${string}-${string}-${string}-${string}`;
        ...
        const statusRoomId = `test-room-${Date.now()}` as `${string}-${string}-${string}-${string}-${string}`;
        ...
        const disableRoomId = `test-room-${Date.now()}` as `${string}-${string}-${string}-${string}-${string}`;
        ...
        try {
            ...
            results.push(`⚠️ Database test failed: ${dbError instanceof Error ? dbError.message : String(dbError)}`);
        }
