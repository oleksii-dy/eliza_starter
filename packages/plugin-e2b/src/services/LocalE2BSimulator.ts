...
export class LocalE2BSimulator {
  ...
  constructor(options: { useDocker?: boolean; dockerImage?: string } = {}) {
  ...
  async destroySandbox(sandboxId: string): Promise<void> {
    ...
    if (this.useDocker && sandbox.containerId) {
      try {
        await this.execCommand('docker', ['stop', sandbox.containerId]);
        await this.execCommand('docker', ['rm', sandbox.containerId]);
