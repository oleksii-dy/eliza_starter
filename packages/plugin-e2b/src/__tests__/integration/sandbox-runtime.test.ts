...
import { ImprovedE2BService } from '../../services/ImprovedE2BService.js';
...
describe.skip('Sandbox-Only Agent Runtime Tests', () => {
  ...
  let improvedE2bService: ImprovedE2BService;
  ...
  afterAll(async () => {
    ...
    // Cleanup not needed for mock runtime
  });
...
`, 'python');
  ...
  describe('Agent Message Processing in Sandboxes', () => {
    it('should process messages that require computation', async () => {
    ...
    it('should handle data visualization requests', async () => {
      const roomId = createUniqueUuid(runtime, 'test-room');
      const message: Memory = {
        id: createUniqueUuid(runtime, 'test-msg'),
        entityId: createUniqueUuid(runtime, 'test-user'),
    ...
    it('should process complex multi-step computations', async () => {
      const roomId = createUniqueUuid(runtime, 'test-room');
      const message: Memory = {
        id: createUniqueUuid(runtime, 'test-msg'),
        entityId: createUniqueUuid(runtime, 'test-user'),
...
`;
  ...
  describe('Performance and Metrics', () => {
    it('should provide execution metrics', async () => {
      const service = improvedE2bService;
  ...
  describe('Sandbox-First Agent Behavior', () => {
    it('should prefer sandbox execution for any computational request', async () => {
      const roomId = createUniqueUuid(runtime, 'test-room');
    ...
    it('should maintain computational context across messages', async () => {
      const roomId = createUniqueUuid(runtime, 'test-room');
      ...
      const setupResponses = await runtime.processMessage(setupMessage);
      ...
      const useResponses = await runtime.processMessage(useMessage);
      expect(useResponses.length).toBeGreaterThan(0);
