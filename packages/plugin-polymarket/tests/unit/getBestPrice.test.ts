...
import { getBestPriceAction, type BestPriceResponseData } from '../../src/actions/getBestPrice';
import { initializeClobClient } from '../../src/utils/clobClient';
import { callLLMWithTimeout } from '../../src/utils/llmHelpers';

type BestPriceContent = Content & { data: BestPriceResponseData };
...
describe('getBestPrice Action', () => {
  ...
  describe('handler', () => {
    it('should successfully get price using LLM extraction', async () => {
      ...
      expect((result.data as BestPriceResponseData).tokenId).toBe('123456');
      expect((result.data as BestPriceResponseData).side).toBe('buy');
    ...
    it('should fallback to regex extraction when LLM fails', async () => {
      ...
      expect(result).toBeDefined();
      expect((result.data as BestPriceResponseData).tokenId).toBe('789012');
      expect((result.data as BestPriceResponseData).side).toBe('sell');
    ...
    it('should format price correctly for different values', async () => {
      ...
      for (const testPrice of testPrices) {
        ...
        expect((result.data as BestPriceResponseData).formattedPrice).toBe(
          testPrice.expectedFormatted
        );
        expect((result.data as BestPriceResponseData).percentagePrice).toBe(
          testPrice.expectedPercentage
        );
    ...
    it('should include timestamp in response data', async () => {
      ...
      expect((result.data as BestPriceResponseData).timestamp).toBeDefined();
      expect(new Date((result.data as BestPriceResponseData).timestamp)).toBeInstanceOf(Date);
    });
    ...
    it('should handle very long token IDs', async () => {
      ...
      expect((result.data as BestPriceResponseData).tokenId).toBe(longTokenId);
  ...
  describe('action metadata', () => {
    ...
    it('should have usage examples', () => {
      expect(getBestPriceAction.examples).toBeDefined();
      if (getBestPriceAction.examples) {
        expect(getBestPriceAction.examples.length).toBeGreaterThan(0);
        expect(getBestPriceAction.examples[0]).toHaveProperty('prompt');
      }
    });
