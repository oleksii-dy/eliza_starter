...
export const getAccountAccessStatusAction: Action = {
    name: 'GET_ACCOUNT_ACCESS_STATUS',
    similes: [
    ...
    handler: async (
        ...
        try {
            ...
            logger.warn('[getAccountAccessStatusAction] LLM call failed or timed out, proceeding with action anyway:', error);
        }
        ...
        try {
            ...
            if (hasConfiguredManagedApiCredentials) {
                try {
                    ...
                    logger.error('[getAccountAccessStatusAction] Error fetching API keys with credentials:', err);
                ...
                apiKeysError = 'API credentials (CLOB_API_KEY, etc.) for listing managed keys are not configured in .env settings.';
            }
            ...
            const responseContent: Content = {
                text: responseText,
                actions: ['GET_ACCOUNT_ACCESS_STATUS'],
                data: {
                    certRequired,
                    managedApiKeys: apiKeysList, // Renamed for clarity
                    activeSessionKey: sessionApiKeyId ? { id: sessionApiKeyId, label: sessionApiLabel, source: sessionApiSource } : undefined,
                    error: apiKeysError,
    ...
    examples: [
        [
            ...
            { name: '{{user2}}', content: { text: "Okay, checking your Polymarket account access status...", actions: ['GET_ACCOUNT_ACCESS_STATUS'] } }
        ],
        [
            { name: '{{user1}}', content: { text: 'Do I need U.S. certification for Polymarket?' } },
            { name: '{{user2}}', content: { text: "Let me check your account's U.S. certification requirement...", actions: ['GET_ACCOUNT_ACCESS_STATUS'] } }
        ]
    ],
}; 