...
export const checkOrderScoringAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<boolean> => {
        ...
        if (!privateKey) {
            logger.warn('[checkOrderScoringAction] A private key (WALLET_PRIVATE_KEY, PRIVATE_KEY, or POLYMARKET_PRIVATE_KEY) is required.');
            return false;
        }
        if (!clobApiKey || !clobApiSecret || !clobApiPassphrase) {
            ...
            logger.warn(`[checkOrderScoringAction] Missing required API credentials for L2 authentication: ${missing.join(', ')}.`);
            return false;
        }
    ...
    handler: async (
        ...
        try {
            ...
            const orderIdRegex = /(?:order|ID|orders|IDs|check\s+scoring\s+for)[:\s#]?([0-9a-zA-Z_,\s\-_]+(?:0x[0-9a-fA-F]+)?)/gi;
            let matches;
            ...
            if (extractedIds.length > 0) {
                ...
                const errorContent: Content = { text: `❌ **Error**: ${errorMessage}`, actions: ['CHECK_ORDER_SCORING'], data: { error: errorMessage } };
                if (callback) await callback(errorContent);
                throw new Error(errorMessage);
            }
            logger.info(`[checkOrderScoringAction] Regex extracted Order IDs: ${JSON.stringify(llmResult.orderIds)}`);
        }

        const orderIdsToScore = llmResult.orderIds!;
        const apiParams: OfficialOrdersScoringParams = { orderIds: orderIdsToScore };

        logger.info(`[checkOrderScoringAction] Checking scoring for Order IDs: ${orderIdsToScore.join(', ')}`);
        ...
        try {
            ...
            const responseContent: Content = {
                ...
                data: { request: apiParams, response: scoringResponse, timestamp: new Date().toISOString() },
            };
            ...
            logger.error(`[checkOrderScoringAction] Error checking order scoring for IDs ${orderIdsToScore.join(', ')}:`, error);
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred.';
            const errorContent: Content = {
                text: `❌ **Error checking order scoring**: ${errorMessage}`,
                actions: ['CHECK_ORDER_SCORING'],
                data: { error: errorMessage, orderIds: orderIdsToScore, timestamp: new Date().toISOString() },
            };
            if (callback) await callback(errorContent);
            throw error;
        }
    ...
    examples: [
        [
            { name: '{{user1}}', content: { text: 'Are orders 123xyz and abc789 scoring?' } },
            { name: '{{user2}}', content: { text: "Checking scoring status for orders 123xyz and abc789.", actions: ['CHECK_ORDER_SCORING'] } }
        ],
        [
            { name: '{{user1}}', content: { text: 'Is my order 0xOrderMain scoring rewards?' } },
            { name: '{{user2}}', content: { text: "Let me check if your order 0xOrderMain is scoring rewards.", actions: ['CHECK_ORDER_SCORING'] } }
        ]
    ],
}; 