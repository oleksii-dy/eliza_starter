...
export const setupWebsocketAction: Action = {
    ...
    description: 'Establishes a WebSocket connection to Polymarket using the ws library and subscribes to specified market and/or user channels for real-time updates.',

    validate: async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<boolean> => {
        logger.info(`[setupWebsocketAction] Validate called.`);
        const clobWsUrl = runtime.getSetting('CLOB_WS_URL') || 'wss://ws-subscriptions-clob.polymarket.com/ws'; // Note: /ws path segment might be added later
        if (!clobWsUrl) {
            logger.warn('[setupWebsocketAction] CLOB_WS_URL is required in settings for WebSocket connections.');
            return false;
        }
    ...
    handler: async (
        ...
        if (existingClient && typeof existingClient.terminate === 'function') {
            logger.info('[setupWebsocketAction] Terminating existing WebSocket client before creating a new one.');
            try {
        ...
        try {
            ...
            if (extractedParams.error && (!extractedParams.markets || extractedParams.markets.length === 0) && !extractedParams.userId) {
                logger.warn(`[setupWebsocketAction] LLM indicated an issue and did not find parameters: ${extractedParams.error}. Triggering regex fallback.`);
            ...
            if (extractedMarkets || extractedUserId) {
                logger.info(`[setupWebsocketAction] Parameters extracted by LLM: markets=${JSON.stringify(extractedMarkets)}, userId=${extractedUserId}`);
            } else {
                logger.info('[setupWebsocketAction] LLM did not find any markets or userId, proceeding to regex fallback.');
        ...
        if ((!extractedMarkets || extractedMarkets.length === 0) && !extractedUserId) {
            const errorMsg = 'Please specify at least one market (condition ID) or a userId for user-specific updates. Neither LLM nor regex could extract them.';
            logger.warn(`[setupWebsocketAction] Validation Failure: ${errorMsg} Input was: "${message.content?.text}"`);
            if (callback) await callback({ text: `❌ ${errorMsg}` });
            throw new Error(errorMsg);
        }

        // Determine subscription type and construct WebSocket URL
        const baseWsUrl = runtime.getSetting('CLOB_WS_URL') || 'wss://ws-subscriptions-clob.polymarket.com/ws'; // Ensure /ws is part of base or added
        ...
        if (subscriptionType === 'user') {
            if (!extractedUserId) { // Should not happen due to prior checks, but as a safeguard
            ...
            if (!apiKey || !apiSecret || !apiPassphrase) {
                const errMsg = 'API Key, Secret, and Passphrase are required in settings for user channel WebSocket subscriptions.';
    ...
    examples: [
        [
            { name: '{{user1}}', content: { text: 'Connect to Polymarket WebSocket and subscribe to market 0xMarket123.' } },
            { name: '{{user2}}', content: { text: "Establishing WebSocket connection and subscribing to market 0xMarket123...", actions: ['SETUP_WEBSOCKET', 'HANDLE_REALTIME_UPDATES'] } }
        ],
        [
            { name: '{{user1}}', content: { text: 'Listen to my order updates on Polymarket via WebSocket.', data: { userId: "0xUserAddress" } } }, // Assuming LLM extracts or user provides their address
            { name: '{{user2}}', content: { text: "Connecting to WebSocket and subscribing to your user-specific order updates...", actions: ['SETUP_WEBSOCKET', 'HANDLE_REALTIME_UPDATES'] } }
        ],
        [
            { name: '{{user1}}', content: { text: 'Subscribe to trades for markets 0xMarketA, 0xMarketB and my user ID 0xMyUser.' } },
            { name: '{{user2}}', content: { text: "Connecting and subscribing to markets 0xMarketA, 0xMarketB and user 0xMyUser...", actions: ['SETUP_WEBSOCKET', 'HANDLE_REALTIME_UPDATES'] } }
        ]
    ],
}; 