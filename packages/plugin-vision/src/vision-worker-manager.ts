...
import type { VisionConfig, Florence2Result, OCRResult, ScreenCapture, EnhancedSceneDescription } from './types';
...
export class VisionWorkerManager {
  ...
  private updateOCRCache(msg: any): void {
  ...
  private readOCRResult(): OCRResult | null {
    try {
      ...
      const frameId = this.ocrResultsView.getUint32(offset + 4, true);
      const timestamp = this.ocrResultsView.getFloat64(offset + 8, true);
  ...
  getLatestScreenCapture(): ScreenCapture | null {
    ...
    try {
      ...
      const displayIndex = Atomics.load(this.screenAtomicState, this.DISPLAY_INDEX);
  ...
  getLatestEnhancedScene(): EnhancedSceneDescription {
    ...
    const tiles = this.generateTiles(screenCapture?.width || 0, screenCapture?.height || 0);
