...
export const AgentKitAdmin: React.FC = () => {
  ...
  const executeAction = async (actionName: string) => {
    try {
      ...
      setActions((prev) =>
        prev.map((a) =>
          a.name === actionName ? { ...a, lastExecuted: new Date(), status: 'success' as const } : a
        )
      );
    } catch (err: any) {
      setError(err.message);
      setActions((prev) =>
        prev.map((a) => (a.name === actionName ? { ...a, status: 'error' as const } : a))
      );
    }
  };
      ...
      {walletInfo ? (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <code className="text-sm bg-gray-100 px-2 py-1 rounded">{walletInfo.address}</code>
          </div>
          <div className="flex items-center justify-between">
        ...
        <div className="space-y-3">
          {actions.map((action) => (
            ...
            >
              ...
              {action.status === 'error' && (
                <p className="text-red-500 text-sm mt-2">Last execution failed</p>
              )}
            </div>
          ))}
        </div>
