...
describe('Custodial Wallet Actions', () => {
  ...
  beforeEach(() => {
    ...
    mockCustodialWalletService = {
      createWallet: mock(),
      getWallet: mock(),
      getWalletsForEntity: mock(),
      hasPermission: mock(),
      executeTransaction: mock(),
      transferOwnership: mock(),
      addController: mock(),
      removeController: mock(),
    };

    mockTrustService = {
      evaluate: mock().mockResolvedValue({ score: 0.8 }),
      validateFinancialOperation: mock().mockResolvedValue(true),
    };
    ...
    mockRuntime = {
      ...
      getService: mock((name: string) => {
        if (name === 'custodial-wallet') {
          return mockCustodialWalletService;
        }
        if (name === 'trust') {
  ...
  describe('createCustodialWalletAction', () => {
    ...
    it('should create wallet successfully', async () => {
      ...
      const _result = await createCustodialWalletAction.handler(
        mockRuntime,
        createMessage,
        mockState,
        {},
  ...
  describe('listCustodialWalletsAction', () => {
    it('should list wallets for entity', async () => {
      ...
      mockCustodialWalletService.getWalletsForRoom = mock().mockResolvedValue([]);
  ...
  describe('addWalletControllerAction', () => {
    it('should add controller successfully', async () => {
      ...
      await addWalletControllerAction.handler(mockRuntime, addMessage, mockState, {}, mockCallback);
