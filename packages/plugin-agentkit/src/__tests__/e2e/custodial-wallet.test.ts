...
export class CustodialWalletTestSuite implements TestSuite {
    ...
    description = "Comprehensive tests for custodial wallet service and actions with trust integration";
    ...
    tests = [
...

        {
            ...
            fn: async (runtime: IAgentRuntime) => {
                ...
                const ownerCanView = await custodialService.hasPermission(wallet.id, "owner-123" as any, 'view');
                const ownerCanTransfer = await custodialService.hasPermission(wallet.id, "owner-123" as any, 'transfer');
                const ownerCanAdmin = await custodialService.hasPermission(wallet.id, "owner-123" as any, 'admin');
                ...
                const strangerCanView = await custodialService.hasPermission(wallet.id, "stranger-456" as any, 'view');
                const strangerCanTransfer = await custodialService.hasPermission(wallet.id, "stranger-456" as any, 'transfer');
                const strangerCanAdmin = await custodialService.hasPermission(wallet.id, "stranger-456" as any, 'admin');
...

        {
            ...
            fn: async (runtime: IAgentRuntime) => {
                ...
                const originalOwnerId = wallet.ownerId;
...

        {
            ...
            fn: async (runtime: IAgentRuntime) => {
                ...
                if (!callbackContent || !callbackContent.text.includes("Created custodial wallet successfully")) {
