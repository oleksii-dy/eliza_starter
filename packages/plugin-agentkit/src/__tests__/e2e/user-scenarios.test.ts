...
export class AgentKitUserScenariosTestSuite implements TestSuite {
    ...
    description = "Comprehensive user request scenarios for AgentKit - explicit commands and implicit inference";
            ...
            fn: async (runtime: IAgentRuntime) => {
                ...
                for (const request of explicitRequests) {
                    const foundActions = runtime.actions.filter(action => 
                        request.expectedActions.some(expected => 
                            action.name.includes(expected) || action.similes?.includes(expected)
                        )
                    );

                    if (foundActions.length > 0) {
                        console.log(`✓ Explicit request "${request.text}" -> Found ${foundActions.length} matching actions`);
                        ...
                        try {
                            const isValid = await foundActions[0].validate(runtime, testMessage as any, {} as any);
                            if (isValid) {
                                validatedRequests++;
                                console.log(`  ✓ Action ${foundActions[0].name} validates successfully`);
                            }
                        } catch (error) {
                            console.warn(`  ⚠️ Validation error for ${foundActions[0].name}:`, error instanceof Error ? error.message : String(error));
                        }
                    } else {
                ...
                console.log(`✓ Explicit requests test: ${validatedRequests}/${explicitRequests.length} requests validated`);
            },
        },
        {
            ...
            fn: async (runtime: IAgentRuntime) => {
                ...
                for (const scenario of implicitScenarios) {
                    ...
                    const relevantActions = runtime.actions.filter(action => 
                        scenario.expectedCategories.some(category => 
                    ...
                    if (relevantActions.length > 0) {
                        ...
                        console.log(`  ✓ Found ${relevantActions.length} relevant actions:`, 
                            relevantActions.slice(0, 3).map(a => a.name).join(", "));
                        ...
                        for (const action of relevantActions.slice(0, 2)) { // Test top 2 actions
                            try {
                ...
                console.log(`✓ Implicit inference test: ${inferenceMatches}/${implicitScenarios.length} scenarios had relevant actions`);
            },
        },
        {
                ...
                for (const journey of complexJourneys) {
                    ...
                    for (let index = 0; index < journey.steps.length; index++) {
                        ...
                        const matchingActions = runtime.actions.filter(action => {
                            ...
                            const keywords = [
                                'balance', 'wrap', 'supply', 'bridge', 'trade', 'swap', 
                                'price', 'tweet', 'post', 'mint', 'nft', 'list', 'compound'
                            ];
                            
                            return keywords.some(keyword => 
                                stepLower.includes(keyword) && actionName.includes(keyword)
                            );
                        });

                        if (matchingActions.length > 0) {
                            stepProgress++;
                            console.log(`    ✓ Found actions: ${matchingActions.slice(0, 2).map(a => a.name).join(", ")}`);
                        } else {
                    ...
                    console.log(`  Journey completeness: ${stepProgress}/${journey.steps.length} steps (${completeness.toFixed(1)}%)`);
                }
            },
        },
        {
            ...
            fn: async (runtime: IAgentRuntime) => {
                ...
                for (const edgeCase of edgeCases) {
                    ...
                    if (validatingActions === 0) {
                        ...
                        console.log(`  ✓ Few actions validated: ${validatingActions} (reasonable for edge case)`);
                    } else {
                        console.log(`  ⚠️ Many actions validated: ${validatingActions} (might be too permissive)`);
                    }
                }
            },
        },
        {
            ...
            fn: async (runtime: IAgentRuntime) => {
                ...
                const intentTests = [
                    ...
                    { text: "Create something special for this moment", intent: "nft_mint", confidence: "medium" },
                ...
                for (const test of intentTests) {
                    ...
                    const matchingActions = runtime.actions.filter(action =>
                        expectedActions.some(expected =>
                            action.name.includes(expected)
                        )
                    );
                    ...
                    if (matchingActions.length > 0) {
                        ...
                        try {
                            const isValid = await matchingActions[0].validate(runtime, testMessage as any, {} as any);
                            if (isValid) {
                                correctClassifications++;
                                console.log(`✓ Intent "${test.intent}" correctly identified for: "${test.text}"`);
                            } else {
                                console.log(`⚠️ Intent "${test.intent}" identified but validation failed: "${test.text}"`);
                            }
                        } catch (error) {
                ...
                console.log(`✓ Intent classification accuracy: ${correctClassifications}/${intentTests.length} (${accuracy.toFixed(1)}%)`);
