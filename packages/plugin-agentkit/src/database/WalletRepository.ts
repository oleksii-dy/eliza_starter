...
export class WalletRepository {
    ...
    async initialize(): Promise<void> {
        try {
            ...
            await db.run(`CREATE INDEX IF NOT EXISTS idx_wallets_owner ON custodial_wallets(owner_id)`);
            await db.run(`CREATE INDEX IF NOT EXISTS idx_wallets_entity ON custodial_wallets(entity_id)`);
            await db.run(`CREATE INDEX IF NOT EXISTS idx_wallets_room ON custodial_wallets(room_id)`);
            await db.run(`CREATE INDEX IF NOT EXISTS idx_wallets_world ON custodial_wallets(world_id)`);
            await db.run(`CREATE INDEX IF NOT EXISTS idx_permissions_wallet ON wallet_permissions(wallet_id)`);
            await db.run(`CREATE INDEX IF NOT EXISTS idx_permissions_entity ON wallet_permissions(entity_id)`);
            await db.run(`CREATE INDEX IF NOT EXISTS idx_transactions_wallet ON wallet_transactions(wallet_id)`);
            await db.run(`CREATE INDEX IF NOT EXISTS idx_transactions_status ON wallet_transactions(status)`);
    ...
    private getDatabase(): any {
        ...
        const db = (this.runtime as any).db || (this.runtime as any).databaseAdapter?.db;
        if (!db) {
            throw new Error("[WalletRepository] Database adapter not available - ensure SQL plugin is loaded");
        }
        return db;
    }
    ...
    async getWallet(walletId: UUID): Promise<CustodialWallet | null> {
        ...
        const row = await db.get(
            `SELECT * FROM custodial_wallets WHERE id = ?`,
            [walletId]
        );
    ...
    async getWalletsForEntity(entityId: UUID): Promise<CustodialWallet[]> {
        ...
        for (const row of rows) {
            const permissions = await this.getWalletPermissions(row.id);
    ...
    async getWalletsForRoom(roomId: UUID): Promise<CustodialWallet[]> {
        ...
        const rows = await db.all(
            `SELECT * FROM custodial_wallets WHERE room_id = ?`,
            [roomId]
        );
    ...
    async getWalletsForWorld(worldId: UUID): Promise<CustodialWallet[]> {
        ...
        const rows = await db.all(
            `SELECT * FROM custodial_wallets WHERE world_id = ?`,
            [worldId]
        );
    ...
    async getAllWallets(): Promise<CustodialWallet[]> {
        ...
        const rows = await db.all(`SELECT * FROM custodial_wallets`);
    ...
    async updateWalletStatus(walletId: UUID, status: string): Promise<void> {
        ...
        await db.run(
            `UPDATE custodial_wallets SET status = ? WHERE id = ?`,
            [status, walletId]
        );
    }
    ...
    async updateWalletLastUsed(walletId: UUID): Promise<void> {
        ...
        await db.run(
            `UPDATE custodial_wallets SET last_used_at = ? WHERE id = ?`,
            [Date.now(), walletId]
        );
    }
    ...
    async getWalletPermissions(walletId: UUID): Promise<WalletPermission[]> {
        ...
        const rows = await db.all(
            `SELECT * FROM wallet_permissions WHERE wallet_id = ?`,
            [walletId]
        );
     ...
     */
    async getWalletTransactions(walletId: UUID, limit = 50, offset = 0): Promise<WalletTransaction[]> {
        const db = this.getDatabase();
     ...
     */
    private mapRowToWallet(row: any, permissions: WalletPermission[]): CustodialWallet {
        return {
            id: row.id as UUID,
            address: row.address,
            network: row.network,
