...
export class WalletRepository {
  ...
  async getWallet(walletId: UUID): Promise<CustodialWallet | null> {
    ...
    const row = await db.get('SELECT * FROM custodial_wallets WHERE id = ?', [walletId]);
  ...
  async getWalletsForRoom(roomId: UUID): Promise<CustodialWallet[]> {
    ...
    const rows = await db.all('SELECT * FROM custodial_wallets WHERE room_id = ?', [roomId]);
  ...
  async getWalletsForWorld(worldId: UUID): Promise<CustodialWallet[]> {
    ...
    const rows = await db.all('SELECT * FROM custodial_wallets WHERE world_id = ?', [worldId]);
  ...
  async getWalletPermissions(walletId: UUID): Promise<WalletPermission[]> {
    ...
    const rows = await db.all('SELECT * FROM wallet_permissions WHERE wallet_id = ?', [walletId]);
