...
export class AgentKitTrustValidator {
  ...
  static async validateCustodialWalletAccess(
    ...
    try {
      ...
      logger.warn(
        '[AgentKitTrustValidator] Trust service unavailable - requiring minimum trust level'
      );
      return {
        allowed: false,
  ...
  static getTrustRequirement(actionName: string, amount?: number, tokenType?: string): number {
    ...
    let baseRequirement =
      FINANCIAL_TRUST_REQUIREMENTS[actionName as keyof typeof FINANCIAL_TRUST_REQUIREMENTS] || 50;
...
export function wrapAgentKitActionsWithTrust(actions: Action[]): Action[] {
  return actions.map((action) => {
    ...
    logger.info(
      `[AgentKitTrust] Wrapping action ${action.name} with trust requirement ${trustRequirement}`
    );
      ...
      validate: async (runtime: IAgentRuntime, message: Memory, state?: State) => {
        ...
        if (action.name.includes('CUSTODIAL_WALLET') || action.name.includes('WALLET')) {
          ...
          if (walletMatch) {
            ...
            const operation = action.name.includes('TRANSFER_OWNERSHIP')
              ? 'admin'
              : action.name.includes('ADD_CONTROLLER')
                ? 'admin'
            ...
            if (!validation.allowed) {
              logger.warn(
                `[CustodialWalletTrust] Access denied for ${action.name}: ${validation.reason}`
              );
              return false;
            }
          }
...
function isFinancialAction(actionName: string): boolean {
  ...
  return financialKeywords.some((keyword) => actionName.toLowerCase().includes(keyword));
}
...
export class HighValueTransactionValidator {
  ...
  private static async getApproximateUSDValue(
    amount: number,
    tokenSymbol: string
  ): Promise<number> {
