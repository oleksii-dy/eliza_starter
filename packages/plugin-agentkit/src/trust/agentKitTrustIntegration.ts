...
export const AGENTKIT_TRUST_REQUIREMENTS = {
  ...
  EXPORT_CUSTODIAL_WALLET: {
    minTrustScore: 0.95,
    requiredRole: 'ADMIN_ROLE',
    requiresCoordination: true,
  },
  LIST_CUSTODIAL_ADDRESSES: {
    minTrustScore: 0.7,
    requiredRole: 'FINANCE_ROLE',
    requiresCoordination: false,
  },
...
export async function validateAgentKitTrust(
  ...
  try {
    ...
    try {
      ...
      elizaLogger.warn(
        `[AgentKitTrustValidator] Trust score evaluation failed: ${error instanceof Error ? error.message : String(error)}`
      );
    ...
    try {
      ...
      elizaLogger.warn(
        `[AgentKitTrustValidator] Role check failed: ${error instanceof Error ? error.message : String(error)}`
      );
...
async function validateMultiAgentCoordination(
  ...
  try {
    ...
    if (coordinationContext.otherAgents) {
      for (const agentId of coordinationContext.otherAgents) {
        ...
        try {
          ...
          elizaLogger.warn(
            `[AgentKitTrustValidator] Failed to get trust score for agent ${agentId}: ${error instanceof Error ? error.message : String(error)}`
          );
        }
    ...
    return {
      allowed: false,
      reason: `Coordination validation error: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
 ...
 */
export function extractCoordinationContext(
  message: Memory,
  _state?: State
):
  | {
  ...
  return {
    ...
    handler: async (
      ...
      try {
        ...
        await recordAgentKitAction(runtime, message, action.name, true, {
          result,
          coordinationContext,
        });
...
export function wrapAgentKitActionsWithTrust(actions: Action[]): Action[] {
  return actions.map((action) => {
    ...
    let requirements: {
      ...
      requiredRole: string;
      requiresCoordination: boolean;
    } = AGENTKIT_TRUST_REQUIREMENTS.GET_BALANCE; // Default to lowest requirement

    for (const [actionType, reqs] of Object.entries(AGENTKIT_TRUST_REQUIREMENTS)) {
      if (
        action.name.toUpperCase().includes(actionType) ||
...
export async function validateCustodialWalletSecurity(
  ...
  try {
    ...
    return {
      allowed: false,
      reason: `Wallet security validation error: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
