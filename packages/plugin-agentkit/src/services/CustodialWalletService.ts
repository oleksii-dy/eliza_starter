import { Service, type IAgentRuntime, type UUID, elizaLogger, ServiceType, type ServiceTypeName } from "../types/core.d";
...
export class CustodialWalletService extends Service {
    ...
    static serviceName = "custodial-wallet" as ServiceTypeName;
    ...
    async initialize(): Promise<void> {
        try {
            ...
            this.defaultNetwork = (this.runtime!.getSetting("CDP_NETWORK_ID") as string) || "base-sepolia";
            ...
            const encryptionPassphrase = this.runtime!.getSetting("WALLET_ENCRYPTION_PASSPHRASE") as string;
    ...
    async getWalletsForEntity(entityId: UUID): Promise<CustodialWallet[]> {
        try {
            ...
            elizaLogger.error(`[CustodialWalletService] Failed to get wallets for entity ${entityId}:`, error);
            return [];
        }
    }
    ...
    async getWalletsForRoom(roomId: UUID): Promise<CustodialWallet[]> {
        try {
            ...
            elizaLogger.error(`[CustodialWalletService] Failed to get wallets for room ${roomId}:`, error);
            return [];
        }
    }
    ...
    async getWalletsForWorld(worldId: UUID): Promise<CustodialWallet[]> {
        try {
            ...
            elizaLogger.error(`[CustodialWalletService] Failed to get wallets for world ${worldId}:`, error);
            return [];
        }
    }
     ...
     */
    async hasPermission(walletId: UUID, entityId: UUID, operation: 'view' | 'transfer' | 'admin'): Promise<boolean> {
        try {
        ...
        try {
            ...
            try {
                ...
                if (request.tokenAddress) {
                    ...
                    const transferAction = actions.find(a => a.name === 'transfer' || a.name === 'erc20_transfer');
    ...
    async addPermission(walletId: UUID, permission: WalletPermission): Promise<void> {
        try {
            ...
            elizaLogger.info(`[CustodialWalletService] Added ${permission.type} permission for ${permission.entityId} on wallet ${walletId}`);
        } catch (error) {
    ...
    async removePermission(walletId: UUID, entityId: UUID, type: string): Promise<void> {
        try {
            ...
            elizaLogger.info(`[CustodialWalletService] Removed ${type} permission for ${entityId} on wallet ${walletId}`);
        } catch (error) {
     ...
     */
    async getTransactionHistory(walletId: UUID, limit = 50, offset = 0): Promise<WalletTransaction[]> {
        try {
            return await this.repository.getWalletTransactions(walletId, limit, offset);
        } catch (error) {
            elizaLogger.error(`[CustodialWalletService] Failed to get transaction history for wallet ${walletId}:`, error);
            return [];
        }
    }
    ...
    async addController(walletId: UUID, controllerId: UUID, requestedBy: UUID): Promise<void> {
        try {
            ...
            elizaLogger.info(`[CustodialWalletService] Added controller ${controllerId} to wallet ${walletId}`);
        } catch (error) {
    ...
    async removeController(walletId: UUID, controllerId: UUID, requestedBy: UUID): Promise<void> {
        try {
            ...
            elizaLogger.info(`[CustodialWalletService] Removed controller ${controllerId} from wallet ${walletId}`);
        } catch (error) {
    ...
    async transferOwnership(walletId: UUID, newOwnerId: UUID, requestedBy: UUID): Promise<void> {
        try {
            ...
            elizaLogger.info(`[CustodialWalletService] Transferred ownership of wallet ${walletId} from ${wallet.ownerId} to ${newOwnerId}`);
        } catch (error) {
