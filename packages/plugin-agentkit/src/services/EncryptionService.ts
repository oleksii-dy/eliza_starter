...
export class EncryptionService {
    ...
    decrypt(encryptedData: string, keyHex?: string): string {
        try {
            ...
            if (combined.length < EncryptionService.SALT_LENGTH + EncryptionService.IV_LENGTH + EncryptionService.TAG_LENGTH) {
                throw new Error("Invalid encrypted data format");
            }
            
            const salt = combined.subarray(0, EncryptionService.SALT_LENGTH);
            const iv = combined.subarray(EncryptionService.SALT_LENGTH, EncryptionService.SALT_LENGTH + EncryptionService.IV_LENGTH);
            ...
            const encrypted = combined.subarray(EncryptionService.SALT_LENGTH + EncryptionService.IV_LENGTH + EncryptionService.TAG_LENGTH);
