...
export const getBlockNumberAction: Action = {
  ...
  examples: [
    ...
    "Get the latest block number on Polygon network",
    "How many blocks have been mined on Polygon so far?"
  ],
...
export const getBlockDetailsAction: Action = {
  ...
  validate: async (options: any, runtime: IAgentRuntime) => {
    try {
      ...
      if (envBlockIdentifier) {
        ...
        // Try as hash
        if (typeof envBlockIdentifier === 'string' && 
  ...
  handler: async (runtime: IAgentRuntime, message: any, state: any, options: any) => {
    ...
    logger.info(`Successfully retrieved details for block ${blockDetails.number} (hash: ${blockDetails.hash})`);
    logger.debug(`Block timestamp: ${new Date(Number(blockDetails.timestamp) * 1000).toISOString()}`);
    logger.debug(`Block gas used: ${blockDetails.gasUsed.toString()} / ${blockDetails.gasLimit.toString()}`);
...
const blockOptionsSchema = z.object({
  ...
  blockHash: z.string().regex(/^0x[a-fA-F0-9]{64}$/).optional()
...
export const getBlockInfoAction: Action = {
  ...
  execute: async (options: any, runtime: IAgentRuntime) => {
    try {
      ...
      const baseFeePerGas = blockInfo.baseFeePerGas ? formatUnits(blockInfo.baseFeePerGas, 9) : 'N/A';
      
      logger.info(`Block number: ${blockInfo.number}`);
      logger.info(`Block timestamp: ${timestamp}`);
      logger.info(`Gas used: ${gasUsed} / ${gasLimit} (${(Number(blockInfo.gasUsed) * 100 / Number(blockInfo.gasLimit)).toFixed(2)}%)`);
