...
import { validateFereProConfig } from "../enviroment";
...
const fereProTemplate = `Extract the core query from user input and respond with the requested data. If the user asks for a comparison or historical data, make sure to reflect that accurately.
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ...
        await validateFereProConfig(runtime);
        return true;
    },
    ...
    handler: async (
        ...
        const context = {
            ...
            stream: runtime.getSetting("FERE_STREAM") || process.env.FERE_STREAM,
            agent: "ProAgent",
        };
        ...
        try {
            ...
            const response = await service.sendMessage(context, callback);
            ...
            if (response.success) {
                ...
                throw new Error(response.error || "WebSocket error");
            }
            return true;
        } catch (error) {
