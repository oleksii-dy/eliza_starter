...
export class BorrowAction {
    ...
    async borrow(params: BorrowContent, callback: HandlerCallback): Promise<any> {
            ...
            if (borrower.data?.type != 'active') {
                ...
                //const txHashSupply = externalSupplyMessage.hash().toString('hex');
                //const explorerSupplyUrl = `https://testnet.tonscan.org/tx/${txHashSupply}`;
...
export const borrowAction: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            const borrowResult = await action.borrow(borrowDetails, callback);
