...
import {
  ActionChainService,
  type ActionChainWorkflow,
  type ActionChainResult,
  type ActionExecution,
} from '../services/action-chain-service';
...
export const runWorkflowAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      let workflowResult: ActionChainResult | undefined;
...
function generateWorkflowResponse(workflowId: string, result: ActionChainResult): string {
  if (!result.success) {
    return (
  ...
  const duration =
    result.results.length > 0
      ? result.results.reduce((sum: number, r: ActionExecution) => sum + r.duration, 0)
      : 0;
  ...
  if (result.results.length > 0) {
    ...
    result.results.forEach((step: ActionExecution, index: number) => {
