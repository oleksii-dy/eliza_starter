...
import type { GenerationScriptMetadata, EnvVarConfig } from '../types';
import { validateEnvVar } from '../validation';

interface ShellService {
  executeCommand: (cmd: string) => Promise<{ exitCode: number; error?: string; output?: string }>;
}
      ...
      for (const { plugin, varName, config } of generatableVars) {
        try {
          ...
          if (script.dependencies.length > 0) {
            ...
            const installResult = await (shellService as unknown as ShellService).executeCommand(
              installCommand
            );
          ...
          // Execute the script
          const executeResult = await (shellService as unknown as ShellService).executeCommand(
            `node ${scriptPath}`
          );
