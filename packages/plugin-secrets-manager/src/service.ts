import { elizaLogger as logger, Service, type IAgentRuntime } from '@elizaos/core';
...
export class EnvManagerService extends Service {
  ...
  private async scanCharacterSecrets(
    secrets: Record<string, string | number | boolean | null | undefined>
  ): Promise<void> {
    ...
    for (const pluginInstance of this.runtime.plugins) {
      ...
      const declared = (
        pluginInstance as unknown as {
          declaredEnvVars?: Record<string, Partial<EnvVarConfig & { defaultValue?: string }>>;
        }
      ).declaredEnvVars;
