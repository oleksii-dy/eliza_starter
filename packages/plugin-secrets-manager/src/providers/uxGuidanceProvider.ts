...
import type { SecretContext, SecretMetadata, EnvVarConfig, SecretConfig } from '../types';
...
function generateStatusSummary(
  userSecrets: SecretMetadata,
  missingSecrets: Array<{ plugin: string; varName: string; config: EnvVarConfig }>
): StatusSummary {
  ...
  Object.values(userSecrets).forEach((secret: SecretConfig) => {
