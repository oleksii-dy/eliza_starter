  ...
  tests = [
...

    {
      ...
      fn: async (runtime?: any) => {
        ...
        try {
          const expectedProviders = ['ENV_STATUS', 'SECRETS_INFO', 'UX_GUIDANCE', 'SETTINGS'];
      ...
      fn: async (runtime?: any) => {
        ...
        try {
          ...
          if (retrievedValue !== testValue) {
            throw new Error(
              `Secret value mismatch. Expected: ${testValue}, Got: ${retrievedValue}`
            );
          }
...

    {
      ...
      fn: async (runtime?: any) => {
        ...
        try {
          const requestFormAction = testRuntime.actions.find(
            (a: any) => a.name === 'REQUEST_SECRET_FORM'
          );
          ...
          if (!isValid) {
            ...
            console.log(
              '⚠️ REQUEST_SECRET_FORM validation failed (expected if ngrok not available)'
            );
          } else {
      ...
      fn: async (runtime?: any) => {
        ...
        try {
          ...
          if (retrievedData !== sensitiveData) {
            throw new Error(
              `Encryption/decryption failed. Expected: ${sensitiveData}, Got: ${retrievedData}`
            );
          }
