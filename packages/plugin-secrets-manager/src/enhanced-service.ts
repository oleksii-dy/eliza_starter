...
export class EnhancedSecretManager extends EnvManagerService {
  ...
  private async loadWorldSecrets(): Promise<void> {
    try {
      // Get all worlds for this agent using runtime method
      const worlds = await this.runtime.getWorlds({});
  ...
  async set(
    ...
    try {
      ...
      // Validate value if config specifies and value is not null
      if (value && (config?.validationMethod || config?.type)) {
