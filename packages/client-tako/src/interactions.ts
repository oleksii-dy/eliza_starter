...
export class TakoInteractionClient {
    ...
    async handleTakoInteractions(type: "poll" | "chat") {
        ...
        try {
            ...
            if (type === "poll") {
                const followingFeed = await this.client.fetchFeed(20);
                const targetUsersFeed = await this.client.fetchFeed(20, "fid");
  ...
  Text: ${cast.metadata.content}`;
        ...
        const shouldRespond = this.client.takoConfig.TAKO_TARGET_USERS.includes(
            cast.author.fid
        )
            ? "RESPOND"
