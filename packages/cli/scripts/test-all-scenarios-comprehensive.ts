...
class ComprehensiveScenarioRunner {
  ...
  private async testGitHubTodoWorkflow(): Promise<boolean> {
    try {
      ...
      const { githubTodoWorkflowScenario } = await import(
        '../scenarios/plugin-tests/02-github-todo-workflow.js'
      );
      ...
      const convertedScenario = {
        ...
        characters:
          githubTodoWorkflowScenario.actors?.map((actor) => ({
            id: actor.id,
            name: actor.name,
            bio: actor.script?.personality || 'GitHub Todo workflow agent',
            system:
              'You help manage GitHub issues and todo tasks. You can integrate GitHub and todo management effectively.',
          ...
          steps: [
            {
              type: 'message',
              from: 'user',
              content: 'Can you help with GitHub issues and todo management?',
            },
  ...
  private async testProductionPluginConfig(): Promise<boolean> {
    try {
      ...
      const { productionPluginConfigurationScenario } = await import(
        '../scenarios/plugin-configuration-production-test.js'
      );
      ...
      const convertedScenario = {
        ...
        characters:
          productionPluginConfigurationScenario.actors?.map((actor) => ({
            id: actor.id,
            name: actor.name,
          ...
          steps: [
            {
              type: 'message',
              from: 'user',
              content: 'Initialize production plugin configuration',
            },
  ...
  private async testAutocoderFunctionality(): Promise<boolean> {
    try {
        ...
        characters: [
          {
            ...
            system:
              'You are an autocoder agent that helps create ElizaOS plugins. You assist with code generation and plugin development.',
      ...
      const result = await executeRealScenario(autocoderScenario, {
        timeout: 120000,
        maxSteps: 10,
      });
