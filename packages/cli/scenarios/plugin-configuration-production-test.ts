...
class ProductionDatabaseService extends Service {
  ...
  static async start(runtime: IAgentRuntime): Promise<ProductionDatabaseService> {
    ...
    if (!dbUrl) {
      throw new Error('DATABASE_URL environment variable is required for ProductionDatabaseService');
    }
    
    if (!apiKey) {
      throw new Error('DATABASE_API_KEY environment variable is required for ProductionDatabaseService');
    }
  ...
  private async initialize(): Promise<void> {
    ...
    this.isStarted = true;
    console.log('ProductionDatabaseService: Successfully connected with', this.connections.size, 'connection pools');
  }
  ...
  async query(sql: string, params: any[] = []): Promise<any[]> {
    ...
    const connection = Math.random() < 0.7 
    ...
    console.log(`ProductionDatabaseService: Executing query on ${connection.pool}: ${sql.substring(0, 50)}...`);
  ...
  async execute(sql: string, params: any[] = []): Promise<{ affectedRows: number }> {
    ...
    console.log(`ProductionDatabaseService: Executing command on ${connection.pool}: ${sql.substring(0, 50)}...`);
...
const productionDatabaseAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      await dbService.query('UPDATE user_analytics SET query_count = query_count + 1 WHERE user_id = ?', [message.entityId]);
...
[/PRODUCTION SYSTEM STATS]`;
      ...
      return {
        ...
        values: {
          ...
          healthScore,
          alertLevel,
          servicesRunning: systemStats.services.database.isStarted && systemStats.services.cache.cacheSize !== undefined
        },
        data: {
  ...
  calculateHealthScore(stats: any): number {
    ...
    if (stats.services.database.isStarted) {
      score += 20;
      const healthyConnections = stats.services.database.connections?.filter(c => c.status === 'connected').length || 0;
...
const productionPerformanceEvaluator: Evaluator = {
  ...
  examples: [
    {
      ...
      messages: [
        ...
        { name: 'assistant', content: { text: 'Let me analyze the production system performance.' } }
      ],
  ...
  handler: async (
    ...
    try {
      ...
      console.log('PROD_PERFORMANCE_EVALUATOR comprehensive metrics:', JSON.stringify({
        summary: {
        ...
        keyMetrics: {
          ...
          memoryUtilization: (metrics.runtime.memoryUsage.heapUsed / metrics.runtime.memoryUsage.heapTotal * 100).toFixed(1) + '%'
        }
      }, null, 2));
  ...
  calculateSystemLoad(dbService: any, cacheService: any): number {
    ...
    if (dbService) {
      ...
      const avgQueries = stats.connections?.reduce((sum, conn) => sum + conn.queries, 0) / (stats.connections?.length || 1) || 0;
  ...
  analyzePerformance(metrics: any): any {
    ...
    if (metrics.services.database) {
      ...
      if (metrics.services.database.stats.isStarted) {
        totalScore += 15;
        const avgQueryTime = metrics.services.database.auditLog.reduce((sum, log) => 
          sum + (log.executionTime || 0), 0) / Math.max(1, metrics.services.database.auditLog.length);
    ...
    // Memory usage (25%)
    maxScore += 25;
    const memUtilization = metrics.runtime.memoryUsage.heapUsed / metrics.runtime.memoryUsage.heapTotal;
  ...
  init: async (config: Record<string, string>, runtime: IAgentRuntime) => {
    ...
    for (const key of optional) {
      ...
      if (value) {
        ...
        console.log(`production-plugin-with-env-vars: Optional config ${key}: not set (using default)`);
      }
    }
    
    console.log('production-plugin-with-env-vars: All production environment variables validated successfully');
  }
};
...
const productionTestCharacter: Character = {
  ...
  system: 'You are a production test agent designed to validate plugin configuration functionality under realistic conditions.',
  messageExamples: [
    [
      { name: 'user', content: { text: 'test the production database' } },
      { name: 'ProductionConfigTestAgent', content: { text: 'I\'ll test the production database connection with full transaction support.', actions: ['PROD_QUERY_DATABASE'] } }
    ]
  ],
  postExamples: [],
  topics: ['production', 'testing', 'configuration', 'plugins', 'performance'],
  adjectives: ['professional', 'thorough', 'reliable', 'production-ready'],
  knowledge: [],
...
const productionPluginConfigurationScenario: Scenario = {
  ...
  description: 'Comprehensive production-ready test of plugin configuration system with real services and environments',
  ...
  setup: {
    ...
    initialMessages: [
      {
        sender: 'system',
        content: 'Production plugin configuration system test environment initialized. All services are running in production simulation mode.'
      }
    ]
  },
...
export async function testProductionPluginConfiguration(runtime: IAgentRuntime): Promise<void> {
  ...
  try {
    ...
    if (!providerResult || !providerResult.text) throw new Error('Production provider execution failed');
    if (!providerResult.values?.systemStats) throw new Error('Production provider did not return system stats');
    if (typeof providerResult.values.healthScore !== 'number') throw new Error('Production provider did not calculate health score');
    ...
    if (!evalResult.metrics) throw new Error('Production performance evaluation did not generate metrics');
    if (!evalResult.analysis) throw new Error('Production performance evaluation did not generate analysis');
    
    console.log('✅ Test 4 passed: Production performance evaluation generating comprehensive metrics');
    ...
    if (validationAfterDisable) throw new Error('Action should fail validation when production service is disabled');
    ...
    const dbServiceAfterEnable = runtime.getService('prod-database-service') as ProductionDatabaseService;
    ...
    if (!dbStatsAfterEnable.isStarted) throw new Error('ProductionDatabaseService should be started after re-enabling');
    ...
    if (!validationAfterEnable) throw new Error('Action should pass validation when production service is re-enabled');
    ...
    try {
      ...
      throw new Error('Plugin registration should have failed due to environment variable validation');
    } catch (error) {
      if (!error.message.includes('Invalid DATABASE_URL format')) {
        throw new Error(`Unexpected error message: ${error.message}`);
      }
      console.log('✅ Test 6 passed: Environment variable validation working with detailed error messages');
    }
    ...
    console.log('- Overall Health Score:', finalSystemStats.values?.healthScore?.toFixed(2) + '/100');
