...
**IMPORTANT: Test files are automatically generated by the migration tool:**
- DO NOT manually create test files
- Tests are generated AFTER code migration completes
- The tool handles all test creation automatically
...
**Tests are automatically generated by the migration tool - DO NOT create tests manually.**
...
// ✅ V2: Plugin WITHOUT Service (most common case)
const myPlugin: Plugin = {
    name: 'my-plugin',
    description: 'Plugin description',
    services: [], // Empty array if no service in V1
    actions: [...],
    providers: [...],
    tests: [...],
    // init is optional if no services
};
```
...
**CRITICAL: runtime.language.generateText → runtime.useModel Migration:**
...
// ❌ V1: runtime.memory.create pattern
...
**🚨 CRITICAL SERVICE RULE: Only create services if they existed in the original V1 plugin**

**MANDATORY CHECK BEFORE CREATING SERVICE:**
1. **Analyze main branch** - Check if the plugin had a service in V1
2. **DO NOT add services** to plugins that didn't originally have them
3. **Service is OPTIONAL** - Many plugins work fine without services

**How to determine if service is needed:**
```bash
# Check main branch for existing service files
git ls-tree main --name-only | grep -i service
# OR check in src/ directory
ls src/*service* src/*Service* 2>/dev/null
```

**1. Create Service Class (src/service.ts) - ONLY IF SERVICE EXISTED IN V1:**

```typescript
// ⚠️ ONLY CREATE THIS FILE IF:
// 1. The plugin had a service.ts or Service.ts file in V1
// 2. The plugin's functionality requires a service (e.g., Discord bot connection)
// 3. DO NOT add services just because other plugins have them
...
export const ConfigSchema = z.object({
  ...
  // CRITICAL: Use z.coerce.number() for ALL numeric environment variables
  MAX_RESULTS: z.coerce.number().default(10), // ✅ Coerce string to number
  TIMEOUT_MS: z.coerce.number().optional(), // ✅ Optional numeric field
  MAX_FILE_SIZE: z.coerce.number().default(10485760), // ✅ With default
});
...
export function validateMyConfig(runtime: IAgentRuntime): MyConfig {
  const config = {
    ...
    MAX_RESULTS: runtime.getSetting('MY_MAX_RESULTS') || process.env.MY_MAX_RESULTS,
    TIMEOUT_MS: runtime.getSetting('MY_TIMEOUT_MS') || process.env.MY_TIMEOUT_MS,
    MAX_FILE_SIZE: runtime.getSetting('MY_MAX_FILE_SIZE') || process.env.MY_MAX_FILE_SIZE,
  };

  return ConfigSchema.parse(config);
}

// 🚨 ENV VARIABLE RULE: Based on the schema above:
// - API_KEY: REQUIRED (no default) → Add MY_API_KEY to .env
// - API_ENDPOINT: OPTIONAL → Skip (don't add to .env)
// - ENABLE_FEATURE: HAS DEFAULT → Skip (don't add to .env)
// - MAX_RESULTS: HAS DEFAULT → Skip (don't add to .env)
// - TIMEOUT_MS: OPTIONAL → Skip (don't add to .env)
// - MAX_FILE_SIZE: HAS DEFAULT → Skip (don't add to .env)
```

**🚨 CRITICAL ZOD PATTERNS FOR NUMERIC ENVIRONMENT VARIABLES:**

```typescript
// ❌ V1: This will fail with "Expected number, received nan"
const ConfigSchema = z.object({
    MAX_RESULTS: z.number().default(10), // ❌ WRONG
    TIMEOUT: z.number().optional(), // ❌ WRONG
});

// ✅ V2: Use coerce for ALL numeric env vars
const ConfigSchema = z.object({
    MAX_RESULTS: z.coerce.number().default(10), // ✅ CORRECT
    TIMEOUT: z.coerce.number().optional(), // ✅ CORRECT
    REQUIRED_NUMBER: z.coerce.number().min(1), // ✅ Required without default
});
```
...
- ✅ **Skip service tests if no service exists** - Don't test what isn't there
...
**RULE: Only add REQUIRED environment variables based on Zod schema analysis**

5. **Smart Environment Variable Detection - ONLY REQUIRED FIELDS:**
   
   **CRITICAL: Analyze the plugin's config.ts Zod schema to determine which variables are REQUIRED:**
   
   - ✅ **REQUIRED**: Fields with `.min(1)`, no `.optional()`, no `.default()`
   - ❌ **SKIP**: Fields with `.optional()` or `.default()` - these are NOT required
   - ❌ **SKIP**: OPENAI_API_KEY and database variables - already in .env

**SMART ENV DETECTION PATTERN:**

```typescript
// Example Zod schema analysis:
const ConfigSchema = z.object({
  API_KEY: z.string().min(1, "Required"), // ✅ ADD to .env
  ENDPOINT: z.string().url().default("https://..."), // ❌ SKIP - has default
  TIMEOUT: z.coerce.number().optional(), // ❌ SKIP - optional
  MAX_RETRIES: z.coerce.number().default(3), // ❌ SKIP - has default
});
```

**Example: Smart .env additions based on schema:**

```bash
# For plugin-allora (based on config.ts analysis):
# ONLY add required fields without defaults:
ALLORA_API_KEY=test-allora-key-12345

# DO NOT add these (they have defaults or are optional):
# ALLORA_API_ENDPOINT - has default("https://api.allora.network/v1/")
# ALLORA_MODEL - has default("llama-2-7b-chat")
# ALLORA_MAX_TOKENS - has default(512)
# ALLORA_TEMPERATURE - has default(0.7)
# ALLORA_TOP_P - has default(0.9)

# For plugin-coinmarketcap:
COINMARKETCAP_API_KEY=test-cmc-key-67890
# Skip COINMARKETCAP_ENDPOINT if it has a default

# For plugin-news:
NEWS_API_KEY=dummy-news-key-12345
# Skip optional or defaulted fields
```

**⚠️ IMPORTANT: Always check the plugin's config.ts Zod schema to identify truly required fields. Only add dummy values for fields that would fail validation without them.**
...
**Example .env for testing (based on Zod schema analysis):**

```bash
# ONLY add required fields (those without .default() or .optional())
# Example for a plugin with this schema:
# API_KEY: z.string().min(1) - REQUIRED ✅
# ENDPOINT: z.string().default("https://...") - HAS DEFAULT ❌
# TIMEOUT: z.number().optional() - OPTIONAL ❌

MY_API_KEY=test-api-key-12345  # ✅ Only if required (no default/optional)
...
await action.handler(runtime, mockMessage, state, {}, callback);
...
### 🚨 **CRITICAL: Type Import Separation Rules - MUST FOLLOW**

**This is one of the most common V2 migration issues. ALL type imports must be separated from value imports.**

**1. Basic Type Import Separation:**

```typescript
// ❌ V1: Mixed imports
import { Service, type IAgentRuntime, type ServiceType } from "@elizaos/core";
import { Memory, type State, type Content } from "@elizaos/core";

// ✅ V2: Separated imports
import { Service, Memory } from "@elizaos/core";
import type { IAgentRuntime, ServiceType, State, Content } from "@elizaos/core";
```
...
**3. Common Import Patterns to Fix:**

```typescript
// ❌ ALL OF THESE ARE WRONG IN V2:
import { TestSuite } from "@elizaos/core"; // TestSuite is type-only
import { ActionExample, Content } from "@elizaos/core"; // Both are types
import { Provider, type IAgentRuntime } from "@elizaos/core"; // Mixed
...
```typescript
// ❌ V1: Explicit type annotation
export class MyService extends Service {
    static serviceType: ServiceType = "my-service"; // ❌ WRONG
    private config: MyConfig; // ❌ May cause test issues
}

// ✅ V2: Correct patterns
export class MyService extends Service {
    static serviceType = "my-service"; // ✅ No explicit type
    public config: MyConfig; // ✅ Public for test access
}
```

**5. Apply to ALL Files:**
- Check EVERY import statement
- Separate ALL type imports
- Use `import type` for interfaces, types, and type-only exports
...
3. **Basic Action Testing** - Test validate(), handler() structure, examples validation
4. **Deep Action Execution Testing** - Test with multiple scenarios, edge cases, real-world usage
5. **Action Examples Testing** - Verify examples match actual behavior, validate example structure
6. **Memory Creation Testing** - Test memory persistence, validate memory structure and source
7. **Provider State Testing** - Test all providers return proper data/values/text structure
8. **Error Handling Testing** - Test invalid API keys, missing config, network failures
9. **Plugin Lifecycle Testing** - Test init() with/without config, service lifecycle

**PROGRESSIVE TESTING APPROACH:**

The test suite implements a progressive testing strategy where each stage must pass before the next can run:

1. **Stage 1: Basic Structure** → Must pass before any other tests
2. **Stage 2: Service Tests** → Requires Stage 1 to pass
3. **Stage 3: Action Validation** → Requires Stages 1-2 to pass
4. **Stage 4: Deep Action Tests** → Requires Stages 1-3 to pass
5. **Stage 5: Examples Testing** → Requires Stages 1-4 to pass
6. **Stage 6: Memory Testing** → Requires Stages 1-4 to pass

**Benefits of Progressive Testing:**
- Early failure detection - don't waste time on complex tests if basics fail
- Clear debugging path - know exactly which stage failed
- Enforces proper plugin structure - can't skip to advanced features
- Better developer experience - clear progression indicators

**ENHANCED ACTION TESTING PATTERNS:**

- **Scenario-based Testing**: Test actions with basic requests, complex parameters, edge cases
- **Handler Execution**: Actually execute handlers and verify callbacks are called
- **Memory Tracking**: Track and validate all memories created by actions
- **Example Validation**: Ensure action examples are valid and match actual behavior
- **Mock Runtime Enhancement**: Enhanced mocking for useModel, memory tracking, call history
- **Progressive Validation**: Each test stage builds on previous successes

### 🚨 **CRITICAL: Test Import Pattern**

When fixing test-related imports, use type-only imports:
...
**DO NOT create test files manually - the migration tool handles all test generation automatically.**
...
// ONLY import service if it existed in V1
import { MyService } from './service'; // ⚠️ ONLY if service.ts exists

const myPlugin: Plugin = {
  name: 'my-plugin',
  description: 'My plugin description for ElizaOS',
  services: [MyService], // ⚠️ OR [] if no service in V1
  actions: myPluginActions,
  providers: myPluginProviders,
  evaluators: [],
  tests: [testSuite], // REQUIRED: Register test suite
  init: async (config: Record<string, string>, runtime: IAgentRuntime) => {
    // initialization logic - ONLY needed if services exist
  },
};

// Example for plugin WITHOUT service (most common):
const myPlugin: Plugin = {
  name: 'my-plugin',
  description: 'My plugin description for ElizaOS',
  services: [], // ✅ Empty array - no service needed
  actions: myPluginActions,
  providers: myPluginProviders,
  evaluators: [],
  tests: [testSuite],
  // No init needed without services
};
```
