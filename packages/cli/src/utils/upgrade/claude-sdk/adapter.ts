...
export class EnhancedClaudeSDKAdapter implements ClaudeSDKAdapter {
  ...
  async executePrompt(
    ...
    while (retryCount < this.maxRetries) {
      try {
        ...
        if (result.message?.includes('error_max_turns') && maxTurnsCount < 3) {
          ...
          logger.warn(
            `⏸️  Hit conversation limit (attempt ${maxTurnsCount}/3). Waiting ${waitTime / 1000} seconds for reset...`
          );
        ...
        if (this.isRateLimitResult(result)) {
          ...
          logger.warn(
            `⏸️  Rate limit detected. Waiting ${waitTime / 1000} seconds before retry ${retryCount + 1}/${this.maxRetries}...`
          );
          await this.showCountdown(waitTime);
          retryCount++;
          continue;
        ...
        if (retryCount < this.maxRetries) {
          ...
          logger.warn(
            `🔄 Retrying SDK execution (${retryCount}/${this.maxRetries}) in ${waitTime / 1000} seconds...`
          );
        ...
        if (this.isRateLimitError(err) && retryCount < this.maxRetries - 1) {
          const waitTime = this.calculateBackoffTime(retryCount, 'rate_limit');
          logger.warn(
            `⏸️  Rate limit exception. Waiting ${waitTime / 1000} seconds before retry ${retryCount + 1}/${this.maxRetries}...`
          );
          await this.showCountdown(waitTime);
          retryCount++;
          continue;
        ...
        if (retryCount < this.maxRetries) {
          ...
          logger.warn(
            `🔄 Retrying SDK execution after exception (${retryCount}/${this.maxRetries}) in ${waitTime / 1000} seconds...`
          );
          await new Promise((resolve) => setTimeout(resolve, waitTime));
        }
      }
    ...
    throw new Error(
      `SDK execution failed after ${this.maxRetries + maxTurnsCount} attempts. Last error: ${errorMessage}`
    );
  }
  ...
  private async executePromptWithSDK(
    ...
    try {
      ...
      logger.info(
        `✅ SDK execution completed. Cost: $${result.cost?.toFixed(4) || '0'}, Duration: ${result.duration || 0}ms, Success: ${result.success}`
      );
      return result;
    } catch (error) {
  ...
  private isRecoverableSDKError(error: unknown): boolean {
    ...
    return recoverablePatterns.some((pattern) => errorMessage.toLowerCase().includes(pattern));
  }
  ...
  private buildPhasePrompt(phase: MigrationPhase, context: MigrationContext): string {
    const basePrompt =
      context.claudePrompts.get(phase) || this.getDefaultPhasePrompt(phase, context);
...
IMPORTANT: If you encounter any errors during execution, continue with as much work as possible and document what was completed.`;
  ...
  private isRecoverableResult(result: SDKPhaseResult): boolean {
    ...
    return (
      recoverablePatterns.some((pattern) => result.message.toLowerCase().includes(pattern)) ||
  ...
  private isRateLimitResult(result: SDKPhaseResult): boolean {
    ...
    return rateLimitPatterns.some((pattern) => result.message.toLowerCase().includes(pattern));
  }
}
