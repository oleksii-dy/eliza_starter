    ...
    try {
      ...
      try {
        ...
        if (project.isPlugin || project.agents.length === 0) {
          ...
          try {
            // Import the default character but we will override its plugins
            const defaultElizaCharacter = getElizaCharacter();

            // Ensure that the core plugins are loaded first for any plugin test
            const sqlPlugin = await loadPluginModule('@elizaos/plugin-sql');
            const bootstrapPlugin = await loadPluginModule('@elizaos/plugin-bootstrap');
            const openaiPlugin = await loadPluginModule('@elizaos/plugin-openai');
            ...
            // Override the character's plugins to use our controlled list
            const testCharacter = {
              ...defaultElizaCharacter,
              plugins: [], // Start with an empty list
            };

            logger.info(`Starting test agent with plugin: ${project.pluginModule?.name}`);
            logger.debug(
              `Using default character with plugins: ${
                defaultElizaCharacter.plugins ? defaultElizaCharacter.plugins.join(', ') : 'none'
              }`
            );
            logger.info(
            ...
            const runtime = await startAgent(testCharacter, server, undefined, pluginsToTest, {
...
async function runAllTests(options: { port?: number; name?: string; skipBuild?: boolean }) {
  ...
  const e2eResult = await runE2eTests(options, projectInfo);
