...
export class ChainConfigService {
    ...
    async getGasPrice(chainId: number): Promise<{
        ...
        try {
            ...
            if (config.chain.fees?.baseFeeMultiplier) {
                ...
                return {
                    maxFeePerGas: BigInt(Math.ceil(Number(maxFeePerGas) * (config.gasSettings?.maxFeeMultiplier || 1.2))),
                    maxPriorityFeePerGas: BigInt(Math.ceil(Number(maxPriorityFeePerGas) * (config.gasSettings?.maxPriorityFeeMultiplier || 1.1)))
                };
            }
            
            // For legacy chains
            return {
                gasPrice: BigInt(Math.ceil(Number(gasPrice) * (config.gasSettings?.maxFeeMultiplier || 1.1)))
            };
        } catch (error) {
            // Fallback gas prices
            if (config.chain.fees?.baseFeeMultiplier) {
    ...
    getMulticallAddress(chainId: number): `0x${string}` | undefined {
        const config = this.getChain(chainId);
        return config?.multicallAddress || (config?.chain.contracts?.multicall3?.address as `0x${string}` | undefined);
    }

    getENSRegistry(chainId: number): `0x${string}` | undefined {
        const config = this.getChain(chainId);
        return config?.ensRegistry || (config?.chain.contracts?.ensRegistry?.address as `0x${string}` | undefined);
    }
    ...
    async waitForTransaction(chainId: number, hash: `0x${string}`, confirmations?: number): Promise<any> {
        const client = this.getPublicClient(chainId);
        const config = this.getChain(chainId);
...
export function getChainConfig(chainId: number): ChainConfig {
    ...
    const chains = {
        1: { chain: mainnet, rpcUrl: 'https://eth.llamarpc.com', nativeWrappedToken: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2' },
        11155111: { chain: sepolia, rpcUrl: 'https://ethereum-sepolia.publicnode.com', nativeWrappedToken: '0x7b79995e5f793A07Bc00c21412e50Ecae098E7f9' },
        137: { chain: polygon, rpcUrl: 'https://polygon-rpc.com', nativeWrappedToken: '0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270' },
        80001: { chain: polygonMumbai, rpcUrl: 'https://rpc-mumbai.maticvigil.com', nativeWrappedToken: '0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889' },
        42161: { chain: arbitrum, rpcUrl: 'https://arb1.arbitrum.io/rpc', nativeWrappedToken: '0x82aF49447D8a07e3bd95BD0d56f35241523fBab1' },
        421613: { chain: arbitrumGoerli, rpcUrl: 'https://goerli-rollup.arbitrum.io/rpc', nativeWrappedToken: '0xe39Ab88f8A4777030A534146A9Ca3B52bd5D43A3' },
        10: { chain: optimism, rpcUrl: 'https://mainnet.optimism.io', nativeWrappedToken: '0x4200000000000000000000000000000000000006' },
        420: { chain: optimismGoerli, rpcUrl: 'https://goerli.optimism.io', nativeWrappedToken: '0x4200000000000000000000000000000000000006' },
        8453: { chain: base, rpcUrl: 'https://mainnet.base.org', nativeWrappedToken: '0x4200000000000000000000000000000000000006' },
        56: { chain: bsc, rpcUrl: 'https://bsc-dataseed.binance.org', nativeWrappedToken: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c' },
    } as const;
