...
import type { TransactionRequest, SimulationResult, StateChange } from '../interfaces/IWalletService';
...
export class TransactionSimulator {
    ...
    private async simulateWithTenderly(
        ...
        const response = await fetch(`https://api.tenderly.co/api/v1/account/${username}/project/${projectSlug}/simulate`, {
            method: 'POST',
            headers: {
    ...
    private parseDebugTrace(trace: any, tx: TransactionRequest): SimulationResult {
        ...
        const logs = trace.logs?.map((log: any) => ({
            address: log.address,
            topics: log.topics,
            data: log.data
        })) || [];
    ...
    private extractWarnings(simulation: any): string[] {
        ...
        if (simulation.logs?.some((log: any) => 
    ...
    async checkContractSafety(
        ...
        try {
            ...
            if (code.includes('ff')) { // SELFDESTRUCT
