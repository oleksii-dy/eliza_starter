...
import { privateKeyToAccount, generatePrivateKey, english, generateMnemonic, mnemonicToAccount } from 'viem/accounts';
...
export class EVMWalletService extends Service implements IWalletService {
    ...
    capabilityDescription = "Provides standardized access to wallet balances and portfolios." as const;
    ...
    private initializeChains() {
        ...
        for (const chain of chains) {
            ...
            this.walletClients.set(chain.id, createWalletClient({
                chain,
                transport: http()
            }) as any);
        }
    ...
    async deploySmartWallet(params: SmartWalletParams): Promise<WalletInstance> {
        ...
        const result = await this.smartWalletFactory.deploySmartWallet(
            params
        );
    ...
    async estimateGasWithBuffer(tx: TransactionRequest, bufferPercent: number = 10): Promise<GasEstimation> {
        ...
        return {
            ...
            baseFee: gasPrice.maxFeePerGas && gasPrice.maxPriorityFeePerGas 
    ...
    async sendTransaction(
        ...
        await this.dbService.saveTransaction({
            ...
            status: 'pending',
            chainId
        }, wallet.id, options?.sessionId);

        // Wait for confirmation if requested
        if (options?.waitForConfirmation) {
            await this.chainService.waitForTransaction(
                chainId,
                hash,
        ...
        const positionPromises = chainIds.map(async (chainId) => {
            try {
                ...
                return positions.map(pos => ({
                    ...
                    chain: chain,
                    type: pos.type,
                    positions: pos.tokens?.map(token => ({
    ...
    async getAllDefiPositions(wallet: WalletInstance | string, chainIds?: number[]): Promise<DefiPosition[]> {
    ...
    async getPortfolioValue(
        ...
        // Get DeFi positions
        const defiPositions = await this.getAllDefiPositions(wallet, chains?.map(c => c.id));
        ...
        const nftValue = nftHoldings.reduce((sum, nft) => 
            sum + (nft.collection?.floorPrice || 0), 0
        );
