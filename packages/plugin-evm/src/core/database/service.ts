...
export class WalletDatabaseService {
    ...
    constructor(private runtime: IAgentRuntime) {
        ...
        const connectionString = runtime.getSetting('DATABASE_URL') || 
    ...
    async saveTransaction(tx: TransactionHistory & {chainId: number}, walletId: string, sessionId?: string): Promise<void> {
        try {
        ...
        try {
            if (chainId !== undefined) {
                const results = await this.db
                    ...
                    .where(
                        and(
                            eq(tokenBalances.walletId, asUUID(walletId)),
                            eq(tokenBalances.chainId, chainId)
                        )
                    )
        ...
        try {
            if (chainIds && chainIds.length > 0) {
                const results = await this.db
                    ...
                    .where(
                        and(
                            eq(nftHoldings.walletId, asUUID(walletId)),
                            inArray(nftHoldings.chainId, chainIds)
                        )
                    )
