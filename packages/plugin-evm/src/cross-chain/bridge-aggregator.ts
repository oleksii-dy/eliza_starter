...
export class BridgeAggregatorService {
   ...
   */
  async executeBridge(quote: BridgeQuote, recipient?: Address): Promise<Hash> {
    try {
      ...
      const txHash = await this.mevProtection.sendProtectedTransaction(bridgeTx, protectionLevel);
   ...
   */
  async findRoutes(fromChain: number, toChain: number, token?: Address): Promise<BridgeRoute[]> {
  ...
  private async getAllQuotes(params: BridgeParams): Promise<BridgeQuote[]> {
    const routes = await this.findRoutes(params.fromChain, params.toChain, params.fromToken);
