...
export class ENSResolver {
  ...
  async resolveAddress(ensName: string): Promise<Address | null> {
    try {
      ...
      logger.error(`Error resolving ENS name ${ensName}:`, _error);
      return null;
    }
  }
  ...
  async resolveName(address: Address): Promise<string | null> {
    try {
      ...
      logger.error(`Error reverse resolving address ${address}:`, _error);
      return null;
    }
  }
  ...
  async getProfile(ensName: string): Promise<ENSProfile | null> {
    try {
      ...
      logger.error(`Error getting ENS profile for ${ensName}:`, _error);
      return null;
    }
  }
...
export function createENSResolver(_runtime: IAgentRuntime) {
