...
export class BridgeAggregator {
  ...
  private initializeClients() {
    ...
    for (const chain of chains) {
      ...
      ); // Type assertion to handle viem version mismatch
    }
  }
  ...
  async getBestQuote(params: BridgeParams): Promise<BridgeQuote | null> {
    ...
    const quotePromises = routes.map((route) => this.getQuoteForRoute(route, params));
  ...
  private async getStargateRoute(
    ...
    const fromRouter =
      BRIDGE_CONTRACTS.stargate.routers[
        fromChain as keyof typeof BRIDGE_CONTRACTS.stargate.routers
      ];
    const toRouter =
      BRIDGE_CONTRACTS.stargate.routers[toChain as keyof typeof BRIDGE_CONTRACTS.stargate.routers];
    ...
    const poolId =
      BRIDGE_CONTRACTS.stargate.poolIds[
        tokenSymbol as keyof typeof BRIDGE_CONTRACTS.stargate.poolIds
      ];
  ...
  private async getStargateQuote(
    ...
    try {
      const router =
        BRIDGE_CONTRACTS.stargate.routers[
          params.fromChain as keyof typeof BRIDGE_CONTRACTS.stargate.routers
        ];
      if (!router) {
        return null;
      }
      ...
      const toAddress = encodeAbiParameters(parseAbiParameters('address'), [
  ...
  private async getHopRoute(
    ...
    const tokenSymbol = await this.getTokenSymbol(token, fromChain);
    const bridges =
      BRIDGE_CONTRACTS.hop.bridges[tokenSymbol as keyof typeof BRIDGE_CONTRACTS.hop.bridges];
  ...
  private async getHopQuote(route: BridgeRoute, params: BridgeParams): Promise<BridgeQuote | null> {
    try {
      // In production, would call Hop API or SDK
      const response = await axios.get('https://api.hop.exchange/v1/quote', {
        params: {
  ...
  private async getAcrossRoute(
    ...
    const fromPool =
      BRIDGE_CONTRACTS.across.spokePool[
        fromChain as keyof typeof BRIDGE_CONTRACTS.across.spokePool
      ];
    const toPool =
      BRIDGE_CONTRACTS.across.spokePool[toChain as keyof typeof BRIDGE_CONTRACTS.across.spokePool];
  ...
  private async getAcrossQuote(
    ...
    try {
      // Across API endpoint
      const response = await axios.get('https://app.across.to/api/suggested-fees', {
        params: {
  ...
  private async getSynapseRoute(
    ...
    const fromBridge =
      BRIDGE_CONTRACTS.synapse.bridge[fromChain as keyof typeof BRIDGE_CONTRACTS.synapse.bridge];
    const toBridge =
      BRIDGE_CONTRACTS.synapse.bridge[toChain as keyof typeof BRIDGE_CONTRACTS.synapse.bridge];
  ...
  private async getSynapseQuote(
    ...
    try {
      // Synapse REST API
      const response = await axios.get('https://syn-api-x.herokuapp.com/v1/get_quote', {
        params: {
  ...
  private async getConnextRoute(
    ...
    const fromConnext =
      BRIDGE_CONTRACTS.connext.connext[fromChain as keyof typeof BRIDGE_CONTRACTS.connext.connext];
    const toConnext =
      BRIDGE_CONTRACTS.connext.connext[toChain as keyof typeof BRIDGE_CONTRACTS.connext.connext];

    if (!fromConnext || !toConnext) {
      return null;
    }
  ...
  private async getConnextQuote(
    ...
    try {
      ...
      const response = await axios.post('https://bridge.connext.network/v1/quote', {
  ...
  async executeBridge(quote: BridgeQuote, params: BridgeParams): Promise<Hex> {
  ...
  private async executeStargateBridge(quote: BridgeQuote, params: BridgeParams): Promise<Hex> {
  ...
  private async executeHopBridge(quote: BridgeQuote, params: BridgeParams): Promise<Hex> {
    logger.info(`Executing Hop bridge from chain ${params.fromChain} to ${params.toChain}`);
    return `0x${'0'.repeat(64)}` as Hex;
  }

  private async executeAcrossBridge(quote: BridgeQuote, params: BridgeParams): Promise<Hex> {
    logger.info(`Executing Across bridge from chain ${params.fromChain} to ${params.toChain}`);
    return `0x${'0'.repeat(64)}` as Hex;
  }

  private async executeSynapseBridge(quote: BridgeQuote, params: BridgeParams): Promise<Hex> {
    logger.info(`Executing Synapse bridge from chain ${params.fromChain} to ${params.toChain}`);
    return `0x${'0'.repeat(64)}` as Hex;
  }

  private async executeConnextBridge(quote: BridgeQuote, params: BridgeParams): Promise<Hex> {
