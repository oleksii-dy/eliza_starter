...
export class AccountAbstractionProvider {
    ...
    async buildUserOperation(params: {
        ...
        try {
            ...
            throw new Error(`Failed to build UserOperation: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async sendUserOperation(userOp: UserOperation): Promise<Hash> {
        try {
            ...
            throw new Error(`Failed to send UserOperation: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async getUserOpReceipt(userOpHash: Hash): Promise<{
        ...
        try {
            ...
            throw new Error(`Failed to get receipt: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    private async getPaymasterData(userOp: UserOperation): Promise<PaymasterData> {
        ...
        try {
            ...
            throw new Error(`Failed to get paymaster data: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
     ...
     */
    buildBatchExecuteCallData(transactions: Array<{
        to: Address;
        value: bigint;
        data: Hex;
    }>): Hex {
