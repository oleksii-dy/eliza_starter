...
import type { SessionPermission, WalletSession, SessionKey as ISessionKey } from '../core/interfaces/IWalletService';
...
export class SessionManager {
    ...
    async createSession(config: SessionConfig): Promise<WalletSession> {
        try {
            ...
            // Create session ID
            const sessionId = asUUID(keccak256(encodePacked(
    ...
    async signWithSession(
        ...
        try {
            ...
            throw new Error(`Failed to sign with session: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
