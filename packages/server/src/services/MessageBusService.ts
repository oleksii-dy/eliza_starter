  ...
  private async sendAgentResponseToBus(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // The MESSAGE_SENT event is now emitted by the server endpoint after successful message creation
        logger.info(
          `[${this.runtime.character.name}] MessageBusService: Successfully sent response to central server`
        );
      }
    } catch (error) {
      logger.error(
