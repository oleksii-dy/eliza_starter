...
export function createAgentConfigurationsRouter(
  ...
  router.get('/:agentId/configurations/:pluginName', async (req, res) => {
    ...
    try {
      ...
      logger.error(
        `[AGENT CONFIG] Error retrieving configuration for plugin ${pluginName} in agent ${agentId}:`,
        error
      );
      sendError(
        res,
  ...
  router.patch('/:agentId/configurations/:pluginName', async (req, res) => {
    ...
    try {
      ...
      logger.error(
        `[AGENT CONFIG] Error updating configuration for plugin ${pluginName} in agent ${agentId}:`,
        error
      );
      sendError(
        res,
  ...
  router.patch(
    '/:agentId/configurations/:pluginName/components/:componentType/:componentName',
    async (req, res) => {
      const agentId = validateUuid(req.params.agentId);
      if (!agentId) {
        return sendError(res, 400, 'INVALID_ID', 'Invalid agent ID format');
      ...
      if (!pluginName || !componentType || !componentName) {
        return sendError(
          ...
          400,
          'INVALID_PARAMS',
          'Plugin name, component type, and component name are required'
        );
      }
      ...
      if (!['action', 'provider', 'evaluator', 'service'].includes(componentType)) {
        return sendError(
          ...
          400,
          'INVALID_TYPE',
          'Component type must be action, provider, evaluator, or service'
        );
      }
      ...
      try {
        ...
        if (!validationResult.valid) {
          return sendError(
            ...
            400,
            'VALIDATION_ERROR',
            `Cannot update component: ${validationResult.errors.join(', ')}`
          );
        }
        ...
        const componentConfig = configManager.getComponentConfig(
          pluginName,
          componentName,
          componentType as 'action' | 'provider' | 'evaluator'
        );
        ...
        logger.error(
          `[AGENT CONFIG] Error updating component ${componentName} configuration for plugin ${pluginName} in agent ${agentId}:`,
          error
        );
        sendError(
          res,
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        logger.error(
          `[AGENT CONFIG] Error retrieving component ${componentName} configuration for plugin ${pluginName} in agent ${agentId}:`,
          error
        );
        sendError(
          res,
  ...
  router.post(
    '/:agentId/configurations/:pluginName/components/:componentType/:componentName/enable',
    async (req, res) => {
      const agentId = validateUuid(req.params.agentId);
      if (!agentId) {
        return sendError(res, 400, 'INVALID_ID', 'Invalid agent ID format');
      ...
      try {
        ...
        logger.error(
          `[AGENT CONFIG] Error enabling component ${componentName} for plugin ${pluginName} in agent ${agentId}:`,
          error
        );
        sendError(
          res,
  ...
  router.post(
    '/:agentId/configurations/:pluginName/components/:componentType/:componentName/disable',
    async (req, res) => {
      const agentId = validateUuid(req.params.agentId);
      if (!agentId) {
        return sendError(res, 400, 'INVALID_ID', 'Invalid agent ID format');
      ...
      try {
        ...
        logger.error(
          `[AGENT CONFIG] Error disabling component ${componentName} for plugin ${pluginName} in agent ${agentId}:`,
          error
        );
        sendError(
          res,
  ...
  router.post(
    '/:agentId/configurations/:pluginName/components/:componentType/:componentName/toggle',
    async (req, res) => {
      const agentId = validateUuid(req.params.agentId);
      if (!agentId) {
        return sendError(res, 400, 'INVALID_ID', 'Invalid agent ID format');
      ...
      try {
        const configManager = runtime.getConfigurationManager();
        const currentConfig = configManager.getComponentConfig(
          pluginName,
          componentName,
          componentType as 'action' | 'provider' | 'evaluator' | 'service'
        );
        ...
        const { overrideReason = `${newEnabledState ? 'Enabled' : 'Disabled'} via API toggle` } =
          req.body;
        ...
        // Get updated component configuration
        const updatedConfig = configManager.getComponentConfig(
          pluginName,
          componentName,
          componentType as 'action' | 'provider' | 'evaluator' | 'service'
        );
        ...
        logger.error(
          `[AGENT CONFIG] Error toggling component ${componentName} for plugin ${pluginName} in agent ${agentId}:`,
          error
        );
        sendError(
          res,
    ...
    try {
      ...
      if (pluginConfig) {
        ...
        for (const [actionName, actionConfig] of Object.entries(pluginConfig.actions || {})) {
          comparison.actions[actionName] = {
            configured: (actionConfig as any).enabled,
            registered: runtimeStatus.actions.includes(actionName),
            inSync: (actionConfig as any).enabled === runtimeStatus.actions.includes(actionName),
          };
        }
        ...
        for (const [providerName, providerConfig] of Object.entries(pluginConfig.providers || {})) {
          comparison.providers[providerName] = {
            configured: (providerConfig as any).enabled,
            registered: runtimeStatus.providers.includes(providerName),
            inSync:
              (providerConfig as any).enabled === runtimeStatus.providers.includes(providerName),
          };
        }

        // Check evaluators
        for (const [evaluatorName, evaluatorConfig] of Object.entries(
          pluginConfig.evaluators || {}
        )) {
          comparison.evaluators[evaluatorName] = {
            configured: (evaluatorConfig as any).enabled,
            registered: runtimeStatus.evaluators.includes(evaluatorName),
            inSync:
              (evaluatorConfig as any).enabled === runtimeStatus.evaluators.includes(evaluatorName),
          };
        }
        ...
        for (const [serviceName, serviceConfig] of Object.entries(pluginConfig.services || {})) {
          comparison.services[serviceName] = {
            configured: (serviceConfig as any).enabled,
            registered: runtimeStatus.services.includes(serviceName as any),
            inSync:
              (serviceConfig as any).enabled ===
              runtimeStatus.services.includes(serviceName as any),
          };
        }
      }
      ...
      sendSuccess(res, {
        ...
        inSync: Object.values(comparison).every((componentGroup) =>
          Object.values(componentGroup).every((comp) => (comp as any).inSync)
        ),
      });
    } catch (error) {
      logger.error(
        `[AGENT CONFIG] Error retrieving runtime status for plugin ${pluginName} in agent ${agentId}:`,
        error
      );
      sendError(
        res,
