...
import { setViewport, loginAsTestUser, waitForAnimation, takeScreenshot } from '../../support/utils/helpers'
import { moveToPosition, openPlayerInventory, performAttack, performJump } from '../../support/commands/player'
    ...
    cy.get(selectors.inventoryItem)
      ...
      .then($item => {
        if ($item.length > 0) {
          ...
          cy.get(selectors.inventorySlot).eq(10).trigger('mousemove').trigger('mouseup')
        }
      })
