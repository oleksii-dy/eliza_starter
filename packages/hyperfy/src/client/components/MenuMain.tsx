...
function MenuMainIndex({ world, pop, push }) {
  ...
  return (
    <Menu title='Menu' blur={false}>
      <MenuItemText label='Name' hint='Change your display name' placeholder='Enter name' value={name} onChange={changeName} />
      ...
      {isBuilder && (
        <MenuItemBtn label='Apps' hint='View all apps in the world' onClick={() => world.ui.toggleApps()} nav={false} />
      )}
    </Menu>
  )
}
  ...
  return (
    <Menu title='Menu' blur={false}>
      ...
      <MenuItemToggle
        ...
        value={isFullscreen}
        onChange={value => typeof toggleFullscreen === 'function' && toggleFullscreen(value)}
      />
      <MenuItemRange
        label='UI Scale'
