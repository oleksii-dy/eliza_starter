...
describe('Entity Physics (In-Sim)', () => {
  ...
  describe('Basic Movement', () => {
    ...
    it('should handle collisions between entities', async () => {
      ...
      scenario.world.events.on('collision', (data: any) => {
        if (
          (data.entity1 === entity1.id && data.entity2 === entity2.id) ||
  ...
  describe('Advanced Physics', () => {
    ...
    it('should respect physics constraints', async () => {
      ...
      expect(staticEntity.position.x).toBeCloseTo(0, 2)
      expect(staticEntity.position.y).toBeCloseTo(0, 2)
      expect(staticEntity.position.z).toBeCloseTo(0, 2)
    })
    ...
    it('should handle angular velocity', async () => {
      ...
      expect(entity.rotation.x).not.toBeCloseTo(initialRotation.x, 3)
      expect(entity.rotation.y).not.toBeCloseTo(initialRotation.y, 3)
      expect(entity.rotation.z).not.toBeCloseTo(initialRotation.z, 3)
      expect(entity.rotation.w).not.toBeCloseTo(initialRotation.w, 3)
    })
  })
