...
describe('Multiplayer E2E Scenarios', () => {
  ...
  describe('Player Join/Leave', () => {
    it('should handle player joining world', async () => {
      ...
      expect(scenario.world.entities.has(player1.id)).toBe(true);
      expect(scenario.world.entities.players.has(player1.id)).toBe(true);
    ...
    it('should handle multiple players joining', async () => {
      ...
      for (const player of players) {
        ...
        const visiblePlayers = Array.from(scenario.world.entities.players.values()).filter(
  ...
  describe('World Events', () => {
    ...
    it('should handle zone transitions', async () => {
      ...
      const distanceToDanger = Math.sqrt(
        Math.pow(player.position.x - dangerZone.position.x, 2) + Math.pow(player.position.z - dangerZone.position.z, 2)
      );
