      ...
      if (combat) {
        ...
        try {
          await scenario.runUntil(() => {
            ...
            if (!mobEntity) {return true;} // Mob removed
  ...
  describe('Loot Drop System', () => {
    it('should create loot drops from mob death', async () => {
      ...
      try {
        ...
        if (typeof lootSystem.createLootDrop === 'function') {
          try {
            const lootDrop = lootSystem.createLootDrop(
              { x: 100, y: 0, z: 100 },
              testItems,
              'test-player'
            );
    ...
    it('should handle chest loot drops', async () => {
      ...
      const chestEntity = {
        ...
        components: new Map(),
        getComponent(type: string) { return this.components.get(type) || null; },
        hasComponent(type: string) { return this.components.has(type); },
        addComponent(type: string, component: any) { this.components.set(type, component); }
      };
