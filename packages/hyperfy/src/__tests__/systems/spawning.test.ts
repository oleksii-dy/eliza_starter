...
describe('SpawningSystem', () => {
    ...
    ;(world as any).getEntitiesInRange = (position: any, range: number) => {
      ...
      for (const entity of world.entities.items.values()) {
        ...
        const dist = Math.sqrt(
          Math.pow(rpgEntity.data.position.x - position.x, 2) + Math.pow(rpgEntity.data.position.z - position.z, 2)
        )
        if (dist <= range) {
  ...
  describe('Spawner Registration', () => {
    it('should register spawners', () => {
      const spawnerId = spawningSystem.registerSpawner({
        ...
        entityDefinitions: [{ entityType: 'npc', entityId: 1, weight: 100 }],
        maxEntities: 3,
        respawnTime: 30000,
      })
  ...
  describe('Update Cycle', () => {
    ...
    it('should clean up destroyed entities', () => {
      ...
      // Mock entity lookup
      spyOn(spawningSystem as any, 'getEntity').mockImplementation((...args: unknown[]) => {
        const id = args[0]
        if (id === 'entity1') {
          return { id, data: { type: 'npc' } }
        }
  ...
  describe('Different Entity Types', () => {
    it('should create NPCs', () => {
      ...
      const spawnerId = spawningSystem.registerSpawner({
        type: SpawnerType.NPC,
        position: { x: 0, y: 0, z: 0 },
        entityDefinitions: [{ entityType: 'npc', entityId: 5, weight: 100 }],
      })

      const spawner = (spawningSystem as any).spawners.get(spawnerId)
      spawningSystem.spawnEntity(spawner)

      expect(npcSystem.spawnNPC).toHaveBeenCalledWith(5, expect.any(Object), spawnerId)
    })
    ...
    it('should handle unsupported entity types', () => {
      ...
      const resourceId = spawningSystem.registerSpawner({
        type: SpawnerType.RESOURCE,
        position: { x: 0, y: 0, z: 0 },
        entityDefinitions: [{ entityType: 'resource', weight: 100 }],
      })
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[SpawningSystem] Resource spawning not yet implemented')
  ...
  describe('Spatial Queries', () => {
    it('should find entities near position', () => {
      ...
      const entities = (spawningSystem as any).getEntitiesNear({ x: 0, y: 0, z: 0 }, 10)

      expect(entities).toHaveLength(2)
    })

    it('should validate spawn positions', () => {
      const isValid = (spawningSystem as any).isValidSpawnPosition({ x: 0, y: 0, z: 0 }, {})
