...
describe('VisualRepresentationSystem', () => {
  ...
  describe('Visual Creation', () => {
    it('should create visual for entity with explicit template', () => {
      ...
      expect(visual?.template.color).toBe(65280); // Green color for sword (test template)
      expect(visual?.template.size.height).toBe(1.2);
      expect(visual?.template.animations).toContain('idle'); // Test templates use 'idle'
    });
    ...
    it('should auto-detect template from entity type', () => {
      ...
      const visual = visualSystem.getVisual(entity.id);
      expect(visual).toBeDefined();
      expect(visual?.template.color).toBe(255); // Blue color for goblin (test template)
      expect(visual?.template.animations).toContain('idle'); // Test templates use 'idle'
    });
    ...
    it('should handle resource entities', () => {
      ...
      if (resource) {
        expect(visual).toBeDefined();
        expect(visual?.template.geometryType).toBe('box'); // Test templates use 'box'
        expect(visual?.template.animations).toContain('idle');
      }
    });
  });
  ...
  describe('Animation System', () => {
    ...
    it('should play animations', () => {
      ...
      // Play animation
      visualSystem.playAnimation(testEntity.id, 'idle', false, 1000);
      ...
      expect(animation.animationType).toBe('idle');
    ...
    it('should update animations over time', () => {
      // Play idle animation
      visualSystem.playAnimation(testEntity.id, 'idle', true, 1000);
      ...
      visualSystem.playAnimation(testEntity.id, 'idle', false, 100);
    ...
    it('should handle multiple animation types', () => {
      const animationTypes = ['idle', 'pulse', 'rotate', 'bounce']; // Use available animation types
  ...
  describe('Integration with Spawning System', () => {
    it('should create visuals for spawned NPCs', () => {
      ...
      if (npc) {
        ...
        expect(visual).toBeDefined();
        // NPC may get default template if not specifically defined in test templates
        expect(visual?.template.animations).toContain('idle');
      }
    });
  ...
  describe('Material and Geometry Types', () => {
    ...
    it('should apply metallic materials for appropriate items', () => {
      ...
      const visual = visualSystem.getVisual(entity.id);
      expect(visual).toBeDefined();
      expect(visual?.template.material?.metalness).toBe(0); // Test templates use metalness: 0
      expect(visual?.template.material?.roughness).toBe(1); // Test templates use roughness: 1
    });
  });
