// Custom reporter interfaces since bun:test doesn't export these types
interface TaskResultPack extends Array<any> {
  0: string
  1: { state?: string; errors?: any[] }
}
...
export default class GameMetricsReporter {
  ...
  onFinished(_files?: any[], errors?: unknown[]) {
    ...
    if (this.metrics.size > 0) {
      ...
      for (const [testName, metrics] of this.metrics) {
        console.log(`\n${testName}:`)
        console.log(
          `  Frame Time: avg=${metrics.frameTime.avg.toFixed(2)}ms, p95=${metrics.frameTime.p95.toFixed(2)}ms`
        )
        ...
        console.log(
          `  Memory: ${(metrics.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)}MB / ${(metrics.memoryUsage.heapTotal / 1024 / 1024).toFixed(2)}MB`
        )
      }
    }
    ...
    for (const [testName, metrics] of this.metrics) {
      if (metrics.frameTime.p95 > 16.67) {
        performanceIssues.push(
          `${testName}: Frame time p95 (${metrics.frameTime.p95.toFixed(2)}ms) exceeds 60fps target`
        )
      }

      if (metrics.memoryUsage.heapUsed > 500 * 1024 * 1024) {
        performanceIssues.push(
          `${testName}: High memory usage (${(metrics.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)}MB)`
        )
      }
    }
