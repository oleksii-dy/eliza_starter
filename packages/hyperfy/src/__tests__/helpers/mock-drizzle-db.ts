          ...
          where: (condition: any) => ({
            get: () => {
              ...
              return Promise.resolve(
                data[tableName]?.find(row => row[field] === value)
              );
            },
...
function getFieldName(condition: any): string {
  ...
  if (condition?.left?.name) {return condition.left.name;}
  if (condition?.column?.name) {return condition.column.name;}
  return 'id';
}

function getFieldValue(condition: any): any {
  // Handle eq() conditions
  if (condition?.right !== undefined) {return condition.right;}
