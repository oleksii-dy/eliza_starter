...
describe('PvP Combat Scenarios', () => {
  ...
  describe('3v3 Team Combat Test', () => {
    it('should handle 3v3 team combat (Melee vs Ranged vs Magic)', async () => {
      ...
      world.events.on('combat:damage_dealt', (data: any) => {
        if (data.attackerId.startsWith('melee_')) teamStats.A.damage += data.damage;
        if (data.attackerId.startsWith('ranged_')) teamStats.B.damage += data.damage;
        if (data.attackerId.startsWith('magic_')) teamStats.C.damage += data.damage;
      });
      
      world.events.on('combat:entity_died', (data: any) => {
        if (data.entityId.startsWith('melee_')) teamStats.A.deaths++;
        if (data.entityId.startsWith('ranged_')) teamStats.B.deaths++;
        if (data.entityId.startsWith('magic_')) teamStats.C.deaths++;
  ...
  describe('Visual Combat Verification', () => {
    it('should create visually verifiable combat scenario', async () => {
      ...
      if (deathEffects > 0) {
        ...
        const atRespawn = (g1Movement.position.x === 0 && g1Movement.position.z === 0) ||
...
// Helper function to create a test world with all necessary systems
function createTestWorld() {
  ...
  return {
    ...
    createEntity: function(config: any) {
      const entity = {
        id: config.id || `entity_${Date.now()}_${Math.random()}`,
        name: config.name || 'Test Entity',
        type: config.type,
        components: [],
        
        addComponent: function(component: any) {
          // Remove existing component of same type
          this.components = this.components.filter(c => c.type !== component.type);
    ...
    getEntityById: function(id: string) {
      return this.entities.get(id);
    },
    
    addEntity: function(entity: any) {
      this.entities.set(entity.id, entity);
    },
    
    addSystem: function(system: any) {
