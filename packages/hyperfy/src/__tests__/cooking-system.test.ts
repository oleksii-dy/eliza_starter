...
describe('Cooking System Tests', () => {
  ...
  describe('Cooking Process', () => {
    it('should fail cooking without required level', async () => {
      ...
      // Add raw ingredients
      inventorySystem.addItem(player.id, 383, 5); // Raw Shark
    ...
    it('should fail cooking without ingredients', async () => {
      ...
      world.events.on('cooking:error', data => {
        errorReceived = true;
        expect(data.message).toContain('Raw Shrimp');
      });
    ...
    it('should successfully start cooking with valid conditions', async () => {
      ...
      // Add raw ingredients
      inventorySystem.addItem(player.id, 317, 10); // Raw Shrimp

      // Set near fire
      cooking.nearFire = true;

      // Debug: Check if the component is properly attached and accessible
      const retrievedCooking = cookingSystem.getCookingComponent(player.id);
      expect(retrievedCooking).not.toBeNull();
      expect(retrievedCooking.nearFire).toBe(true);
  ...
  describe('Food Quality and Healing', () => {
    ...
    it('should produce burnt food on failure', () => {
      ...
      for (const recipe of recipes) {
        ...
        expect(typeof recipe.burntItemId).toBe('number');

        // Get the item definition to check the name
        const itemDef = inventorySystem.itemRegistry.get(recipe.burntItemId);
        expect(itemDef).toBeDefined();
        expect(itemDef.name.toLowerCase()).toContain('burnt');
      }
    });
  });
