...
describe('RPG Comprehensive System Tests', () => {
  ...
  describe('Integration Tests', () => {
    ...
    it('should maintain consistent visual theming', () => {
      ...
      const trees = Object.values(RESOURCE_DEFINITIONS)
        .filter(r => r.type.toString().startsWith('tree_'));
      ...
      const rocks = Object.values(RESOURCE_DEFINITIONS)
        .filter(r => r.type.toString().startsWith('rock_'));
...
describe('Visual Verification Tests', () => {
  ...
  it('should create distinct visual representations for all resource types', () => {
    ...
    visualTests.forEach(test => {
      ...
      if (test.emissive) console.log(`  - Glows with: ${test.emissive}`);
      if (test.metalness) console.log(`  - Metallic: ${test.metalness}`);
    });
