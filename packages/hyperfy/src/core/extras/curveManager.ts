...
export function curveManager({ curve, width, height, xLabel, yLabel, yMin = 0, yMax = 1 }: CurveManagerOptions) {
  ...
  function update(runTransition?: boolean) {
    ...
    g.selectAll('.tangentesCont')
      .data(curve!.keyframes, (d: any) => d.id)
      .join(enter =>
        enter
          .append('g')
          ...
          .each(function (this: any, _d: any) {
            ...
            d3.select(this)
              ...
              .attr('opacity', (d: any) => (d.id === curve!.lastKeyframe.id ? 0 : 1))
              .attr('fill', 'none')
              .attr('stroke', '#008ec4');
            ...
            d3.select(this)
              ...
              .attr('r', 5)
              .attr('cursor', 'move')
              .call(d3.drag<SVGCircleElement, any>().on('start', dragstartedKey).on('drag', draggedTangIn).on('end', dragendedKey));
            ...
            d3.select(this)
              ...
              .attr('r', 5)
              .attr('cursor', 'move')
              .call(d3.drag<SVGCircleElement, any>().on('start', dragstartedKey).on('drag', draggedTangOut).on('end', dragendedKey));
            ...
            d3.select(this)
              ...
              .call(d3.drag<SVGCircleElement, any>().on('start', dragstartedKey).on('drag', draggedKey).on('end', dragendedKey));
          })
      )
  ...
  function draggedKey(this: SVGCircleElement, event: any, d: any) {
    const pointer = d3.pointer(event, this as any);
  ...
  function draggedTangIn(this: SVGCircleElement, event: any, d: any) {
    const pointer = d3.pointer(event, this as any);
    d.setInTangentFromHandle(x.invert(pointer[0]), y.invert(pointer[1]));
    update();
  }

  function draggedTangOut(this: SVGCircleElement, event: any, d: any) {
    const pointer = d3.pointer(event, this as any);
