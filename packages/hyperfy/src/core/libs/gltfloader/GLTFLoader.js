...
class GLTFTextureWebPExtension {
  ...
  detectSupport() {
    ...
    if (! this.isSupported) {
      ...
      this.isSupported = new Promise((resolve, reject) => {
    ...
    if (bufferView.extensions && bufferView.extensions[this.name]) {
      ...
      return buffer.then((res) => {
        ...
        if (decoder.decodeGltfBufferAsync) {
          ...
          return decoder.decodeGltfBufferAsync(count, stride, source, extensionDef.mode, extensionDef.filter).then((res) => {
...
class GLTFParser {
  ...
  parse(onLoad, onError) {
    ...
    Promise.all(this._invokeAll((ext) => {
      ...
      return Promise.all(parser._invokeAll((ext) => {
  ...
  loadImageSource(sourceIndex, loader) {
    ...
    const promise = Promise.resolve(sourceURI).then((sourceURI) => {
  ...
  loadMaterial(materialIndex) {
    ...
    if (materialExtensions[EXTENSIONS.KHR_MATERIALS_UNLIT]) {
      ...
      pending.push(Promise.all(this._invokeAll((ext) => {
  ...
  _loadNodeShallow(nodeIndex) {
    ...
    const meshPromise = parser._invokeOne((ext) => {
    ...
    if (nodeDef.camera !== undefined) {
      ...
      pending.push(parser.getDependency('camera', nodeDef.camera).then((camera) => {
