...
export class RigidBody extends Node {
  ...
  mount() {
    ...
    if (this.ctx.moving) {
      return
    } // physics ignored when moving apps around
    if (!PHYSX) {
      console.warn('[rigidbody] PHYSX not initialized yet')
      return
    }
    this.matrixWorld.decompose(_v1 as any, _q1, _v2 as any)
    this.transform = new PHYSX.PxTransform(PHYSX.PxIDENTITYEnum.PxIdentity)
    if (typeof (_v1 as any).toPxTransform === 'function') {
      ;(_v1 as any).toPxTransform(this.transform)
    }
    if (typeof (_q1 as any).toPxTransform === 'function') {
      ;(_q1 as any).toPxTransform(this.transform)
    }
    if (this._type === 'static') {
      ...
      if (this._centerOfMass) {
        ...
        if (typeof (this._centerOfMass as any).toPxTransform === 'function') {
          ;(this._centerOfMass as any).toPxTransform(pose)
        }
  ...
  onInterpolate = (position, quaternion) => {
    if (this.parent) {
      _m1.compose(position, quaternion, _defaultScale as any)
      _m2.copy(this.parent.matrixWorld).invert()
      _m3.multiplyMatrices(_m2, _m1)
      _m3.decompose(this.position as any, this.quaternion, _v1 as any)
  ...
  addForce(force, mode) {
    ...
    if (!force.toPxExtVec3) {
      force = _v1.copy(force)
    }
  ...
  addForceAtPos(force, pos, mode) {
    if (!force?.isVector3) {
      throw new Error('[rigidbody] addForceAtPos force must be Vector3')
    }
    if (!pos?.isVector3) {
      throw new Error('[rigidbody] addForceAtPos force must be Vector3')
    }
  ...
  addForceAtLocalPos(force, pos, mode) {
    if (!force?.isVector3) {
      throw new Error('[rigidbody] addForceAtLocalPos force must be Vector3')
    }
    if (!pos?.isVector3) {
      throw new Error('[rigidbody] addForceAtLocalPos force must be Vector3')
    }
  ...
  setPosition(vec3) {
    ...
    const pose = this.actor.getGlobalPose()
    if (typeof (vec3 as any).toPxTransform === 'function') {
      ;(vec3 as any).toPxTransform(pose)
    }
  ...
  setQuaternion(quat) {
    ...
    const pose = this.actor.getGlobalPose()
    if (typeof (quat as any).toPxTransform === 'function') {
      ;(quat as any).toPxTransform(pose)
    }
    this.actor.setGlobalPose(pose)
  ...
  getVelocityAtPos(pos, vec3) {
    if (!pos?.isVector3) {
      throw new Error('[rigidbody] getVelocityAtPos pos must be Vector3')
    }
    if (!this.actor) {
      return vec3.set(0, 0, 0)
    }
  ...
  getLocalVelocityAtLocalPos(pos, vec3) {
    if (!pos?.isVector3) {
      throw new Error('[rigidbody] getVelocityAtLocalPos pos must be Vector3')
    }
    if (!this.actor) {
      return vec3.set(0, 0, 0)
    }
  ...
  setCenterOfMass(pos) {
    if (!pos?.isVector3) {
      throw new Error('[rigidbody] setCenterOfMass pos must be Vector3')
    }
  ...
  setKinematicTarget(position, quaternion) {
    ...
    if (typeof (position as any).toPxTransform === 'function') {
      ;(position as any).toPxTransform(this._tm)
    }
    if (typeof (quaternion as any).toPxTransform === 'function') {
      ;(quaternion as any).toPxTransform(this._tm)
    }
