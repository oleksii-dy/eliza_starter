...
export class Video extends Node {
  ...
  async mount() {
    ...
    if (screen) {
      ...
      if (!factory) {
        factory = await this.ctx.world.loader.load('video', this._src)
      }
      if (this.n !== n) {
        return
      }
