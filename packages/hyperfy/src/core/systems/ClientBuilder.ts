...
export class ClientBuilder extends System {
  ...
  update(_delta: number) {
    ...
    if (this.selected && this.mode === 'grab') {
      ...
      if (project) {
        ...
        if (this.target.limit! < PROJECT_MIN) {
          this.target.limit = PROJECT_MIN
        }
        if (hitDistance && this.target.limit! > hitDistance) {
          this.target.limit = hitDistance
        }
      }
  ...
  getHitAtReticle(ignoreEntity: AppEntity | null, ignorePlayers: boolean) {
    ...
    for (const _hit of hits) {
      ...
      if (entity === ignoreEntity || (entity?.isPlayer && ignorePlayers)) {
        continue
      }
      hit = _hit
      break
