...
export class ScenarioTestFramework {
   ...
   */
  private async validateVisuals(expectedVisuals: Array<{
    entityId: string;
    color: string;
    position: Vector3;
    visible: boolean;
    ...
    for (const expected of expectedVisuals) {
      try {
        const detected = await this.colorDetector.detectColorAtPosition(
          expected.position,
          expected.color
        );
  ...
  async checkEntityVisual(entityId: string, expectedColor: string, expectedPosition: Vector3): Promise<boolean> {
    try {
      ...
      const tolerance = 1.0;
      const positionMatch = Math.abs(entity.position.x - expectedPosition.x) < tolerance &&
                           Math.abs(entity.position.y - expectedPosition.y) < tolerance &&
                           Math.abs(entity.position.z - expectedPosition.z) < tolerance;

      if (!positionMatch) {
        this.log(`❌ Position mismatch for ${entityId}: expected ${JSON.stringify(expectedPosition)}, got ${JSON.stringify(entity.position)}`);
        return false;
      }
