...
export class BuildingProxySystem {
  ...
  createBuilding(config: BuildingConfig): BuildingProxy | null {
    try {
      ...
      if (interior) {this.buildingGroup.add(interior);}
      if (doorway) {this.buildingGroup.add(doorway);}
  ...
  private createInterior(config: BuildingConfig): THREE.Group {
    ...
    const floorGeometry = new THREE.PlaneGeometry(
      config.interiorSize!.width,
      config.interiorSize!.depth
    );
    ...
    floor.position.set(
      config.position.x,
      config.position.y - config.size.height / 2 + 0.1,
      config.position.z
    );
    ...
    backWall.position.set(
      config.position.x,
      config.position.y,
      config.position.z - config.interiorSize!.depth / 2
    );
    ...
    leftWall.position.set(
      config.position.x - config.interiorSize!.width / 2,
      config.position.y,
      config.position.z
    );
  ...
  private createTriggerZones(config: BuildingConfig): Map<string, TriggerZone> {
    ...
    for (const zoneConfig of config.triggerZones) {
      ...
      const mesh = new THREE.Mesh(geometry, material);
      mesh.position.set(
        zoneConfig.position.x,
        zoneConfig.position.y,
        zoneConfig.position.z
      );
  ...
  private spawnBuildingNPCs(config: BuildingConfig): void {
    if (!config.npcs) {return;}

    // This would integrate with the NPC system
    for (const npcConfig of config.npcs) {
      console.log(`[BuildingProxySystem] Would spawn NPC: ${npcConfig.name} (${npcConfig.type}) at building ${config.id}`);
  ...
  removeBuilding(buildingId: string): boolean {
    ...
    if (building.interior) {this.buildingGroup.remove(building.interior);}
    if (building.doorway) {this.buildingGroup.remove(building.doorway);}
    ...
    if (building.interior) {this.disposeGroup(building.interior);}
    if (building.doorway) {this.disposeMesh(building.doorway);}
