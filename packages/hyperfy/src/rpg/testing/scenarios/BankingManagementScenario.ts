  ...
  async condition(framework) {
    ...
    switch (testData.phase) {
      ...
      case 'test_complete':
        ...
        framework.log(`   ✓ Items deposited: ${testData.itemsDeposited ? 'YES' : 'NO'} (${testData.depositTransactions.length} types)`);
        framework.log(`   ✓ Items withdrawn: ${testData.itemsWithdrawn ? 'YES' : 'NO'} (${testData.withdrawTransactions.length} items)`);
        ...
        const allBankingFeaturesWorking = testData.bankOpened && testData.itemsDeposited && 
                                        testData.itemsWithdrawn && testData.dataValidated;
