...
export const MagicSpellCombatScenario: TestScenario = {
  ...
  async setup(framework) {
    ...
    const player = rpgHelpers.spawnPlayer('magic_test_player', {
      ...
      stats: {
        hitpoints: { current: 80, max: 80, level: 80, xp: 2000000 },
        defense: { level: 20, xp: 4470 },
        attack: { level: 10, xp: 1154 }, // Low melee for magic focus
        strength: { level: 10, xp: 1154 },
        magic: { level: 50, xp: 101333 }, // High magic level
        prayer: { level: 25, xp: 7842, points: 25, maxPoints: 25 },
      },
    ...
    const trainingDummy = rpgHelpers.spawnNPC('training_dummy', {
      ...
      statsOverride: {
        hitpoints: { current: 200, max: 200, level: 40, xp: 37224 },
        defense: { level: 10, xp: 1154 },
        magic: { level: 1, xp: 0 }, // Low magic defence
      },
    })
    ...
    const magicTarget = rpgHelpers.spawnNPC('wizard', {
      ...
      statsOverride: {
        hitpoints: { current: 100, max: 100, level: 50, xp: 101333 },
        defense: { level: 15, xp: 2411 },
        magic: { level: 25, xp: 7842 },
        attack: { level: 20, xp: 4470 },
      },
    })
  ...
  async condition(framework) {
    ...
    switch (testData.phase) {
        ...
        if (!testData.combatSpellsTested && trainingDummy) {
          ...
          if (currentMana >= spellToTest.mana && testData.spellsCast.length < 4) {
            ...
            const hasRunes = spellToTest.runes.every(runeType => {
              return playerInventory.items.some(
                item => item.metadata?.name?.toLowerCase().includes(runeType) && item.quantity > 0
              )
            })
      ...
      case 'accuracy_test':
        ...
        framework.log(
          `🎯 Phase 7: Testing spell accuracy vs mobile target... Hits: ${testData.spellAccuracy.hits}/${testData.spellAccuracy.attempts}`
        )
        ...
        if (magicTarget && testData.spellAccuracy.attempts < 10) {
          ...
          if (currentMana >= spellToTest.mana) {
            ...
            if (hit) {
              ...
              damage = Math.floor(Math.random() * 6) + 2 // 2-8 damage
      ...
      case 'test_complete':
        ...
        framework.log(
          `   🎯 Spell accuracy: ${testData.spellAccuracy.hits}/${testData.spellAccuracy.attempts} (${((testData.spellAccuracy.hits / testData.spellAccuracy.attempts) * 100).toFixed(1)}%)`
        )
        ...
        const spellVarietyTested =
          testData.combatSpellsTested && testData.utilitySpellsTested && testData.teleportTested
        const manaSystemWorking = testData.manaConsumed > 0
        const accuracyReasonable =
          testData.spellAccuracy.attempts > 0 && testData.spellAccuracy.hits / testData.spellAccuracy.attempts > 0.3
