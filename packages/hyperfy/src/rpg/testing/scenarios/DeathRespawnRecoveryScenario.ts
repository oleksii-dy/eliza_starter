    ...
    switch (testData.phase) {
        ...
        if (player && killer) {
          ...
          if (combatSystem) {
            ...
            if (killerEntity && playerEntity) {
              ...
              framework.log(`📊 Killer stats: ${killerStats ? `HP: ${killerStats.hitpoints?.current}/${killerStats.hitpoints?.max}` : 'MISSING'}`);
              framework.log(`📊 Player stats: ${playerStats ? `HP: ${playerStats.hitpoints?.current}/${playerStats.hitpoints?.max}` : 'MISSING'}`);
              ...
              if (killerPos && playerPos) {
                const distance = Math.sqrt(
                  Math.pow(killerPos.x - playerPos.x, 2) + 
                  Math.pow(killerPos.z - playerPos.z, 2)
                );
      ...
      case 'combat_started':
        ...
        if (player) {
          ...
          if (stats) {
            framework.log(`❤️ Player health: ${stats.hitpoints.current}/${stats.hitpoints.max}, In combat: ${combat?.inCombat || false}`);
      ...
      case 'items_recovered':
        ...
        if (finalPlayer) {
          ...
          const recoveredItems = finalInventory ? finalInventory.items.filter(item => 
      ...
      case 'test_complete':
        ...
        const allMechanicsWorking = testData.playerDied && testData.playerRespawned && 
                                  testData.gravestoneCreated && testData.itemsRecovered;
