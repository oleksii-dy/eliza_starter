  ...
  async condition(framework) {
    ...
    switch (testData.phase) {
      case 'setup_complete':
        ...
        if (inventory && inventory.items.length > 0) {
          ...
          itemsToDrop.forEach((item, index) => {
            ...
            const dropPosition = {
              x: testArea.dropZone.x + (index * 0.5),
              y: testArea.dropZone.y,
              z: testArea.dropZone.z
            };
      ...
      case 'items_dropped':
        ...
        if (movement) {
          movement.destination = testData.testArea.pickupZone;
        ...
        if (playerPos) {
          const distanceToPickup = Math.sqrt(
            Math.pow(playerPos.x - testData.testArea.pickupZone.x, 2) +
            Math.pow(playerPos.z - testData.testArea.pickupZone.z, 2)
          );
      ...
      case 'stacking_tested':
        ...
        if (inventory) {
          ...
          framework.log(`🎒 Inventory full test: ${inventoryFull ? 'PASSED' : 'FAILED'} (${inventory.items.length}/${inventory.maxSlots} slots)`);
        }
        return false;
      ...
      case 'test_complete':
        ...
        framework.log(`   ✓ Items dropped: ${testData.itemsDropped ? 'YES' : 'NO'} (${testData.dropTransactions.length} items)`);
        framework.log(`   ✓ Items picked up: ${testData.itemsPickedUp ? 'YES' : 'NO'} (${testData.pickupTransactions.length} items)`);
        ...
        const allPickupFeaturesWorking = testData.itemsDropped && testData.itemsPickedUp && 
                                       testData.stackingTested && testData.fullInventoryTested;
