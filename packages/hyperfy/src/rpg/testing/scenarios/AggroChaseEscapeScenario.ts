  ...
  async condition(framework) {
    ...
    switch (testData.phase) {
      ...
      case 'at_aggro_point':
        ...
        if (distance <= 8 && !testData.aggroTriggered) { // 8 tile aggro range
      ...
      case 'aggro_triggered':
        ...
        if (distance < 12 && !testData.chaseStarted) { // Increased range to account for mob movement
      ...
      case 'start_escape':
        ...
        if (playerMovement) {
          if (!playerMovement.destination || calculateDistance(playerMovement.destination, testData.waypoints.escapePosition) > 1) {
      ...
      case 'at_escape_position':
        ...
        framework.log(`⏳ Phase 5: Waiting for de-aggro... Mob distance: ${distance.toFixed(1)}, In combat: ${isInCombat}`);
        
        // Simulate mob losing aggro when player is far enough
        if (distance > 15 && !testData.playerEscaped) { // 15+ tiles = lost aggro
      ...
      case 'test_reaggro':
        ...
        if (reapproachMovement) {
          if (!reapproachMovement.destination || calculateDistance(reapproachMovement.destination, testData.waypoints.aggroTrigger) > 1) {
      ...
      case 'final_validation':
        ...
        testData.phases.push({ 
          ...
          time: elapsed,
          allMechanicsComplete: testData.aggroTriggered && testData.chaseStarted && 
                              testData.playerEscaped && testData.mobReturned && testData.reAggroTested
        });
      ...
      case 'test_complete':
        ...
        const allPhasesComplete = testData.aggroTriggered && testData.chaseStarted && 
                                 testData.playerEscaped && testData.mobReturned && testData.reAggroTested;
