  ...
  async condition(framework) {
    ...
    switch (testData.phase) {
      case 'setup_complete':
        ...
        if (stats && inventory) {
          const healthItem = inventory.items.find(item => 
            item.metadata?.type === 'health' || item.metadata?.name?.includes('brew')
          );
          ...
          if (healthItem && stats.hitpoints.current < stats.hitpoints.max) {
            ...
            framework.log(`❤️ Used ${healthItem.metadata.name}, healed ${actualHeal} HP (${oldHP} → ${stats.hitpoints.current})`);
          }
        }
        return false;
      ...
      case 'health_used':
        ...
        if (stats && inventory) {
          const prayerItem = inventory.items.find(item => 
            item.metadata?.type === 'prayer'
          );
          ...
          if (prayerItem && stats.prayer && stats.prayer.points < stats.prayer.maxPoints) {
            ...
            stats.prayer.points = Math.min(
              stats.prayer.maxPoints, 
              stats.prayer.points + restoreAmount
            );
            ...
            framework.log(`🙏 Used ${prayerItem.metadata.name}, restored ${actualRestore} prayer (${oldPrayer} → ${stats.prayer.points})`);
          }
        }
        return false;
      ...
      case 'prayer_used':
        ...
        if (stats && inventory) {
          const foodItem = inventory.items.find(item => 
            item.metadata?.type === 'food'
          );
          ...
          if (foodItem && stats.hitpoints.current < stats.hitpoints.max) {
            ...
            framework.log(`🍖 Ate ${foodItem.metadata.name}, healed ${actualHeal} HP (${oldHP} → ${stats.hitpoints.current})`);
          }
        }
        return false;
      ...
      case 'food_eaten':
        ...
        if (inventory) {
          const energyItem = inventory.items.find(item => 
            item.metadata?.type === 'energy'
          );
      ...
      case 'energy_used':
        ...
        if (playerPos) {
          ...
          if (distanceToCombat < 5) {
            ...
            if (inventory) {
              const foodItem = inventory.items.find(item => 
                item.metadata?.type === 'food' && item.quantity > 0
              );
      ...
      case 'combat_tested':
        ...
        if (inventory) {
          ...
          const foodItems = inventory.items.filter(item => 
            item.metadata?.type === 'food' && item.quantity > 0
          );
      ...
      case 'test_complete':
        ...
        if (testData.consumptionLog.length > 0) {
          ...
          testData.consumptionLog.forEach((log, index) => {
            framework.log(`   ${index + 1}. ${log.item} (${log.type}): ${log.healAmount || log.restoreAmount || 'effect'}`);
          });
        }

        // Validate overall success
        const allConsumablesFeaturesWorking = testData.healthPotionUsed && testData.prayerPotionUsed && 
                                            testData.foodEaten && testData.energyPotionUsed;
