...
import { Vector3, RPGEntity, SkillType, EquipmentSlot, InventoryComponent, StatsComponent, CombatComponent, MovementComponent, ResourceComponent, ItemComponent, NPCComponent, QuestComponent, ConstructionComponent, ConstructionSiteComponent, SkillsComponent } from '../../types/index';
...
export class WoodcuttingScenario extends BaseTestScenario {
      ...
      if (axeItem) {
        inventory.equipment[EquipmentSlot.WEAPON] = {
          ...
          equipment: {
            ...
            requirements: { woodcutting: { level: 1, xp: 0 } },
            bonuses: {
              attackStab: -2, attackSlash: 7, attackCrush: 2, attackMagic: 0, attackRanged: 0,
              defenseStab: 0, defenseSlash: 0, defenseCrush: 0, defenseMagic: 0, defenseRanged: 0,
              meleeStrength: 7, rangedStrength: 0, magicDamage: 0, prayerBonus: 0
            }
          },
  ...
  async validate(): Promise<boolean> {
    try {
      ...
      const hasLogs = inventory.items.some(item => 
        item?.metadata?.name === 'Logs' || item?.itemId === 1511 // Normal logs
      );
