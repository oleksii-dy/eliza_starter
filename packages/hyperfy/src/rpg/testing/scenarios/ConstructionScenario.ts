...
import { Vector3, RPGEntity, SkillType, RoomType, HotspotType, InventoryComponent, StatsComponent, CombatComponent, MovementComponent, ResourceComponent, ItemComponent, NPCComponent, QuestComponent, ConstructionComponent, ConstructionSiteComponent, SkillsComponent } from '../../types/index';
...
export class ConstructionScenario extends BaseTestScenario {
  ...
  private setupMaterialComponents(material: RPGEntity, materialData: { id: number; name: string; quantity: number }): void {
  ...
  private async attemptConstruction(hasSkill: boolean): Promise<boolean> {
    ...
    const hasAllMaterials = siteComponent.requirements.materials.every(req => {
      return inventory.items.some(item => 
        item?.itemId === req.itemId && item?.quantity >= req.quantity
      );
    });
    ...
    siteComponent.requirements.materials.forEach(req => {
      const itemSlot = inventory.items.findIndex(item => 
        item?.itemId === req.itemId && item?.quantity >= req.quantity
      );
