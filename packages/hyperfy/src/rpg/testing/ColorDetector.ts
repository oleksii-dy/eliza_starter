...
export class ColorDetector {
  ...
  async detectEntitiesInImage(imagePath: string): Promise<DetectedEntity[]> {
    try {
      ...
      for (const [entityType, hexColor] of Object.entries(ENTITY_COLOR_MAP)) {
        ...
        const positions = this.findColorPositions(
          imageData,
          width,
          height,
          targetColor
    ...
    for (let y = 0; y < height; y += step) {
      for (let x = 0; x < width; x += step) {
        ...
        if (distance <= this.config.colorTolerance) {
          const clusterInfo = this.analyzeCluster(
            imageData,
            width,
            height,
            x,
  ...
  private calculateEntityConfidence(
    ...
    const avgPositionConfidence = positions.reduce(
      (sum, pos) => sum + pos.confidence, 0
    ) / positions.length;
    ...
    const edgePositions = positions.filter(pos =>
      pos.x < 50 || pos.x > imageWidth - 50 ||
      pos.y < 50 || pos.y > imageHeight - 50
    ).length;
  ...
  async compareImages(
    ...
    for (const beforeEntity of beforeEntities) {
      ...
      if (afterEntity && beforeEntity.positions.length > 0 && afterEntity.positions.length > 0) {
        ...
        const distance = Math.sqrt(
          Math.pow(beforePos.x - afterPos.x, 2) +
          Math.pow(beforePos.y - afterPos.y, 2)
        );
   ...
   */
  private createEntityOverlaySVG(
    width: number,
    height: number,
