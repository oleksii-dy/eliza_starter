...
export class VisualResourceTest extends System {
  ...
  private createTestResourceVisual(
    ...
    mesh.position.set(
      position.x, 
      position.y + resourceDef.visual.scale / 2, 
      position.z
    );
    ...
    this.createFloatingLabel(resourceDef.name, {
      x: position.x,
  ...
  private logVisualTestResults(): void {
    ...
    for (const [resourceType, resourceDef] of Object.entries(RESOURCE_DEFINITIONS)) {
      const info = {
        ...
        metallic: resourceDef.visual.metalness > 0 ? '⚡' : '',
        rarity: resourceDef.rarity
      };
    ...
    for (const [category, resources] of Object.entries(categories)) {
      ...
      resources.forEach(resource => {
        console.log(`  ${resource.name} (Lvl ${resource.level}): ${resource.color} ${resource.emissive}${resource.metallic} [${resource.rarity}]`);
      });
      console.log('');
    }
  ...
  private testColorContrast(colors: string[]): void {
    ...
    for (let i = 0; i < colors.length; i++) {
      for (let j = i + 1; j < colors.length; j++) {
        ...
        if (similarity > 0.8) { // Very similar colors
  ...
  public generateVisualTestReport(): any {
    ...
    // Analyze categories
    for (const [resourceType, resourceDef] of Object.entries(RESOURCE_DEFINITIONS)) {
      if (resourceType.startsWith('tree_')) report.categories.trees++;
      else if (resourceType.startsWith('rock_')) report.categories.rocks++;
      else if (resourceType.startsWith('fishing_')) report.categories.fishing++;
