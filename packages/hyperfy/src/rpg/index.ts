...
export const HyperfyRPGPlugin = {
  ...
  coreSystems: [
    'combat', 'inventory', 'npc', 'loot', 'spawning', 'skills', 'movement', 'visualRepresentation', 
    'navigation', 'agentPlayer', 'resourceSpawning', 'enhancedSkills', 'enhancedMagic', 
    'advancedCombat', 'combatAnimation', 'smithing', 'cooking', 'equipment'
  ],

  // Optional systems (can be enabled/disabled)
  optionalSystems: [
    'quest', 'banking', 'trading', 'prayer', 'shop', 'magic', 'ranged', 'deathRespawn',
    'pvp', 'playerHomes', 'grandExchange', 'clan', 'construction', 'minigame', 'visualResourceTest', 'runeScapeDemo'
  ],
  ...
  allSystems: [
    ...
    { name: 'advancedCombat', System: () => import('./systems/combat/AdvancedCombatSystem').then(m => m.AdvancedCombatSystem) },
    { name: 'combatAnimation', System: () => import('./systems/combat/CombatAnimationSystem').then(m => m.CombatAnimationSystem) },
  ...
  inject: {
    world: {
      ...
      addRPGPlayer(entity: any, playerId: string, spawnPosition: any = null) {
        ...
        if (movementSystem) {movementSystem.initializePlayer(playerId, spawnPosition);}
        if (inventorySystem) {inventorySystem.createInventory(playerId);}
        if (skillsSystem) {skillsSystem.initializePlayerSkills(playerId);}
