...
export class RuneScapeDemo extends System {
  ...
  async initialize(): Promise<void> {
    ...
    this.resourceSystem =
      this.world.systems.find(s => s instanceof ResourceSpawningSystem) || new ResourceSpawningSystem(this.world)
    this.skillsSystem =
      this.world.systems.find(s => s instanceof EnhancedSkillsSystem) || new EnhancedSkillsSystem(this.world)
    this.magicSystem =
      this.world.systems.find(s => s instanceof EnhancedMagicSystem) || new EnhancedMagicSystem(this.world)
  ...
  private async demonstrateWoodcutting(): Promise<void> {
    ...
    for (const tree of trees) {
      // Give required level
      if (tree.level > 1) {
        this.skillsSystem.addExperience(this.demoPlayer.id, SkillType.WOODCUTTING, this.getXPForLevel(tree.level + 5))
      }
  ...
  private async demonstrateMining(): Promise<void> {
    ...
    for (const ore of ores) {
      if (ore.level > 1) {
        this.skillsSystem.addExperience(this.demoPlayer.id, SkillType.MINING, this.getXPForLevel(ore.level + 5))
      }
  ...
  private async demonstrateFishing(): Promise<void> {
    ...
    for (const fishSpot of fish) {
      if (fishSpot.level > 1) {
        this.skillsSystem.addExperience(this.demoPlayer.id, SkillType.FISHING, this.getXPForLevel(fishSpot.level + 5))
      }
  ...
  private async demonstrateUtilityMagic(): Promise<void> {
    const utilitySpells = [SpellType.LOW_LEVEL_ALCHEMY, SpellType.HIGH_LEVEL_ALCHEMY, SpellType.TELEKINETIC_GRAB]
  ...
  private async demonstrateTeleportMagic(): Promise<void> {
    const teleportSpells = [SpellType.VARROCK_TELEPORT, SpellType.LUMBRIDGE_TELEPORT, SpellType.FALADOR_TELEPORT]
  ...
  private logResourceDistribution(): void {
    ...
    for (const zone of zones) {
      ...
      console.log(
        `  🗺️ ${zone.name}: Trees=${counts.tree || 0}, Rocks=${counts.rock || 0}, Fishing=${counts.fishing || 0}`
      )
    }
  }
  ...
  private async visitZone(zoneType: ZoneType, coords: { x: number; z: number }): Promise<void> {
    ...
    console.log(
      `    Features: Bank=${zone.features.hasBank}, Shops=${zone.features.hasShops}, PvP=${zone.features.pvpEnabled}`
    )
  ...
  private logCharacterSummary(): void {
    ...
    // Find highest skills
    const skillLevels = Object.entries(skills.skills)
      .map(([skill, data]) => ({
        skill,
        level: data.level,
      }))
  ...
  private getXPForLevel(level: number): number {
    ...
    const xpTable = [
      0, 0, 83, 174, 276, 388, 512, 650, 801, 969, 1154, 1358, 1584, 1833, 2107, 2411, 2746, 3115, 3523, 3973, 4470,
