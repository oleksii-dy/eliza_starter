...
export class RPGTestingSystem extends System {
  ...
  private async createTestUI(): Promise<void> {
    try {
      ...
      const uiEntity = this.world.entities?.create ?
        this.world.entities.create('rpg-test-ui', uiEntityData) :
        null;
  ...
  spawnTestEntity(
    ...
    try {
      ...
      const entity = this.world.entities?.create ?
        this.world.entities.create(`test_${id}`, entityData) :
        null;
  ...
  private getGeometryForType(type: string): string {
    switch (type) {
      ...
      case 'building': return 'box';
      case 'waypoint': return 'sphere';
      case 'obstacle': return 'box';
      case 'item': return 'sphere';
      default: return 'box';
    }
  }
  ...
  async runBankingTest(): Promise<TestResult> {
    ...
    return this.runTestWithTimeout('banking_test', async () => {
      ...
      try {
        ...
        const bank = this.spawnTestEntity(
          'bank_main',
          'building',
          { x: 0, y: 1.5, z: 0 },
          '#FFD700',
        ...
        const player = this.spawnTestEntity(
          'player_banking',
          'player',
          { x: -5, y: 1, z: 0 },
          '#FF69B4'
  ...
  async runCombatTest(): Promise<TestResult> {
    ...
    return this.runTestWithTimeout('combat_test', async () => {
      ...
      try {
        ...
        // Spawn red goblin
        const goblin = this.spawnTestEntity(
          'goblin_enemy',
          'npc',
          { x: 3, y: 1, z: 0 },
          '#FF0000'
  ...
  private validateEntityVisuals(entityIds: string[]): number {
    ...
    entityIds.forEach(id => {
      ...
      const entity = this.testEntities.get(id) ||
                    (this.world.entities?.get ? this.world.entities.get(id) : null);
