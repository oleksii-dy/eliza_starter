...
export class EnhancedSkillsSystem extends System {
  ...
  private handleResourceHarvestComplete(data: any): void {
    ...
    const activeAction = Array.from(this.activeActions.values())
      .find(action => action.playerId === playerId && !action.completed);
  ...
  private getSkillForResource(resourceType: string): SkillType {
    if (resourceType.startsWith('tree_')) return SkillType.WOODCUTTING;
    if (resourceType.startsWith('rock_')) return SkillType.MINING;
    if (resourceType.startsWith('fishing_')) return SkillType.FISHING;
  ...
  public getActiveActions(entityId: string): SkillAction[] {
    return Array.from(this.activeActions.values())
      .filter(action => action.playerId === entityId && !action.completed);
  }
  ...
  public getTrainingRecommendations(entityId: string, skillType: SkillType): any[] {
    ...
    switch (skillType) {
      case SkillType.WOODCUTTING:
        if (level >= 1) recommendations.push({ method: 'Normal trees', xpRate: 25, location: 'Lumbridge' });
        if (level >= 15) recommendations.push({ method: 'Oak trees', xpRate: 37.5, location: 'Varrock' });
        if (level >= 30) recommendations.push({ method: 'Willow trees', xpRate: 67.5, location: 'Draynor' });
        if (level >= 45) recommendations.push({ method: 'Maple trees', xpRate: 100, location: 'Various' });
        if (level >= 60) recommendations.push({ method: 'Yew trees', xpRate: 175, location: 'Wilderness' });
        if (level >= 75) recommendations.push({ method: 'Magic trees', xpRate: 250, location: 'Wilderness' });
        break;
        
      case SkillType.MINING:
        if (level >= 1) recommendations.push({ method: 'Copper/Tin ore', xpRate: 17.5, location: 'Lumbridge' });
        if (level >= 15) recommendations.push({ method: 'Iron ore', xpRate: 35, location: 'Varrock' });
        if (level >= 30) recommendations.push({ method: 'Coal', xpRate: 50, location: 'Varrock' });
        if (level >= 40) recommendations.push({ method: 'Gold ore', xpRate: 65, location: 'Falador' });
        if (level >= 55) recommendations.push({ method: 'Mithril ore', xpRate: 80, location: 'Wilderness' });
        if (level >= 70) recommendations.push({ method: 'Adamant ore', xpRate: 95, location: 'Wilderness' });
        if (level >= 85) recommendations.push({ method: 'Runite ore', xpRate: 125, location: 'Wilderness' });
        break;
        
      case SkillType.FISHING:
        if (level >= 1) recommendations.push({ method: 'Shrimp (net)', xpRate: 10, location: 'Lumbridge' });
        if (level >= 20) recommendations.push({ method: 'Trout (bait)', xpRate: 50, location: 'Barbarian Village' });
        if (level >= 30) recommendations.push({ method: 'Salmon (bait)', xpRate: 70, location: 'Barbarian Village' });
        if (level >= 40) recommendations.push({ method: 'Lobster (cage)', xpRate: 90, location: 'Fishing Guild' });
        if (level >= 50) recommendations.push({ method: 'Swordfish (harpoon)', xpRate: 100, location: 'Fishing Guild' });
        if (level >= 76) recommendations.push({ method: 'Shark (harpoon)', xpRate: 110, location: 'Fishing Guild' });
        break;
    }
