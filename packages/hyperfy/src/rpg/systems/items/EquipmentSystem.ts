...
export class EquipmentSystem extends System {
  ...
  private consumeItem(playerId: string, itemId: string): boolean {
    ...
    if (!entity || !itemDef || !itemDef.consumable) return false;
  ...
  public swapEquippedItems(playerId: string, fromSlot: EquipmentSlot, toSlot: EquipmentSlot): boolean {
    ...
    if (fromItem && toItem) {
      ...
      if (fromItemDef && fromItemDef.equipmentSlot !== toSlot) return false;
      if (toItemDef && toItemDef.equipmentSlot !== fromSlot) return false;
    }
  ...
  private canPlayerEquipItem(playerId: string, itemDef: ItemDefinition): boolean {
    ...
    if (!skillsSystem || !itemDef.requirements) return true;
  ...
  private updateEquipmentBonuses(playerId: string): void {
    ...
    for (const equippedItem of Object.values(equipment.slots)) {
      ...
      if (itemDef.stats.attackBonus) equipment.bonuses.attackBonus += itemDef.stats.attackBonus;
      if (itemDef.stats.strengthBonus) equipment.bonuses.strengthBonus += itemDef.stats.strengthBonus;
      if (itemDef.stats.defenceBonus) equipment.bonuses.defenceBonus += itemDef.stats.defenceBonus;
      if (itemDef.stats.rangedBonus) equipment.bonuses.rangedBonus += itemDef.stats.rangedBonus;
      if (itemDef.stats.rangedDefence) equipment.bonuses.rangedDefence += itemDef.stats.rangedDefence;
      if (itemDef.stats.magicBonus) equipment.bonuses.magicBonus += itemDef.stats.magicBonus;
      if (itemDef.stats.magicDefence) equipment.bonuses.magicDefence += itemDef.stats.magicDefence;
      if (itemDef.stats.prayer) equipment.bonuses.prayer += itemDef.stats.prayer;
  ...
  private updateCombatLevel(playerId: string): void {
    ...
    if (!equipment || !skillsSystem) return;
  ...
  public getAllEquippedItems(playerId: string): Record<EquipmentSlot, EquippedItem | null> {
    const entity = this.world.getEntityById(playerId);
    if (!entity) return {} as Record<EquipmentSlot, EquippedItem | null>;
