...
export class ConstructionSystem extends System {
  ...
  enterHouse(playerId: string, ownerId?: string): boolean {
    ...
    if (house.settings.locked && playerId !== targetOwnerId &&
        !house.visitors.includes(playerId)) {
      this.emit('construction:error', {
        playerId,
  ...
  buildRoom(
    ...
    const house = this.houses.get(houseId);
    if (!house || !house.settings.buildMode) {return false;}

    const player = this.world.entities.get(playerId) as PlayerEntity;
    if (!player) {return false;}

    const constructionComponent = player.getComponent<ConstructionComponent>('construction');
    if (!constructionComponent) {return false;}

    // Check room requirements
    const roomRequirements = this.roomCosts.get(roomType);
    if (!roomRequirements) {return false;}
  ...
  interactWithFurniture(
    ...
    const constructionComponent = player.getComponent<ConstructionComponent>('construction');
    if (!constructionComponent || !constructionComponent.inHouse) {return;}
    ...
    // Get furniture
    const furniture = room.furniture.get(hotspotId);
    if (!furniture || !furniture.interactable) {return;}
  ...
  dismissServant(playerId: string): boolean {
    ...
    const house = this.houses.get(houseId);
    if (!house || house.servant.type === 'none') {return false;}
  ...
  updateHouseSettings(
    playerId: string,
    settings: Partial<HouseSettings>
  ): boolean {
    const houseId = this.playerHouses.get(playerId);
    if (!houseId) {return false;}
  ...
  private wouldDisconnectRooms(
    _house: PlayerHouse,
    _position: { floor: number; x: number; z: number }
  ): boolean {
    ...
    switch (effect.type) {
      ...
      case 'restore':
        ...
        if (restoreStats && restoreStats.prayer) {
          ...
          restoreStats.prayer.points = Math.min(restoreStats.prayer.maxPoints + 20, restoreStats.prayer.points + restored);
        }
        break;
