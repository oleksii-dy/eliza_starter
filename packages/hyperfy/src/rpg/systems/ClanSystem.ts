...
export class ClanSystem extends System {
  ...
  promoteMember(promoterId: string, targetId: string): boolean {
    ...
    if (!promoter || !target) {return false;}
  ...
  startClanWar(
    ...
    for (const war of this.clanWars.values()) {
      if (war.status === 'active' || war.status === 'pending') {
        if (war.clan1Id === initiatorClanId || war.clan2Id === initiatorClanId ||
            war.clan1Id === targetClanId || war.clan2Id === targetClanId) {
          this.emit('clan:error', {
            playerId: initiatorId,
  ...
  joinClanWar(playerId: string, warId: string): boolean {
    ...
    if (!war || war.status !== 'pending') {return false;}

    const playerClanId = this.playerClans.get(playerId);
    if (!playerClanId) {return false;}
    ...
    if (stats && stats.combatLevel && war.rules.combatLevelRange) {
      ...
      if (combatLevel < war.rules.combatLevelRange[0] ||
          combatLevel > war.rules.combatLevelRange[1]) {
        this.emit('clan:error', {
          playerId,
  ...
  searchClans(query: string): Clan[] {
    ...
    for (const clan of this.clans.values()) {
      if (clan.settings.joinType === 'closed') {continue;}
  ...
  private cleanupInactiveMembers(): void {
    ...
    for (const clan of this.clans.values()) {
      if (clan.settings.kickInactiveDays <= 0) {continue;}
  ...
  private validateClanName(name: string): boolean {
    if (name.length < this.MIN_CLAN_NAME_LENGTH ||
        name.length > this.MAX_CLAN_NAME_LENGTH) {
      return false;
    }
  ...
  private validateClanTag(tag: string): boolean {
    if (tag.length < this.MIN_CLAN_TAG_LENGTH ||
        tag.length > this.MAX_CLAN_TAG_LENGTH) {
      return false;
    }
  ...
  private updatePlayerClanComponent(
    playerId: string,
    clanId: string | null,
    rank: ClanRank | null
  ): void {
  ...
  private disbandClan(clanId: string): void {
    ...
    for (const [warId, war] of this.clanWars) {
      if ((war.clan1Id === clanId || war.clan2Id === clanId) &&
          (war.status === 'pending' || war.status === 'active')) {
