...
export class ResourceSpawningSystem extends System {
  ...
  private generateSpawnersForResource(
    zoneType: ZoneType, 
    zoneDef: any, 
    resourceDist: any
  ): void {
  ...
  private findValidSpawnPosition(zoneDef: any, resourceDist: any): { x: number; y: number; z: number } | null {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (resourceDist.preferredAreas && resourceDist.preferredAreas.length > 0 && Math.random() < 0.7) {
        ...
        const preferredArea = resourceDist.preferredAreas[
          Math.floor(Math.random() * resourceDist.preferredAreas.length)
        ];
      ...
      if (x < zoneDef.bounds.minX || x > zoneDef.bounds.maxX ||
          z < zoneDef.bounds.minZ || z > zoneDef.bounds.maxZ) {
        continue;
      }
  ...
  private spawnInitialResources(): void {
    ...
    for (const spawner of this.resourceSpawners.values()) {
      if (Math.random() < 0.8) { // 80% chance to spawn initially
  ...
  private completeHarvest(entityId: string, playerId: string): void {
    const resource = this.resourceEntities.get(entityId);
    if (!resource || !resource.harvestInProgress) return;
  ...
  public getResourcesByType(resourceType: ResourceType): ResourceEntity[] {
    return Array.from(this.resourceEntities.values())
      .filter(resource => resource.resourceType === resourceType && !resource.depleted);
  }
