...
import { 
  HandPlacementDetector, 
  GeometryData, 
  Vector3, 
  Quaternion, 
  WeaponAnalysis 
} from '../HandPlacementDetector';
import { 
  WeaponOrientationSystem, 
  WeaponConfiguration, 
  AttachmentPoint 
} from '../WeaponOrientationSystem';
...
describe('Advanced Weapon Placement Tests', () => {
    ...
    it('should handle realistic sword proportions and placement', async () => {
      ...
      const realisticSwordGeometry: GeometryData = {
        vertices: new Float32Array([
          ...
          -0.025, -0.6, -0.0015,   // Blade tip
           0.025, -0.6,  0.0015,
          -0.025,  0.35, -0.0015,  // Blade base
           0.025,  0.35,  0.0015,
          
          // Crossguard (20cm wide, 2cm thick)
          -0.1,   0.35, -0.01,     // Guard left
           0.1,   0.35,  0.01,     // Guard right
          -0.1,   0.37, -0.01,
           0.1,   0.37,  0.01,
          
          // Handle (15cm long, 3cm diameter)
          -0.015, 0.37, -0.015,    // Handle bottom
           0.015, 0.37,  0.015,
          -0.015, 0.52, -0.015,    // Handle top
           0.015, 0.52,  0.015,
          
          // Pommel (5cm diameter)
          -0.025, 0.52, -0.025,    // Pommel
           0.025, 0.52,  0.025,
          -0.025, 0.55, -0.025,
           0.025, 0.55,  0.025,
        ]),
        faces: new Uint32Array([
          ...
          0, 1, 2, 1, 3, 2,  // Blade
          4, 5, 6, 5, 7, 6,  // Guard
          8, 9, 10, 9, 11, 10, // Handle
          12, 13, 14, 13, 15, 14, // Pommel
        ]),
      };

      const analysis = await handDetector.analyzeWeapon(
        realisticSwordGeometry,
        'sword',
        'realistic medieval longsword'
      );
      ...
      // Grip should be positioned on the handle area (between 0.37 and 0.52)
      expect(primaryGrip.position.y).toBeGreaterThan(0.25);
      expect(primaryGrip.position.y).toBeLessThan(0.55);
      ...
      console.log(`  Grip position: (${primaryGrip.position.x.toFixed(3)}, ${primaryGrip.position.y.toFixed(3)}, ${primaryGrip.position.z.toFixed(3)})`);
    ...
    it('should handle complex bow geometry with string placement', async () => {
      ...
      const bowGeometry: GeometryData = {
        vertices: new Float32Array([
          ...
          -0.03, -0.8, 0,    // Lower limb tip
          -0.02, -0.4, 0,    // Lower limb middle
          -0.025, 0, 0,       // Handle center
          -0.02, 0.4, 0,     // Upper limb middle
          -0.03, 0.8, 0,     // Upper limb tip
      ...
      console.log(`  Bow hand: (${bowHand!.position.x.toFixed(3)}, ${bowHand!.position.y.toFixed(3)}, ${bowHand!.position.z.toFixed(3)})`);
      console.log(`  String hand: (${stringHand!.position.x.toFixed(3)}, ${stringHand!.position.y.toFixed(3)}, ${stringHand!.position.z.toFixed(3)})`);
    });
    ...
    it('should analyze great axe with appropriate two-handed placement', async () => {
      ...
      const greatAxeGeometry: GeometryData = {
        vertices: new Float32Array([
          ...
          -0.15, 0.6, -0.05,   // Blade left
           0.15, 0.6,  0.05,   // Blade right
          -0.15, 0.8, -0.05,   // Blade top left
           0.15, 0.8,  0.05,   // Blade top right
          -0.05, 0.6, -0.05,   // Blade root left
           0.05, 0.6,  0.05,   // Blade root right
          
          // Long handle (1.5m)
          -0.025, -0.9, -0.025, // Handle bottom
           0.025, -0.9,  0.025,
          -0.025,  0.6, -0.025, // Handle top
           0.025,  0.6,  0.025,
        ]),
        faces: new Uint32Array([
          0, 1, 2, 1, 3, 2,    // Blade
          4, 5, 0, 5, 1, 0,    // Blade attachment
          6, 7, 8, 7, 9, 8,    // Handle
        ]),
      };
      ...
      const twoHandedAnalysis = await handDetector.analyzeWeapon(
        greatAxeGeometry,
        'axe',
        'two-handed battle axe'
      );
      ...
      console.log(`  Primary: (${primaryGrip!.position.x.toFixed(3)}, ${primaryGrip!.position.y.toFixed(3)}, ${primaryGrip!.position.z.toFixed(3)})`);
      console.log(`  Secondary: (${secondaryGrip!.position.x.toFixed(3)}, ${secondaryGrip!.position.y.toFixed(3)}, ${secondaryGrip!.position.z.toFixed(3)})`);
    });
  });
  ...
  describe('Attachment Point Systems', () => {
    ...
    it('should calculate proper sheathed positions relative to character bones', async () => {
      ...
      console.log(`Sheathed position: (${sheathedAttachment!.position.x.toFixed(3)}, ${sheathedAttachment!.position.y.toFixed(3)}, ${sheathedAttachment!.position.z.toFixed(3)})`);
  ...
  describe('Weapon Orientation and Physics', () => {
    it('should generate realistic weapon orientations for different combat states', async () => {
      ...
      console.log(`  Rest: (${orientation.restPosition.x.toFixed(3)}, ${orientation.restPosition.y.toFixed(3)}, ${orientation.restPosition.z.toFixed(3)}, ${orientation.restPosition.w.toFixed(3)})`);
      console.log(`  Combat: (${orientation.combatPosition.x.toFixed(3)}, ${orientation.combatPosition.y.toFixed(3)}, ${orientation.combatPosition.z.toFixed(3)}, ${orientation.combatPosition.w.toFixed(3)})`);
    });
    ...
    it('should calculate realistic physics properties for different weapon sizes', async () => {
      ...
      expect(largeSwordConfig.physicsProperties.inertiaTensor.x).toBeGreaterThan(daggerConfig.physicsProperties.inertiaTensor.x);

      console.log(`Physics comparison:`);
      console.log(`  Dagger mass: ${daggerConfig.physicsProperties.mass.toFixed(3)}kg, damage: ${daggerConfig.metadata.damage}`);
      console.log(`  Large sword mass: ${largeSwordConfig.physicsProperties.mass.toFixed(3)}kg, damage: ${largeSwordConfig.metadata.damage}`);
    });
    ...
    it('should handle asymmetric weapon design (katana)', async () => {
      ...
      const katanaGeometry: GeometryData = {
        vertices: new Float32Array([
          ...
          -0.015, -0.7, 0,     // Tip
           0.015, -0.7, 0,
          -0.02,  -0.3, 0.01,  // Curve start
           0.02,  -0.3, 0.01,
          -0.025,  0.3, 0.02,  // Handle start
           0.025,  0.3, 0.02,
          
          // Tsuka (handle) - longer than European sword
          -0.015,  0.3, 0,     // Handle start
           0.015,  0.3, 0,
          -0.015,  0.6, 0,     // Handle end
           0.015,  0.6, 0,
        ]),
        faces: new Uint32Array([
          0, 1, 2, 1, 3, 2,    // Blade
          2, 3, 4, 3, 5, 4,    // Guard area
          6, 7, 8, 7, 9, 8,    // Handle
        ]),
      };
      ...
      expect(grip.position.y).toBeGreaterThan(0.25);
      expect(grip.position.y).toBeLessThan(0.65);

      console.log(`Katana grip position: (${grip.position.x.toFixed(3)}, ${grip.position.y.toFixed(3)}, ${grip.position.z.toFixed(3)})`);
    });
