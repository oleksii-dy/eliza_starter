...
describe('AI Creation System Integration', () => {
  ...
  describe('Complete Weapon Creation Pipeline', () => {
    const mockSwordGeometry: GeometryData = {
      vertices: new Float32Array([
        ...
        -0.02, -0.8, -0.01,  // Bottom left
         0.02, -0.8,  0.01,  // Bottom right
        -0.02,  0.8, -0.01,  // Top left
         0.02,  0.8,  0.01,  // Top right
        // Hilt vertices
        -0.05, -0.2, -0.02,  // Hilt left
         0.05, -0.2,  0.02,  // Hilt right
        -0.05, -0.1, -0.02,  // Hilt top left
         0.05, -0.1,  0.02,  // Hilt top right
      ]),
    ...
    it('should create weapon from text with complete analysis and configuration', async () => {
      ...
      const weaponResult = await meshyService.createItem(
    ...
    it('should handle two-handed weapon analysis correctly', async () => {
      ...
      // Create staff
      const staffResult = await meshyService.createItem(
    ...
    it('should create character with rigging pipeline', async () => {
      ...
      const characterResult = await meshyService.createCharacter(
    ...
    it('should create building structures correctly', async () => {
      ...
      const buildingResult = await meshyService.createBuilding(
    ...
    it('should handle batch weapon creation', async () => {
      ...
      (global.fetch as jest.Mock)
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ result: 'sword-task' }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ result: 'axe-task' }),
        })
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ result: 'bow-task' }),
        });
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(
        meshyService.createItem('test item', 'weapon', 'sword')
      ).rejects.toThrow('Failed to create 3D model from text: Meshy API error: 500 Internal Server Error');
    ...
    it('should analyze weapon proportions and confidence correctly', async () => {
      const perfectSwordGeometry: GeometryData = {
        vertices: new Float32Array([
          ...
          -0.02, -0.8, -0.01,  // Blade bottom
           0.02, -0.8,  0.01,
          -0.02,  0.8, -0.01,  // Blade top
           0.02,  0.8,  0.01,
          -0.05, -0.2, -0.02,  // Hilt
           0.05, -0.2,  0.02,
        ]),
  ...
  describe('Physics and Metadata Calculations', () => {
    it('should calculate realistic physics properties', async () => {
      ...
      expect(config.physicsProperties.mass).toBeGreaterThan(1.0);

      // Damage should scale with size
      expect(config.metadata.damage).toBeGreaterThan(25); // Base sword damage
