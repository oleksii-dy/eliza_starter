...
export class HandPlacementDetector {
  ...
  async analyzeWeapon(
    ...
    console.log(`   Grips found: ${grips.length}, Primary at: (${grips[0]?.position.x.toFixed(2)}, ${grips[0]?.position.y.toFixed(2)}, ${grips[0]?.position.z.toFixed(2)})`);
   ...
   */
  private calculateAttachmentPoint(
    boundingBox: BoundingBox,
    grips: HandGrip[],
    heuristic: WeaponHeuristic
  ): Vector3 {
   ...
   */
  private calculateOrientationQuaternion(
    primaryAxis: 'x' | 'y' | 'z',
  ...
  async batchAnalyze(
    ...
    for (const weapon of weaponData) {
      try {
        const analysis = await this.analyzeWeapon(
          weapon.geometryData,
          weapon.weaponType,
          weapon.description
        );
