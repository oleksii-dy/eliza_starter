...
class PerformanceTest {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.serverProcess.stdout.on('data', (data) => {
        ...
        if (output.includes('running on port 4444')) {
          ...
          if (systemsLoaded >= expectedSystems * 0.9) { // 90% threshold
  ...
  async testSpawningPerformance() {
    ...
    return new Promise((resolve) => {
      ...
      const dataHandler = (data) => {
        ...
        // Count entity spawns
        if (output.includes('Spawned') && (output.includes('npc') || output.includes('resource') || output.includes('chest'))) {
          entitiesSpawned++;
        }
        ...
        if (output.includes('All test spawners activated')) {
          ...
          this.logTest('Spawner Creation', 'PASSED', `${spawnersCreated} spawners in ${this.metrics.spawnerCreationTime}ms`);
  ...
  async testMemoryUsage() {
    ...
    try {
      ...
      } else if (currentMemory.heapUsed < 200 * 1024 * 1024) { // 200MB
  ...
  async testConcurrentOperations() {
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        ...
        this.logTest('Concurrent Operations', 'PASSED', 
