...
class QuestSystemValidator {
  ...
  async testSystemAvailability() {
    ...
    const requiredSystems = [
      'quest', 'navigation', 'agentPlayer', 'spawning', 'visualRepresentation', 'npc'
    ];
  ...
  async testAgentBehavior() {
    ...
    return new Promise((resolve) => {
      const monitorInterval = setInterval(() => {
        ...
        actionsCompleted = this.testResults.filter(r => 
          r.test.startsWith('Agent Action') && r.status === 'INFO'
        ).length;

        if (actionsCompleted >= expectedActions.length * 0.7) { // 70% completion threshold
  ...
  async testQuestProgression() {
    ...
    const questEvents = this.testResults.filter(r => 
      r.test.includes('Quest') || r.description.includes('quest')
    );
  ...
  async testPerformance() {
    ...
    const systemCount = this.testResults.filter(r => 
      r.test.includes('System Init') && r.status === 'PASSED'
    ).length;

    const errorCount = this.testResults.filter(r => 
      r.status === 'ERROR'
    ).length;

    this.logTest('System Performance', systemCount >= 5 ? 'PASSED' : 'WARNING', 
