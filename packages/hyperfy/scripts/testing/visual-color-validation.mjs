  ...
  async runServerWithScreenshots() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.serverProcess.stdout.on('data', data => {
        ...
        if (
          output.includes('Successfully created NPC') ||
    ...
    try {
      ...
      for (const screenshot of this.screenshots) {
        ...
        for (const [entityType, colorInfo] of Object.entries(this.colorMap)) {
          ...
          if (pixelCount >= 4) {
            this.logTest(`Color: ${entityType}`, 'PASSED', `${colorInfo.name} found with ${pixelCount} pixels`)
          } else {
            this.logTest(`Color: ${entityType}`, 'FAILED', `${colorInfo.name} only ${pixelCount} pixels (need ≥4)`)
          }
        }
