  ...
  try {
    ...
    const results = await page.evaluate(async () => {
      ...
      if (spawningSystem) {
        verification.spawning = {
          systemLoaded: true,
          spawners: spawningSystem.spawners
            ? Array.from(spawningSystem.spawners.values()).map(spawner => ({
                id: spawner.id,
            ...
            : [],
          activeSpawns: spawningSystem.activeSpawns
            ? Array.from(spawningSystem.activeSpawns.entries()).map(([entityId, spawnerId]) => ({
                entityId,
                spawnerId,
              }))
            : [],
    ...
    if (results.spawning.spawners && results.spawning.spawners.length > 0) {
      ...
      results.spawning.spawners.forEach(spawner => {
        console.log(
          `     ${spawner.id}: ${spawner.isActive ? 'ACTIVE' : 'INACTIVE'} at [${spawner.position.x}, ${spawner.position.y}, ${spawner.position.z}]`
        )
        console.log(
          `       Entities: ${spawner.activeEntities}/${spawner.maxEntities}, Types: ${spawner.entityDefinitions.map(def => def.entityType).join(', ')}`
        )
      })
    }
    ...
    if (results.npcs.npcs && results.npcs.npcs.length > 0) {
      ...
      results.npcs.npcs.forEach(npc => {
        ...
        console.log(
          `     ${npc.name || npc.id}: ${npc.type} at [${pos?.x?.toFixed(2) || 'N/A'}, ${pos?.y?.toFixed(2) || 'N/A'}, ${pos?.z?.toFixed(2) || 'N/A'}]`
        )
    ...
    if (results.visuals.visuals && results.visuals.visuals.length > 0) {
      ...
      results.visuals.visuals.forEach(visual => {
        ...
        if (pos) {
          console.log(
            `       Position: [${pos.x.toFixed(2)}, ${pos.y.toFixed(2)}, ${pos.z.toFixed(2)}], Visible: ${visual.visible ? 'YES' : 'NO'}`
          )
        }
      })
    }
    ...
    console.log('\n🏁 OVERALL ASSESSMENT:')
    console.log(
      `   All Systems Loaded: ${results.summary.allSystemsLoaded ? '✅' : '❌'} (${systemsOk}/${Object.keys(results.systems).length})`
    )
    console.log(`   Spawning Working: ${results.summary.spawningWorking ? '✅' : '❌'}`)
    console.log(`   Visuals Working: ${results.summary.visualsWorking ? '✅' : '❌'}`)
    console.log(
      `   NPCs with Positions: ${results.npcs.npcs ? results.npcs.npcs.filter(npc => npc.position && npc.position.x !== undefined).length : 0}`
    )
    ...
    // Final verdict
    const allWorking =
      results.summary.allSystemsLoaded && results.summary.spawningWorking && results.summary.visualsWorking
