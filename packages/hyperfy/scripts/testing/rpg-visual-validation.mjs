...
async function runRPGVisualValidation(options = {}) {
  const { headless = false, timeout = 45000, tolerance = 30, saveResults = true, debug = false } = options
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        return window.world && window.world.getSystem && window.world.graphics && window.world.graphics.renderer
      },
      { timeout: 30000 }
    )
      ...
      async (tolerance, debug) => {
        ...
        window.RPGVisualTester = {
          createRPGMob(name, color, position, type = 'enemy', size = 1.2) {
            ...
            console.log(
              `[RPG Test] Added ${name} to scene with color ${color.toString(16)} at position:`,
        ...
        const rpgMobs = [{ name: 'Test Cube', color: 0xff0000, position: [0, 0, -5], type: 'test' }]
        ...
        const mobResults = rpgMobs.map((mob, index) => {
          ...
          console.log(
            `[RPG Test] ${mob.name}: Expected ${mob.color.toString(16)}, Best match at ${bestMatch.point}: ${bestMatch.pixelColor.hex}, Diffs: R${bestMatch.differences.r} G${bestMatch.differences.g} B${bestMatch.differences.b}`
          )
    ...
    results.mobResults.forEach(result => {
      ...
      if (!result.passed) {
        ...
        console.log(
          `     Position: [${result.position.join(', ')}] -> Screen[${result.screenPosition.x}, ${result.screenPosition.y}]`
        )
