...
async function validateRPGColors() {
  ...
  const validationResults = await page.evaluate(() => {
    ...
    const results = rpgObjects.map(obj => {
      ...
      const actualDominant =
        pixelColor.r > pixelColor.g && pixelColor.r > pixelColor.b
          ? 'red'
          : pixelColor.g > pixelColor.r && pixelColor.g > pixelColor.b
            ? 'green'
            : pixelColor.b > pixelColor.r && pixelColor.b > pixelColor.g
              ? 'blue'
              : 'mixed'

      const expectedDominant =
        expectedR > expectedG && expectedR > expectedB
          ? 'red'
          : expectedG > expectedR && expectedG > expectedB
            ? 'green'
            : expectedB > expectedR && expectedB > expectedG
              ? 'blue'
              : 'mixed'
  ...
  validationResults.results.forEach(result => {
    const statusIcon = result.verdict === 'PASS' ? '✅' : result.verdict === 'ACCEPTABLE' ? '🟡' : '❌'
    ...
    if (result.verdict !== 'PASS') {
      console.log(
        `   Color differences: R:${result.differences.r}, G:${result.differences.g}, B:${result.differences.b}`
      )
    }
  })
