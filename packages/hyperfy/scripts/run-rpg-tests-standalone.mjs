...
async function testBankingSystem() {
  ...
  const success = banking.bankItems[0]?.itemId === 995 && 
...
async function testMovementSystem() {
  ...
  for (const waypoint of waypoints) {
    ...
    const distance = Math.sqrt(
      Math.pow(movement.position.x - waypoint.x, 2) +
      Math.pow(movement.position.z - waypoint.z, 2)
    );
...
async function runStandaloneTests() {
  ...
  console.log('\n' + '='.repeat(50));
  console.log('📈 Overall Results:', `${passed}/${tests.length} tests passed (${((passed/tests.length)*100).toFixed(1)}%)`);
  console.log('');
