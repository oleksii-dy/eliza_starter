...
class RPGTestScenarios {
  ...
  async validatePlayerMovement(screenshots, setupData) {
    ...
    const passed = playerStart && playerEnd && 
    ...
    return {
      ...
      details: {
        ...
        movementDistance: playerStart && playerEnd ? 
          Math.abs(playerEnd.positions[0].x - playerStart.positions[0].x) : 0,
  ...
  async validateCombat(screenshots, setupData) {
    ...
    const hasInitialEntities = results[0].some(entities => 
      entities.find(e => e.type === 'player') && entities.find(e => e.type === 'goblin')
    );
    ...
    for (const [scenarioKey, scenario] of Object.entries(RPG_SCENARIOS)) {
      ...
      const screenshotInterval = setInterval(async () => {
        ...
        const screenshotPath = join(
          RPG_TEST_CONFIG.SCREENSHOT_DIR, 
          `${scenarioKey}_${timestamp}.png`
        );
      ...
      // Get video path
      await context.close();
      const videoPath = RPG_TEST_CONFIG.ENABLE_VIDEO ? 
        join(RPG_TEST_CONFIG.VIDEO_DIR, `${scenarioKey}.webm`) : null;
                ...
                <div class="scenario-content">
                    ...
                    ${scenario.screenshots ? `
                        ...
                        <div class="screenshot-grid">
                            ${scenario.screenshots.slice(-3).map((screenshot, index) => `
