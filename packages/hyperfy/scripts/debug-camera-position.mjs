...
async function runCameraDebugTest() {
  ...
  try {
    ...
    page.on('console', msg => {
      const text = msg.text()
      if (
        text.includes('[RPGTestHelpers]') ||
        text.includes('[VisualRepresentationSystem]') ||
        text.includes('[CameraDebug]')
      ) {
    ...
    for (let i = 0; i < testPositions.length; i++) {
      ...
      await page.evaluate(testData => {
        ...
        try {
          ...
          console.log(
            `[CameraDebug] Spawned ${testData.name} at ${JSON.stringify(testData.pos)} with color ${testData.color}`
          )
        } catch (error) {
