...
export class AgentRuntime implements IAgentRuntime {
  ...
  public currentActionRunState: ActionResult | null = null;
  ...
  async processActions(
    ...
    this.currentActionRunState = { values: {}, data: {}, text: '' };
    ...
    for (const response of responses) {
      ...
      for (const responseAction of actions) {
        // Get the latest state, which will now include previous action state via the new provider
        const currentState = await this.composeState(message, null, false, true);
        ...
        try {
          ...
          const result = await action.handler(this, message, currentState, {}, callback, responses);
          this.logger.debug(`Success: Action ${action.name} executed successfully.`);

          // If the action returned a state, merge it into the run's accumulated action state
          if (result) {
            this.logger.debug(`Action ${action.name} returned state`, result);

            // Deep merge is safer but for now a shallow merge will do.
            // A proper deep merge utility should be used if nested state becomes common.
            this.currentActionRunState.values = {
              ...this.currentActionRunState.values,
              ...result.values,
            };
            this.currentActionRunState.data = {
              ...this.currentActionRunState.data,
              ...result.data,
            };
            if (result.text) {
              this.currentActionRunState.text = (
                this.currentActionRunState.text ? `${this.currentActionRunState.text}\n` : ''
              ).concat(result.text);
            }
          }
    ...
    // Clean up the accumulated state after the entire action chain has been processed.
    this.currentActionRunState = null;
  }
