...
var paymentReceiveTransactionScenario = {
    ...
    evaluator: function (response) {
        var hasWalletInfo = response.toLowerCase().includes('wallet') ||
            response.toLowerCase().includes('address') ||
            response.toLowerCase().includes('balance');
        var hasTransactionInfo = response.toLowerCase().includes('transaction') ||
...
exports.default = paymentReceiveTransactionScenario;
