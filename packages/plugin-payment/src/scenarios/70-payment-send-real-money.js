...
var paymentSendRealMoneyScenario = {
    ...
    evaluator: function (response) {
        var hasPaymentIntent = response.toLowerCase().includes('send') ||
            response.toLowerCase().includes('transfer') ||
            response.toLowerCase().includes('processing');
        var mentionsAmount = /\d+(\.\d+)?\s*(ETH|USDC|SOL|MATIC)/i.test(response);
        var hasAddress = /0x[a-fA-F0-9]{40}/.test(response) || // Ethereum address
...
exports.default = paymentSendRealMoneyScenario;
