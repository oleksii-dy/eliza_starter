...
var polymarketBetPlacementScenario = {
    ...
    evaluator: function (response) {
        var hasBetIntent = response.toLowerCase().includes('bet') ||
            response.toLowerCase().includes('placing') ||
            response.toLowerCase().includes('order');
        var hasPolymarket = response.toLowerCase().includes('polymarket');
        var hasAmount = /\d+\s*(USDC|USD|\$)/i.test(response);
        var hasOdds = /\$0\.\d+|odds|shares|price/i.test(response);
        return hasBetIntent && hasPolymarket && hasAmount && hasOdds;
    },
    tags: ['payment', 'polymarket', 'bet', 'prediction', 'trading'],
};
exports.default = polymarketBetPlacementScenario;
