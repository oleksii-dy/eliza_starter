...
export function createProgram(): Command {
  ...
  program
    ...
    .option('-t, --temperature <temp>', 'Temperature (0-1)', (value) => {
    ...
    .option('-M, --max-tokens <tokens>', 'Max tokens', (value) => {
  ...
  // Memorize command
  program
    .command('memorize <content...>')
    .description('Add content to CLAUDE.md memory')
    .option('-u, --user', 'Save to user-level CLAUDE.md')
    .action(async (content: string[], options) => {
      const text = content.join(' ');
      await handleMemorizeCommand(text, options.user ? 'user' : 'project');
    });

  // Enhanced chat with autonomous features
  program
    .command('enhanced')
    .description('Start enhanced chat with tool calling and autonomous features')
    .action(async () => {
      const opts = program.opts<CLIOptions>();
      const { handleEnhancedChat } = await import('./enhanced');
      await handleEnhancedChat(opts);
    });
...
async function handleInitCommand(): Promise<void> {
  const { getAutonomousAgent } = await import('../autonomous');
  const agent = getAutonomousAgent();

  console.log(chalk.blue('🚀 Initializing CLAUDE.md for this project...'));
  console.log();
  ...
  try {
    ...
    console.log(chalk.gray('- Build and test commands'));
    console.log(chalk.gray('- Key dependencies'));
    console.log(chalk.gray('- Existing development guidelines'));
    console.log();
    console.log('You can now use this project-specific context in your Claude sessions.');
  } catch (error) {
    console.error(chalk.red(`Error creating CLAUDE.md: ${error instanceof Error ? error.message : 'Unknown error'}`));
    process.exit(1);
  }
}

async function handleMemorizeCommand(content: string, location: 'project' | 'user'): Promise<void> {
  const { getAutonomousAgent } = await import('../autonomous');
  const agent = getAutonomousAgent();

  try {
    await agent.memorize(content, location);
    const target = location === 'user' ? '~/.claude/CLAUDE.md' : './CLAUDE.md';
    console.log(chalk.green(`✅ Memorized to ${target}`));
  } catch (error) {
    console.error(chalk.red(`Error memorizing: ${error instanceof Error ? error.message : 'Unknown error'}`));
    process.exit(1);
  }
}

export { handleChatCommand, handleCompleteCommand, handleAuthCommand, handleInitCommand, handleMemorizeCommand };
